...
Finally, often, a lot of work is (still) done in Matlab.
...
\end{lstlisting}
The  template parameter \texttt{LINEARSOLVER} describes which linear solver we use in this example. We opted for a direct solver, so: 
\begin{lstlisting}
...
in the method \texttt{CellTimeEquation} in a straight-forward way. After the initialization of $v$ and the shape function $\phi_i$ and the usual loops over the quadrature points and  degrees of freedom of the cell we fill the local cell vector by
\begin{lstlisting}
...
\todo{Was ist MBB? Konnest Du vielleicht auch eine Referenz zur Konfiguration angeben?}
...
\todo{\section{Outlook}}
\todo{Ich finde nach wir vor, dass wir kurz skizzieren sollten, was wir als
  naechstes vor haben}
\todo{In this work, we introduced
the software DOpElib to solve PDEs
and Optimization problems. In the future,
we plan to implement efficient routines 
for nonstationary optimization and different 
types of control and state constraints. 
Moreover, we plan to link to other 
software packages, specifically, in order 
to use parallel computing routines.  
}
