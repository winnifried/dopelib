...
of problems involving partial differential equations (PDEs)\todocg{Die Abkuerzung PDE wird auf Seite 1 schon erlaeuternt}. 
...
\begin{description}
\item[\textit{Discretization}]\todocg{Hier scheint etwas verlorengegangen zu sein, der Satz macht so keinen Sinn:} Specify what kind of triangulation:
in most cases this is the
finite element and quadrature rules. Furthermore, other
things might be adjusted as well.
\item[\textit{Problem-data}] Define the weak form, the corresponding matrices, the
...
evaluate a functional \todocg{Ist hier was verrutscht? der Satz passt im Moment nicht so wirklich, der Teil mit dem Vektor ist irgendwie verlorengegangen?}. Thus, we 
