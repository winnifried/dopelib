...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\begin{abstract}
\todo{Ich finde die Adressen uneinheitlich. CG und ich koennten die
  Telefonnummern wegstreichen, um uns an Winni anzupassen. Und Winni
keonnte Hausnummer oder Postleitzahl hinzufuegen. Immerhin ist 
das Standart in (fast) allen Journals}
...
\label{introduction}
\todo{Reihenfolge bei der Erwaehnung von Dopelib mit vollem Namen und 
des Anfangs in 2010 passt nicht ueberein. 2010 wird am Ende erwahent und der
volle Name von Dope wird erst genannt nachdem wir bereits die Abkuerzung
benutzt haben. Das geht so nicht!}
...
\todo{Das wir in Heidelberg angefangen haben wird schon vorher erwaehnt!
  Wuerde den Satz einfach streichen}
The project was initially initiated \todocg{initially initiated - doppelt gemoppelt?} at the University of Heidelberg in 2010 and  
...
\todo{Warum Tylor-Hood? Ist inf-sup stabil wuerde ich erwaehnen}
...
\end{align}
\todo{wir schreiben newton immer klein. eigentlich ist das ein Eigenname: Newton}
...
\begin{description}
\item[\textit{Discretization}]\todocg{Hier scheint etwas verlorengegangen zu
    sein, der Satz macht so keinen Sinn:} 
\todo{Vorschlag: Specification of triangulation issues such as finite elements
  and quadrature rules}  
  \todocg{Gegenvorschlag: Specification of discretization issues...}
\todo{Finde ich gut: Koennen wir machen}
Specify what kind of triangulation:
in most cases this is the
finite element and quadrature rules. Furthermore, other
things might be adjusted as well.
\item[\textit{Problem-data}] \todo{Definition of } Define the weak form, the corresponding matrices, the
...
\item[\textit{Additional features}] ParameterReader to read in the run-time parameters
...
We split the program into the parts \texttt{main.cc}, \texttt{localfunctional.h} and \texttt{localpde.h}. The \texttt{main.cc}-file is responsible for the declaration of various objects which have mostly to do with the \textit{discretization}, the \textit{solvers} and the general \textit{program sequence}, whereas the other two contain most of the problem-data -- especially the cell-wise contributions of the weak form in \texttt{localpde.h} and the functional in \texttt{localfunctional.h}. 
...
\begin{lstlisting}
  PDEProblemContainer<LocalPDE> 
             prob_container(local_pde, dof_handler);                                            
\end{lstlisting}
...
In the next lines, we finally implement our concrete PDE. Here, we aim to provide 
...
%The main difference, and common to all functionals is that we only want to evaluate a functional 
\todocg{Ist hier was verrutscht? der Satz passt im
  Moment nicht so wirklich, der Teil mit dem Vektor ist irgendwie
  verlorengegangen?}. 
\todo{Ich hatte Deine Anmerkung so verstanden, als ob Du alles nach dem i.e.,
  geloescht 
haben wolltest. Im main TW steht noch das Original}
\todocg{Ich habe den Rest auskommentiert. Mein Vorschlag waere: The main
  difference is that as we only want to evaluate a functional, i.e. a number,
  and not a residual-vector, we just loop over the quadrature points and not
  over the degrees of freedom on the face. }
\todo{Finde ich gut. Kannst Du so machen!}
%Thus, we just loop over the quadrature points and not over the degrees of freedom on the face.
...
needs to be provided in addition to the  \todocg{Verstehe diesen Satz nicht. Das sind doch die Ableitungen des Funktionals und der PDE, warum addition?}
\begin{lstlisting} 
  void CellEquation_U(...)
   {
