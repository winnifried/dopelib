...
the equation, in \dope, we are\marginpar{here using?} using finite elements to this end. This means we need to pick a 
...
\item[Solvers] Which (non-)linear solver should we use? \marginpar{besser algorithmen?}
...
We describe now a program designed to compute a value $J(u)$, where $u=(v,p)$ is the solution of the Navier-Stokes equations \eqref{eq:ns} on the domain $\Omega$ as seen in figure~\ref{fig:example_ns} with  $f=0$, and the functional $J$ is defined as [TODO: Definition von J. Nutze Benchmark case oder lenkt das zu sehr ab?]. Additionally, for given inflow $v_{\operatorname{in}}$ we set  boundary conditions $
v = v_{\operatorname{in}} \text{ on } \Gamma_{in}\text{ and }v = 0  \text{ on } \Gamma_{wall}$.
\marginpar{Das Beispiel hier besser nochmal zusammen schreiben?}
...
After that, we create a triangulation named \texttt{triangulation}. This is done exactly as in \deal{}, so we can choose between several possibilities: We could explicitly create a grid by providing a list of vertices and cells, read in a triangulation via \texttt{dealii::GridIn}-class or use the \texttt{dealii::GridGenerator}, which offers common domains like hypercubes or hyper-balls.

[code, Grid einlesen im Benchmark-Fall, ansonsten GridGenerator something. Hier auch auf Randfarben kurz eingehen.]

Next, we create  quadrature rules (for the evaluation of integrals over cells and faces) and a finite element appropriate for the PDE at hand. We choose a gaussian quadrature of order three and the previosly mentioned Tayler-Hood element. \marginpar{Wie klar machen, dass vieles von deal.II uebernommen und nicht selbst geschrieben?}
\begin{lstlisting}
...
\begin{lstlisting}
  SpaceTimeHandler<FE> dof_handler(triangulation, finite_element);
\end{lstlisting}
...
\end{lstlisting}
Next, we incorporate the dirichlet data. First, we define two functions representing the dirichlet boundary conditions. These are given to an interface\marginpar{SimpleDirichletData weglassen?} class \texttt{SimpleDirichletData}. 
\begin{lstlisting}
  ZeroFunction<2> zf;
  InflowFunction<2> if;
  SimpleDirichletData<VECTOR, 2> dd_zf(zf);
  SimpleDirichletData<VECTOR, 2> dd_if(if);
\end{lstlisting}
Then we have to tell the \texttt{prob\_container} for which components and boundaries we prescribe the dirichlet conditions.
\begin{lstlisting}
  ...
  prob_container.SetDirichletBoundaryColors(0, comp_mask, &dd_if);
  prob_container.SetDirichletBoundaryColors(2, comp_mask, &dd_zf);
  prob_container.SetDirichletBoundaryColors(3, comp_mask, &dd_zf);
\end{lstlisting}
...
\end{lstlisting}
\marginpar{Kein IDC, dafuer quadformeln eingefuegt.}
...
\end{lstlisting}
\marginpar{outputhandler erlaeutern?}
...
%  \end{lstlisting}
  \begin{lstlisting}
     ...
     cdc.GetGradsState("last_newton_solution", _ugrads);
 \end{lstlisting}
\texttt{\_uvalues} and \texttt{\_ugrads} are of type \texttt{std::vector<...>}, with $\ldots$ symbolizing the appropriate data type. In these two lines, we load the (gradient of the) solution of the last newton iteration in all the quadrature points on the cell into the two \texttt{std::vector} objects.

In the next linex, we loop over the quadrature points and the number of dofs of the cell. In the innermost loop, we evaluate the weak form tested with $\phi_{i,h}$ on the cell with the help of the previosly selected quadrature formula. 
