...
template<typename CONTROLNONLINEARSOLVER, typename NONLINEARSOLVER,
		...
		//TODO
		void ComputeEigenfunctions(/*const StateVector<VECTOR> &u,*/const ControlVector<VECTOR> &v);
		...
		build_control_matrix_ = false; bool state_reinit_, adjoint_reinit_, eigenvaluestate_reinit_,
		...
		{ state_reinit_ = true; adjoint_reinit_ = true; gradient_reinit_ = true; eigenvaluestate_reinit_ = true; } cost_needs_precomputations_=0; }
		...
		{ state_reinit_ = true; eigenvaluestate_reinit_ = true; adjoint_reinit_ = true; gradient_reinit_ = true; } cost_needs_precomputations_ = 0; }
		...
		NONLINEARSOLVER, CONTROLINTEGRATOR, INTEGRATOR, PROBLEM, VECTOR,
		dopedim,
		dealdim>::GetNonlinearSolver( std::string type) { if ((type == "state") || (type == "tangent") || (type == "eigenvaluestate")) { return nonlinear_state_solver_; } else if ((type == "adjoint") || (type == "adjoint_hessian") || (type == "adjoint_for_ee")) { return nonlinear_adjoint_solver_; } else { throw DOpEException("No Solver for Problem type:`" + type + "' found", "StatReducedProblem::GetNonlinearSolver");
		...
		NONLINEARSOLVER, CONTROLINTEGRATOR, INTEGRATOR, PROBLEM, VECTOR,
		dopedim,
		dealdim>::GetControlNonlinearSolver() { if ((this->GetProblem()->GetType() == "gradient") || (this->GetProblem()->GetType() == "hessian")) { return nonlinear_gradient_solver_; } else { throw DOpEException( "No Solver for Problem type:`" + this->GetProblem()->GetType() + "' found", "StatReducedProblem::GetControlNonlinearSolver");
		...
		state_reinit_ = true; adjoint_reinit_ = true; gradient_reinit_ = true; eigenvaluestate_reinit_ = true; }

		build_state_matrix_ = true; build_adjoint_matrix_ = true;

		GetU().ReInit(); GetZ().ReInit(); GetDU().ReInit(); GetDZ().ReInit(); GetZForEE().ReInit();

		build_control_matrix_ = true; cost_needs_precomputations_ = 0;
		...
		NONLINEARSOLVER, CONTROLINTEGRATOR, INTEGRATOR, PROBLEM, VECTOR,
		dopedim,
		dealdim>::ComputeEigenfunctions( const ControlVector<VECTOR> &q ) {
			this->GetIntegrator().AddDomainData("control", &(q.GetSpacialVector())); //TODO Warum nicht GetControlIntegrator?!
			...
			this->GetOutputHandler()->Write("Computing Eigenvalues and Eigenvectors:",
			                                     4 + this->GetBasePriority());
			this->GetProblem()->AddAuxiliaryToIntegrator(this->GetIntegrator());

			int index_eigenval = 0;

			AddUDD();

		    std::vector<PETScWrappers::MPI::Vector> eigenfunctions;
		    std::vector<double> eigenvalues;
		    eigenfunctions.resize((int) (10));
		    eigenvalues.resize(eigenfunctions.size());

			build_state_matrix_ = this->GetNonlinearSolver("eigenvaluestate").NonlinearSolve(
			                             problem, (GetU().GetSpacialVector()),eigenvalues, eigenfunctions, true, build_state_matrix_/*, n*/);

//			this->GetIntegrator().AddDomainData("state", (eigenfunctions[index_eigenval]));
			std::cout << "################################################################" << std::endl;
...
//
//			//-------------------- AB HIER ABLEITUNGEN ----------------------//
//			// welcher Eigenwert/ -vektor soll abgeleitet werden... Position im Vektor festlegen
//
//
//			PETScWrappers::MPI::Vector eigenfunction;
//			double eigenvalue;
//
//			eigenfunction = eigenfunctions[index_eigenval];
//			eigenvalue = eigenvalues[index_eigenval];
//
//
//			double eigenvalue_derivative = 0;
//			this->SetProblemType("eigenvalueadjoint");
////			auto &problem2 = this->GetProblem()->GetEigenvalueAdjointProblem();
//			//TODO noch einen Solver für "eigenvalueadjoint anlegen??
//			this->GetNonlinearSolver("eigenvaluestate").EigenvalueDerivativeSolve(problem/*2*/, eigenvalue, eigenfunction, eigenvalue_derivative);
//
//
//			std::cout << "derivative in eigenvalueproblem.h" << eigenvalue_derivative << std::endl;
//
//			//-------------------- ENDE ABLEITUNGEN ----------------------//
//			DeleteUDD();
//			this->GetProblem()->DeleteAuxiliaryFromIntegrator(this->GetIntegrator());
...
//
//		this->GetOutputHandler()->Write("Computing Reduced Gradient:", 4 + this->GetBasePriority());
//
//		//Preparations for ControlInTheDirichletData
//		VECTOR tmp; if (this->GetProblem()->HasControlInDirichletData()) { tmp.reinit(GetU().GetSpacialVector()); this->SetProblemType("adjoint"); auto &problem = this->GetProblem()->GetAdjointProblem();
//
//		this->GetProblem()->AddAuxiliaryToIntegrator(this->GetIntegrator());
//
//		if (dopedim == dealdim) {
...
//			this->GetIntegrator().AddParamData("control", &(q.GetSpacialVectorCopy()));
//		} else {
//			throw DOpEException("dopedim not implemented", "StatReducedProblem::ComputeReducedGradient");
//		}
...
//		tmp *= -1.;
//
//		if (dopedim == dealdim) { this->GetIntegrator().DeleteDomainData("control"); } else if (dopedim == 0) { this->GetIntegrator().DeleteParamData("control"); q.UnLockCopy(); } else { throw DOpEException("dopedim not implemented", "StatReducedProblem::ComputeReducedGradient"); } if (cost_needs_precomputations_ != 0) { this->GetIntegrator().DeleteParamData("cost_functional_pre"); }
//
//		this->GetIntegrator().DeleteDomainData("state"); this->GetIntegrator().DeleteDomainData("last_newton_solution"); this->GetProblem()->DeleteAuxiliaryFromIntegrator( this->GetIntegrator()); }
//		//Endof Dirichletdata Preparations
//		this->SetProblemType("gradient"); if (gradient_reinit_ == true) { GetControlNonlinearSolver().ReInit(*(this->GetProblem())); gradient_reinit_ = false; }
//
//		this->GetProblem()->AddAuxiliaryToIntegrator( this->GetControlIntegrator());
//
//		if (dopedim == dealdim) { this->GetControlIntegrator().AddDomainData("control", &(q.GetSpacialVector())); } else if (dopedim == 0) { this->GetControlIntegrator().AddParamData("control", &(q.GetSpacialVectorCopy())); } else { throw DOpEException("dopedim not implemented", "StatReducedProblem::ComputeReducedGradient"); } if (cost_needs_precomputations_ != 0) { auto func_vals = GetAuxiliaryParams("cost_functional_pre"); this->GetControlIntegrator().AddParamData("cost_functional_pre",&(func_vals->second)); } this->GetControlIntegrator().AddDomainData("state", &(GetU().GetSpacialVector())); this->GetControlIntegrator().AddDomainData("adjoint", &(GetZ().GetSpacialVector())); if (this->GetProblem()->HasControlInDirichletData()) this->GetControlIntegrator().AddDomainData("adjoint_residual", &tmp);
//
//		gradient_transposed = 0.; if (dopedim == dealdim) { this->GetControlIntegrator().AddDomainData("last_newton_solution", &(gradient_transposed.GetSpacialVector()));
////        this->GetControlIntegrator().ComputeNonlinearResidual(
////            *(this->GetProblem()), gradient.GetSpacialVector(), true);
//		this->GetControlIntegrator().ComputeNonlinearResidual( *(this->GetProblem()), gradient.GetSpacialVector()); this->GetControlIntegrator().DeleteDomainData("last_newton_solution"); } else if (dopedim == 0) { this->GetControlIntegrator().AddParamData("last_newton_solution", &(gradient_transposed.GetSpacialVectorCopy()));
////        this->GetControlIntegrator().ComputeNonlinearResidual(
////            *(this->GetProblem()), gradient.GetSpacialVector(), true);
...
//		}
//
//		gradient *= -1.; gradient_transposed = gradient;
//
//		//Compute l^2 representation of the Gradient
//
//		build_control_matrix_ = this->GetControlNonlinearSolver().NonlinearSolve( *(this->GetProblem()), gradient_transposed.GetSpacialVector(), true, build_control_matrix_); if (dopedim == dealdim) { this->GetControlIntegrator().DeleteDomainData("control"); } else if (dopedim == 0) { this->GetControlIntegrator().DeleteParamData("control"); q.UnLockCopy(); } else { throw DOpEException("dopedim not implemented", "StatReducedProblem::ComputeReducedGradient"); } if (cost_needs_precomputations_ != 0) { this->GetControlIntegrator().DeleteParamData("cost_functional_pre"); } this->GetControlIntegrator().DeleteDomainData("state"); this->GetControlIntegrator().DeleteDomainData("adjoint"); if (this->GetProblem()->HasControlInDirichletData()) this->GetControlIntegrator().DeleteDomainData("adjoint_residual");
//
//		this->GetProblem()->DeleteAuxiliaryFromIntegrator( this->GetControlIntegrator());
//
//		this->GetOutputHandler()->Write(gradient, "Gradient" + this->GetPostIndex(), this->GetProblem()->GetDoFType()); this->GetOutputHandler()->Write(gradient_transposed, "Gradient_Transposed" + this->GetPostIndex(), this->GetProblem()->GetDoFType());
		...
		NONLINEARSOLVER, CONTROLINTEGRATOR, INTEGRATOR, PROBLEM, VECTOR,
		dopedim,
		dealdim>::ComputeReducedCostFunctional( const ControlVector<VECTOR> &q) { this->ComputeReducedState(q);
//
//		this->GetOutputHandler()->Write("Computing Cost Functional:", 4 + this->GetBasePriority());
//
//		this->SetProblemType("cost_functional"); cost_needs_precomputations_ = this->GetProblem()->FunctionalNeedPrecomputations(); if (cost_needs_precomputations_ != 0) { unsigned int n_pre = cost_needs_precomputations_; AllocateAuxiliaryParams("cost_functional_pre",n_pre);
//
//		this->GetProblem()->AddAuxiliaryToIntegrator(this->GetIntegrator());
//
//		if (dopedim == dealdim) { this->GetIntegrator().AddDomainData("control", &(q.GetSpacialVector())); } else if (dopedim == 0) { this->GetIntegrator().AddParamData("control", &(q.GetSpacialVectorCopy())); } else { throw DOpEException("dopedim not implemented", "StatReducedProblem::ComputeReducedCostFunctional"); } this->GetIntegrator().AddDomainData("state", &(GetU().GetSpacialVector()));
//
//		CalculatePreFunctional("cost_functional","_pre",n_pre,0);
//
//		if (dopedim == dealdim) { this->GetIntegrator().DeleteDomainData("control"); } else if (dopedim == 0) { this->GetIntegrator().DeleteParamData("control"); q.UnLockCopy(); } else { throw DOpEException("dopedim not implemented", "StatReducedProblem::ComputeReducedCostFunctional"); } this->GetIntegrator().DeleteDomainData("state"); this->GetProblem()->DeleteAuxiliaryFromIntegrator(this->GetIntegrator()); this->SetProblemType("cost_functional"); }
//		//End of Precomputations
//
//		this->GetProblem()->AddAuxiliaryToIntegrator(this->GetIntegrator());
//
//		if (dopedim == dealdim) { this->GetIntegrator().AddDomainData("control", &(q.GetSpacialVector())); } else if (dopedim == 0) { this->GetIntegrator().AddParamData("control", &(q.GetSpacialVectorCopy())); } else { throw DOpEException("dopedim not implemented", "StatReducedProblem::ComputeReducedCostFunctional"); } if (cost_needs_precomputations_ != 0) { auto func_vals = GetAuxiliaryParams("cost_functional_pre"); this->GetIntegrator().AddParamData("cost_functional_pre",&(func_vals->second)); } this->GetIntegrator().AddDomainData("state", &(GetU().GetSpacialVector())); double ret = 0; bool found = false;
//
...
//
//		if (dopedim == dealdim) { this->GetIntegrator().DeleteDomainData("control"); } else if (dopedim == 0) { this->GetIntegrator().DeleteParamData("control"); q.UnLockCopy(); } else { throw DOpEException("dopedim not implemented", "StatReducedProblem::ComputeReducedCostFunctional"); } if (cost_needs_precomputations_ != 0) { this->GetIntegrator().DeleteParamData("cost_functional_pre"); } this->GetIntegrator().DeleteDomainData("state"); this->GetProblem()->DeleteAuxiliaryFromIntegrator(this->GetIntegrator());
//
//		this->GetFunctionalValues()[0].push_back(ret); return ret;
		}
...
//		this->GetProblem()->AddAuxiliaryToIntegrator(this->GetIntegrator());
//
//		this->GetIntegrator().AddDomainData("state", &(GetU().GetSpacialVector())); this->GetControlIntegrator().AddDomainData("state", &(GetU().GetSpacialVector()));
//
//		if (dopedim == dealdim) { this->GetIntegrator().AddDomainData("dq", &(direction.GetSpacialVector())); this->GetIntegrator().AddDomainData("control", &(q.GetSpacialVector())); } else if (dopedim == 0) { this->GetIntegrator().AddParamData("dq", &(direction.GetSpacialVectorCopy())); this->GetIntegrator().AddParamData("control", &(q.GetSpacialVectorCopy())); } else { throw DOpEException("dopedim not implemented", "StatReducedProblem::ComputeReducedHessianVector"); }
//
//		//tangent Matrix is the same as state matrix
//		build_state_matrix_ = this->GetNonlinearSolver("tangent").NonlinearSolve( problem, (GetDU().GetSpacialVector()), true, build_state_matrix_);
//
//		this->GetOutputHandler()->Write((GetDU().GetSpacialVector()), "Tangent" + this->GetPostIndex(), problem.GetDoFType()); } //End Tangent Calculation
//
//		this->GetIntegrator().AddDomainData("adjoint", &(GetZ().GetSpacialVector())); this->GetIntegrator().AddDomainData("tangent", &(GetDU().GetSpacialVector())); this->GetControlIntegrator().AddDomainData("adjoint", &(GetZ().GetSpacialVector())); this->GetControlIntegrator().AddDomainData("tangent", &(GetDU().GetSpacialVector()));
//
//		//After the Tangent has been computed, we can precompute
//		//cost functional derivative-values (if necessary)
//		if (cost_needs_precomputations_ != 0) { unsigned int n_pre = cost_needs_precomputations_; AllocateAuxiliaryParams("cost_functional_pre_tangent",n_pre); CalculatePreFunctional("cost_functional","_pre_tangent",n_pre,0); } // End precomputation of values
//
//		this->GetOutputHandler()->Write("\tSolving Adjoint Hessian:", 5 + this->GetBasePriority()); this->SetProblemType("adjoint_hessian"); {
//		//Adjoint_Hessian
...
//		build_adjoint_matrix_ = this->GetNonlinearSolver("adjoint_hessian").NonlinearSolve( problem, (GetDZ().GetSpacialVector()), true, build_adjoint_matrix_);
//
//		this->GetOutputHandler()->Write((GetDZ().GetSpacialVector()), "Hessian" + this->GetPostIndex(), problem.GetDoFType());
//
//		this->GetIntegrator().AddDomainData("adjoint_hessian", &(GetDZ().GetSpacialVector())); this->GetControlIntegrator().AddDomainData("adjoint_hessian", &(GetDZ().GetSpacialVector()));
//
//		this->GetOutputHandler()->Write( "\tComputing Representation of the Hessian:", 5 + this->GetBasePriority()); } //End Adjoint Hessian
//																													   //Preparations for Control In The Dirichlet Data
//		VECTOR tmp; VECTOR tmp_second; if (this->GetProblem()->HasControlInDirichletData()) { tmp.reinit(GetU().GetSpacialVector()); tmp_second.reinit(GetU().GetSpacialVector()); this->SetProblemType("adjoint"); {
//		// Adjoint
...
//		this->GetProblem()->DeleteAuxiliaryFromIntegrator(this->GetIntegrator());
//
//		this->SetProblemType("hessian"); this->GetProblem()->AddAuxiliaryToIntegrator( this->GetControlIntegrator()); if (dopedim == dealdim) { this->GetIntegrator().DeleteDomainData("dq"); this->GetIntegrator().DeleteDomainData("control"); this->GetControlIntegrator().AddDomainData("dq", &(direction.GetSpacialVector())); this->GetControlIntegrator().AddDomainData("control", &(q.GetSpacialVector())); } else if (dopedim == 0) { this->GetIntegrator().DeleteParamData("dq"); this->GetIntegrator().DeleteParamData("control"); direction.UnLockCopy(); q.UnLockCopy(); this->GetControlIntegrator().AddParamData("dq", &(direction.GetSpacialVectorCopy())); this->GetControlIntegrator().AddParamData("control", &(q.GetSpacialVectorCopy())); } else { throw DOpEException("dopedim not implemented", "StatReducedProblem::ComputeReducedHessianVector"); } if (this->GetProblem()->HasControlInDirichletData()) { this->GetControlIntegrator().AddDomainData("adjoint_residual", &tmp); this->GetControlIntegrator().AddDomainData("hessian_residual", &tmp_second); }
//
//		{ hessian_direction_transposed = 0.; if (dopedim == dealdim) { this->GetControlIntegrator().AddDomainData("last_newton_solution", &(hessian_direction_transposed.GetSpacialVector()));
////          this->GetControlIntegrator().ComputeNonlinearResidual(
////              *(this->GetProblem()), hessian_direction.GetSpacialVector(),
////              true);
//		this->GetControlIntegrator().ComputeNonlinearResidual( *(this->GetProblem()), hessian_direction.GetSpacialVector()); this->GetControlIntegrator().DeleteDomainData("last_newton_solution"); } else if (dopedim == 0) { this->GetControlIntegrator().AddParamData("last_newton_solution", &(hessian_direction_transposed.GetSpacialVectorCopy()));
////         this->GetControlIntegrator().ComputeNonlinearResidual(
////             *(this->GetProblem()), hessian_direction.GetSpacialVector(),
////              true);
//		this->GetControlIntegrator().ComputeNonlinearResidual( *(this->GetProblem()), hessian_direction.GetSpacialVector()); this->GetControlIntegrator().DeleteParamData("last_newton_solution"); hessian_direction_transposed.UnLockCopy(); } hessian_direction *= -1.; hessian_direction_transposed = hessian_direction;
...
//
//		this->GetOutputHandler()->Write(hessian_direction, "HessianDirection" + this->GetPostIndex(), this->GetProblem()->GetDoFType()); this->GetOutputHandler()->Write(hessian_direction_transposed, "HessianDirection_Transposed" + this->GetPostIndex(), this->GetProblem()->GetDoFType()); }
//
//		if (dopedim == dealdim) { this->GetControlIntegrator().DeleteDomainData("dq"); this->GetControlIntegrator().DeleteDomainData("control"); } else if (dopedim == 0) { this->GetControlIntegrator().DeleteParamData("dq"); this->GetControlIntegrator().DeleteParamData("control"); direction.UnLockCopy(); q.UnLockCopy(); } else { throw DOpEException("dopedim not implemented", "StatReducedProblem::ComputeReducedHessianVector"); } this->GetIntegrator().DeleteDomainData("state"); this->GetIntegrator().DeleteDomainData("adjoint"); this->GetIntegrator().DeleteDomainData("tangent"); this->GetIntegrator().DeleteDomainData("adjoint_hessian"); this->GetControlIntegrator().DeleteDomainData("state"); this->GetControlIntegrator().DeleteDomainData("adjoint"); this->GetControlIntegrator().DeleteDomainData("tangent"); this->GetControlIntegrator().DeleteDomainData("adjoint_hessian"); if (this->GetProblem()->HasControlInDirichletData()) { this->GetControlIntegrator().DeleteDomainData("adjoint_residual"); this->GetControlIntegrator().DeleteDomainData("hessian_residual"); } this->GetProblem()->DeleteAuxiliaryFromIntegrator( this->GetControlIntegrator());
//
