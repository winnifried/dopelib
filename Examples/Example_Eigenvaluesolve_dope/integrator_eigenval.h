...
#ifndef Integrator_eigenval_H_
#define Integrator_eigenval_H_
...
template <typename INTEGRATORDATACONT, typename VECTOR, typename SCALAR,
          int dim>
class Integrator_eigenval {
public:
  Integrator_eigenval(INTEGRATORDATACONT &idc1);
  /**
   ...
   */
  Integrator_eigenval(INTEGRATORDATACONT &idc1, INTEGRATORDATACONT &idc2);
  ...
  void ComputeMatrix(PROBLEM &pde, MATRIX &matrix);
    ...
    template <typename PROBLEM, typename MATRIX>
  void ComputeMassMatrix(PROBLEM &pde, MATRIX &matrix);
...
template <typename INTEGRATORDATACONT, typename VECTOR, typename SCALAR,
          int dim>
Integrator_eigenval<INTEGRATORDATACONT, VECTOR, SCALAR, dim>::Integrator_eigenval(
...
template <typename INTEGRATORDATACONT, typename VECTOR, typename SCALAR,
          int dim>
void Integrator_eigenval<INTEGRATORDATACONT, VECTOR, SCALAR, dim>::ReInit() {}
...
template <typename INTEGRATORDATACONT, typename VECTOR, typename SCALAR,
          int dim>
template <typename PROBLEM>
void Integrator_eigenval<INTEGRATORDATACONT, VECTOR, SCALAR,
...
template <typename PROBLEM>
void Integrator_eigenval<INTEGRATORDATACONT, VECTOR, SCALAR,
                dim>::ComputeEigenvalueDerivative(PROBLEM &pde, SCALAR eigenvalue,PETScWrappers::MPI::Vector &eigenvector, SCALAR derivative_eigenvalue) {

	std::cout << "In Integrator: Compute EigenvalueDerivative" << std::endl;
  ...
  for (; element[0] != endc[0]; element[0]++) {
    for (unsigned int dh = 1; dh < dof_handler.size(); dh++) {
      if (element[dh] == endc[dh]) {
        throw DOpEException("Elementnumbers in DoFHandlers are not matching!",
                            "Integrator::ComputeEigenvalueDerivative");
      }
    }
    ...
    if (element[0]->is_locally_owned()) {
      ...
      pde.ElementEquation(edc, local_vector, 1.,1.);
      pde.ElementMassEquation(edc, local_vector, eigenvalue,1.);
//
//      // LocalToGlobal
//      const auto &C = pde.GetDoFConstraints();
//      element[0]->get_dof_indices(local_dof_indices);
...
template <typename INTEGRATORDATACONT, typename VECTOR, typename SCALAR,
          int dim>
template <typename PROBLEM>
void Integrator_eigenval<INTEGRATORDATACONT, VECTOR, SCALAR, dim>::ComputeNonlinearLhs(
...
template <typename INTEGRATORDATACONT, typename VECTOR, typename SCALAR,
          int dim>
template <typename PROBLEM>
void Integrator_eigenval<INTEGRATORDATACONT, VECTOR, SCALAR, dim>::ComputeNonlinearRhs(
    PROBLEM &pde, VECTOR &residual) {
  residual = 0.;
  // Begin integration
  unsigned int dofs_per_element;
...
void Integrator_eigenval<INTEGRATORDATACONT, VECTOR, SCALAR, dim>::ComputeMatrix(
  ...
  for (; element[0] != endc[0]; element[0]++) {
    ...
    if (element[0]->is_locally_owned()) {
      ...
      const auto &C = pde.GetDoFConstraints();
      element[0]->get_dof_indices(local_dof_indices);
//      std::cout << "in Integratoreigenval" << std::endl;
      C.distribute_local_to_global(local_matrix, local_dof_indices, matrix);
//      std::cout << "in Integratoreigenval2" << std::endl;
    } // endif locally owned
...
template <typename INTEGRATORDATACONT, typename VECTOR, typename SCALAR,
          int dim>
template <typename PROBLEM, typename MATRIX>
void Integrator_eigenval<INTEGRATORDATACONT, VECTOR, SCALAR, dim>::ComputeMassMatrix(
    PROBLEM &pde, MATRIX &matrix) {
  matrix = 0.;
  // Begin integration
  unsigned int dofs_per_element;
...
template <typename INTEGRATORDATACONT, typename VECTOR, typename SCALAR,
          int dim>
template <typename PROBLEM>
SCALAR Integrator_eigenval<INTEGRATORDATACONT, VECTOR, SCALAR, dim>::ComputeDomainScalar(
    PROBLEM &pde) {
  {
    SCALAR ret = 0.;
...
template <typename INTEGRATORDATACONT, typename VECTOR, typename SCALAR,
          int dim>
template <typename PROBLEM>
SCALAR Integrator_eigenval<INTEGRATORDATACONT, VECTOR, SCALAR, dim>::ComputePointScalar(
    PROBLEM &pde) {
...
template <typename PROBLEM>
SCALAR
Integrator_eigenval<INTEGRATORDATACONT, VECTOR, SCALAR, dim>::ComputeBoundaryScalar(
    PROBLEM &pde) {
  SCALAR ret = 0.;
  // Begin integration
  const auto &dof_handler =
  ...
  if (!need_boundary_integrals) {
    throw DOpEException("No boundary colors given!",
                        "Integrator_eigenval::ComputeBoundaryScalar");
  }
...
template <typename INTEGRATORDATACONT, typename VECTOR, typename SCALAR,
          int dim>
template <typename PROBLEM>
SCALAR Integrator_eigenval<INTEGRATORDATACONT, VECTOR, SCALAR, dim>::ComputeFaceScalar(
    PROBLEM &pde) {
  SCALAR ret = 0.;
  // Begin integration
  const auto &dof_handler =
...
template <typename INTEGRATORDATACONT, typename VECTOR, typename SCALAR,
          int dim>
void Integrator_eigenval<INTEGRATORDATACONT, VECTOR, SCALAR, dim>::AddDomainData(
...
template <typename INTEGRATORDATACONT, typename VECTOR, typename SCALAR,
          int dim>
void Integrator_eigenval<INTEGRATORDATACONT, VECTOR, SCALAR, dim>::DeleteDomainData(
    std::string name) {
...
Integrator_eigenval<INTEGRATORDATACONT, VECTOR, SCALAR, dim>::GetDomainData() const {
  return domain_data_;
}
...
template <typename INTEGRATORDATACONT, typename VECTOR, typename SCALAR,
          int dim>
void Integrator_eigenval<INTEGRATORDATACONT, VECTOR, SCALAR, dim>::AddParamData(
...
template <typename INTEGRATORDATACONT, typename VECTOR, typename SCALAR,
          int dim>
void Integrator_eigenval<INTEGRATORDATACONT, VECTOR, SCALAR, dim>::DeleteParamData(
    std::string name) {
...
template <typename INTEGRATORDATACONT, typename VECTOR, typename SCALAR,
          int dim>
void Integrator_eigenval<INTEGRATORDATACONT, VECTOR, SCALAR, dim>::DeleteAllData() {
  param_data_.clear();
  domain_data_.clear();
}
...
Integrator_eigenval<INTEGRATORDATACONT, VECTOR, SCALAR, dim>::GetParamData() const {
  return param_data_;
}
...
template <typename INTEGRATORDATACONT, typename VECTOR, typename SCALAR,
          int dim>
INTEGRATORDATACONT &Integrator_eigenval<INTEGRATORDATACONT, VECTOR, SCALAR,
