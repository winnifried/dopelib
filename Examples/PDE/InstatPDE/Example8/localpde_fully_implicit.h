...
public:
	...
	double pf = uvalues_[q_point](2);
	    ...
	    const Tensor<1, 2> phi_i_grads_pf = state_fe_values[phasefield].gradient(i, q_point);
            ...
            local_vector(i) += scale
                               * (scalar_product(((1.0-constant_k_) * pf * pf + constant_k_) *
            ...
            for (unsigned int j = 0; j < n_dofs_per_element; j++)
              {
                ...
                local_matrix(j, i) += scale
                                      * (// Fully monolithic
                                        scalar_product((1-constant_k_) * 2.0 * pf * phi_pf[i] * stress_term_plus,
                                                       phi_grads_u[j])
                                        + scalar_product(((1-constant_k_) * pf * pf + constant_k_) *
   ...
   * state variable, namely displacement and phasefield.
   */
