...
#ifndef SNEDDONMIXED_
#define SNEDDONMIXED_
...
#endif
class SneddonMixedPDE : public PDEInterface<EDC, FDC, DH, VECTOR, dealdim>
{
public:
...
SneddonMixedPDE(ParameterReader &param_reader,double eps, double d) :
			...
			double scale_ico) override
	{
		...
		last_timestep_uvalues_.resize(n_q_points, Vector<double>(5));
		...
		const FEValuesExtractors::Scalar multiplier(4);
		...
		for (unsigned int q_point = 0; q_point < n_q_points; q_point++)
		{
			...
			stress_term.clear();
			stress_term = press * Identity + 2 * mu * E;
			...
			for (unsigned int i = 0; i < n_dofs_per_element; i++)
			{
				...
				if(compressible_ || fabs(edc.GetCenter()[0]) > 10 || fabs(edc.GetCenter()[1]) > 10 || (comp_in_fracture_ && fabs(edc.GetCenter()[0]) < 1. && fabs(edc.GetCenter()[1]) < d_))
				{
				  local_vector(i) += scale_ico
				    * ((-1.0/lambda)* press * phi_i_p
				...
				if(fabs(state_fe_values[multiplier].value(i,q_point) - 1.) < std::numeric_limits<double>::epsilon())
				{
					...
					if(n_neig == 4)
					{
						...
						local_vector(i) += scale * weight* (uvalues_[q_point][4]
																			  - std::max(0.,uvalues_[q_point][4]+s_*(pf-old_timestep_pf)));
						...
						for(unsigned int j = 0; j < n_dofs_per_element; j++)
						{
							...
							{
								local_vector(j) += scale * weight* uvalues_[q_point][4];
							}
						}
					}
					else //Boundary or hanging node (no weight, so it works if hanging)
					{
						local_vector(i) += scale * uvalues_[q_point][4];
					}
				}
			}
		}
			...
			double scale_ico) override
	{
		...
		for (unsigned int q_point = 0; q_point < n_q_points; q_point++)
		{
			...
			Tensor<2,2> stress_term;
			stress_term.clear();
			stress_term = press * Identity // lame_coefficient_lambda_ * tr_E = press
					+ 2 * mu * E;
			...
			for (unsigned int i = 0; i < n_dofs_per_element; i++)
			{
			  ...
			  stress_term_LinU = phi_p[i] * Identity
			    + 2 * mu * E_LinU;
			  ...
			  for (unsigned int j = 0; j < n_dofs_per_element; j++)
			  {
				...
				* ( (phi_grads_u[i][0][0] + phi_grads_u[i][1][1]) * phi_p[j]
			      ...
			      )
			    {
			      ...
			      {
				//max = 0
				if( (uvalues_[q_point][4]+s_*(pf-old_timestep_pf)) <= 0. )
				{
			...
			const EDC<DH, VECTOR, dealdim> &edc_w, double &sum, double scale) override
	{
	  ...
	  // I removed the other choices so that it will not be accidentely used
	  // for the mixed formulation for what it is not done and tested
		...
		if( this->GetTime() != 0.0 )
		{
			...
			PI_h_z_.resize(n_q_points, Vector<double>(5));
			lap_pf_.resize(n_q_points, Vector<double>(5));
			uvalues_.resize(n_q_points, Vector<double>(5));
			last_timestep_uvalues_.resize(n_q_points, Vector<double>(5));
			...
			auxvalues_.resize(n_q_points, Vector<double>(5));
			...
			for (unsigned int q_point = 0; q_point < n_q_points; q_point++)
			{
				...
				// bei pressure driven kann die Energie definiert ueber p auch negativ sein
				// da p auch p_g ausgleicht, wir koennen hier aber die
				// Definition ueber div u nehmen, da nur fuer den elliptischen Fall
				// die Norm definiert ist 
				// d.h. wir setzen div u = 0 fuer nu=0.5
				//  und sonst definieren wir es uber lambda (das muessen wir noch ueberpruefen)
				Tensor<2,2> stress_term;
				stress_term.clear();
				...
				if(compressible_ || fabs(edc.GetCenter()[0]) > 10 || fabs(edc.GetCenter()[1]) > 10 || (comp_in_fracture_ && fabs(edc.GetCenter()[0]) < 1. && fabs(edc.GetCenter()[1]) < d_))
				  ...
				  {
				    // im inkompressiblen Fall ist die tr E= div u = 0
				    stress_term =  2 * mu * E;
				  }
				...
				if (weightEnergy <= (-1*pow(10,-8)))
				  {
				    //std::cout<<"weight energy is negativ"<<weightEnergy<<std::endl;

				    weightEnergy= abs(weightEnergy);
				  }
					...
					if(fabs(state_fe_values[phasefield].value(i,q) - 1.) < std::numeric_limits<double>::epsilon())
					{
							...
							// but if q is in contact
						{
							if(uvalues_[q](4) > 0 )
							{
								...
								for (unsigned int q_point = 0; q_point < n_q_points; q_point++)
								{
									complRes += (1.0/auxvalues_[q](4))*uvalues_[q](4)*(fabs(last_timestep_uvalues_[q_point][2]-uvalues_[q_point][2]))*state_fe_values[phasefield].value(i,q_point)*state_fe_values.JxW(q_point);
								}
							}
						}
					}
	  ...
	  if( this->GetTime() != 0.0 )
		{
			...
			ugrads_.resize(n_q_points, std::vector<Tensor<1, dealdim> >(5));
			ugrads_nbr_.resize(n_q_points, std::vector<Tensor<1, dealdim> >(5));
			PI_h_z_.resize(n_q_points, Vector<double>(5));
			auxvalues_.resize(n_q_points, Vector<double>(5));
			...
			for (unsigned int q_point = 0; q_point < n_q_points; q_point++)
			{
				...
				if(compressible_ || fabs(fdc.GetCenter()[0]) > 10 || fabs(fdc.GetCenter()[1]) > 10 || (comp_in_fracture_ && fabs(fdc.GetCenter()[0]) < 1. && fabs(fdc.GetCenter()[1]) < d_))
				  {
				    stress_term = lambda * tr_E * Identity
				      + 2 * mu * E;
				  }
				...
				localSum += scale * (res * PI_h_z_[q_point](2))
			   ...
			   double scale) override
	{
	  if( this->GetTime() != 0.0 )
		...
		edc.GetFEValuesState();
	      ...
	      uvalues_.resize(n_q_points, Vector<double>(5));
	      last_timestep_uvalues_.resize(n_q_points, Vector<double>(5));
	      //obstacle_.resize(n_q_points, Vector<double>(5));
