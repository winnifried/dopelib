...
main(int argc, char **argv)
{
  ...
  SneddonMixedPDE<CDC, FDC, DOFHANDLER, VECTOR, DIM>::declare_params(pr);
  declare_params(pr);
  ...
  SneddonMixedPDE<CDC, FDC, DOFHANDLER, VECTOR, DIM> LPDE(pr,eps,meshsize);
  ...
  LocalFunctionalBulkMixed<CDC, FDC, DOFHANDLER, VECTOR, DIM> LFBM(pr);
  ...
  dealii::ComponentMask comp_mask(5,false);
  ...
  comp_mask.set(4,false); //Lagrange-multiplier
#else
  ...
  comp_mask[4] = false; //Lagrange-multiplier
#endif
  // Fixed boundaries
