...
class LocalPDE: public PDEInterface<CellDataContainer,FaceDataContainer,dealii::DoFHandler<dealdim>, VECTOR,  dealdim>
{
  public:
                      ...
                      double scale, double)
    {
      ...
      _ugrads.resize(n_q_points, vector<Tensor<1, 3> > (3));
	...
	ugrads[0][2] = _ugrads[q_point][0][2];

        ugrads[1][0] = _ugrads[q_point][1][0];
        ugrads[1][1] = _ugrads[q_point][1][1];
	ugrads[1][2] = _ugrads[q_point][1][2];

	ugrads[2][0] = _ugrads[q_point][2][0];
        ugrads[2][1] = _ugrads[q_point][2][1];
	ugrads[2][2] = _ugrads[q_point][2][2];


	for (unsigned int i = 0; i < n_dofs_per_cell; i++)
        {
	  const Tensor<2, 3> phi_i_grads_u = state_fe_values[displacements].gradient(i, q_point);
    ...
    void CellMatrix(const CellDataContainer<dealii::DoFHandler<dealdim>, 
		    VECTOR, dealdim>& cdc, FullMatrix<double> &local_entry_matrix,
		    double scale, double)
    {
      ...
      std::vector<Tensor<2, 3> > phi_grads_u(n_dofs_per_cell);
      ...
      for (unsigned int q_point = 0; q_point < n_q_points; q_point++)
      {
        ...
        for (unsigned int i = 0; i < n_dofs_per_cell; i++)
        {
          ...
          {
            local_entry_matrix(i, j) +=scale *  scalar_product(phi_grads_u[j], phi_grads_u[i])
                * state_fe_values.JxW(q_point);
          }
        }
      }
	...
	for(unsigned int i = 0; i < n_dofs_per_cell; i++)
	{
	  const Tensor<1, 3> phi_i_u = state_fe_values[displacements].value(i, q_point);
