...
\begin{itemize}
\item {\verb CellEquation_U }: Weak form of the adjoint equation.
\item {\verb CellMatrix_T }: The FE matrix for the adjoint problem.
...
\begin{itemize}
\item {\verb StrongCellResidual }: The cell residual, see \eqref{dwr.cell_residual}.
\item {\verb StrongFaceResidual }: The terms in \eqref{dwr.error_identity} that lies in the interiour (i.e. the jumps).
...
\begin{itemize}
\item {\verb DOFH_higher_order }: With some higher order Finite Elemnts and the already defined triangulation, we build this \texttt{SpaceTimeHandler}. This is needed because  we want to use the patch-wise higher order interpolation of the weights.
