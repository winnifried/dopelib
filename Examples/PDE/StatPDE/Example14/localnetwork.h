...
public:
  ...
  unsigned int GetNPipes() const {return 2;}
  unsigned int GetNNodes() const {return 3;}
  void PipeCouplingResidual(dealii::Vector<double>& res, const dealii::Vector<double>& u) const
  {
    ...
    assert(res.size()==4); //2 Pipes with two components each
