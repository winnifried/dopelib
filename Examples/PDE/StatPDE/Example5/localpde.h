...
//          factor = 0.;
    ...
    void BoundaryEquation (const FaceDataContainer<DoFHandler, VECTOR, dealdim>& fdc,
			   dealii::Vector<double> &local_cell_vector,
    ...
    void BoundaryMatrix (const FaceDataContainer<DoFHandler, VECTOR, dealdim>& /*fdc*/,
			 dealii::FullMatrix<double> &/*local_entry_matrix*/, double /*scale*/, double /*scale_ico*/)
    {
      assert(this->_problem_type == "state");
    }

    void BoundaryRightHandSide (const FaceDataContainer<DoFHandler, VECTOR, dealdim>& /*fdc*/,
				dealii::Vector<double> &/*local_cell_vector*/,
				double /*scale*/)
     {
       	assert(this->_problem_type == "state");
