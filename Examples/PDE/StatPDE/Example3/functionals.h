...
class LocalBoundaryFaceFunctionalDrag: public FunctionalInterface<CellDataContainer,FaceDataContainer,DOFHANDLER, VECTOR, dealdim>
{
  public:
    ...
    LocalBoundaryFaceFunctionalDrag(ParameterReader &param_reader)
    {
      ...
      _lame_coefficient_lambda = (2 * _poisson_ratio_nu * _lame_coefficient_mu) / (1.0 - 2
          * _poisson_ratio_nu);
    }
    ...
    double BoundaryValue(const FaceDataContainer<DOFHANDLER,VECTOR,2>& fdc)
    {
      ...
      if (color == 80)
      {
        ...
        for (unsigned int q_point = 0; q_point < n_q_points; q_point++)
        {
          ...
          cauchy_stress_fluid = J * (-pI + _density_fluid * _viscosity * (grad_v * F_Inverse
  ...
  private:
    double _density_fluid, _viscosity, _lame_coefficient_mu, _poisson_ratio_nu,
    ...
    double BoundaryValue(const FaceDataContainer<DOFHANDLER,VECTOR,2>& fdc)
    {
      ...
      if (color == 80)
      {
        ...
        for (unsigned int q_point = 0; q_point < n_q_points; q_point++)
        {
          ...
          drag_lift_value -= cauchy_stress_fluid * fdc.GetFEFaceValuesState().normal_vector(q_point)
              * fdc.GetFEFaceValuesState().JxW(q_point);
        }
