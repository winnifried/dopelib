  ...
  public:
                      ...
                      double scale, double)
    {
      ...
      for (unsigned int q_point = 0; q_point < n_q_points; q_point++)
      {
        ...
        pressure_grad[0] = _ugrads[q_point][2][0];
        pressure_grad[1] = _ugrads[q_point][2][1];
        ...
        incompressiblity_fluid = (_ugrads[q_point][0][0] + _ugrads[q_point][4][1]
            * _ugrads[q_point][0][0] - _ugrads[q_point][3][1] * _ugrads[q_point][1][0]
            - _ugrads[q_point][4][0] * _ugrads[q_point][0][1] + _ugrads[q_point][1][1]
            + _ugrads[q_point][3][0] * _ugrads[q_point][1][1]);

        // constitutive stress tensors for fluid
        Tensor<2, 2> cauchy_stress_fluid;
        cauchy_stress_fluid = -pI + density_fluid * viscosity * (v_grad * F_Inverse + F_Inverse_T
            * transpose(v_grad));
        ...
        cauchy_stress_structure_STVK = 1.0 / J * F * (lambda * tr_E * I + 2 * mu * E) * F_T;
        ...
        for (unsigned int i = 0; i < n_dofs_per_cell; i++)
        {
          ...
          if (material_id == 0)
          {
            local_cell_vector(i) += scale
                * (convection_fluid * phi_i_v + scalar_product(J * cauchy_stress_fluid
                    * F_Inverse_T, phi_i_grads_v) + incompressiblity_fluid * phi_i_p + alpha_u
                    * cell_diameter * cell_diameter * scalar_product(u_grad, phi_i_grads_u))
                * state_fe_values.JxW(q_point);
          }
          else if (material_id == 1)
          {
            /*
             local_cell_vector(i) += scale * (scalar_product(J * cauchy_stress_structure_INH * F_Inverse_T, phi_i_grads_v)
             + (J - 1.0) * phi_i_p
             + v * phi_i_u)
             * state_fe_values.JxW(q_point);
             */
            local_cell_vector(i) += scale * (scalar_product(J * cauchy_stress_structure_STVK
                * F_Inverse_T, phi_i_grads_v) + alpha_p * cell_diameter * cell_diameter
                * pressure_grad * phi_i_grads_p // harmonic pressure
                //+ _uvalues[q_point](2) * phi_i_p  // for STVK
    ...
    void CellMatrix(const CDC<DH,VECTOR,2>& cdc, dealii::FullMatrix<double> &local_entry_matrix, double, double)
    {
      ...
      for (unsigned int q_point = 0; q_point < n_q_points; q_point++)
      {
        ...
        for (unsigned int j = 0; j < n_dofs_per_cell; j++)
        {
          ...
          const Tensor<1, 2> phi_j_grads_p = state_fe_values[pressure].gradient(j, q_point); // only for STVK
//          const Tensor<1, 2> phi_j_u = state_fe_values[displacements].value(j, q_point);
          ...
          double J_LinU;
          J_LinU = (phi_j_grads_u[0][0] * (1 + _ugrads[q_point][4][1]) + (1
              + _ugrads[q_point][3][0]) * phi_j_grads_u[1][1] - phi_j_grads_u[0][1]
              * _ugrads[q_point][4][0] - _ugrads[q_point][3][1] * phi_j_grads_u[1][0]);

          Tensor<2, 2> F_Inverse_LinU;
          F_Inverse_LinU = (-1.0 / std::pow(J, 2) * J_LinU * F_tilde + 1.0 / J * F_tilde_LinU);
          ...
          double tr_E_LinU = ((1 + _ugrads[q_point][3][0]) * phi_j_grads_u[0][0]
              + _ugrads[q_point][3][1] * phi_j_grads_u[0][1] + (1 + _ugrads[q_point][4][1])
              * phi_j_grads_u[1][1] + _ugrads[q_point][4][0] * phi_j_grads_u[1][0]);
          ...
          piola_kirchhoff_stress_1st_term_LinALL = -pI_LinP * J * F_Inverse_T - pI
              * J_F_Inverse_T_LinU;
          ...
          piola_kirchhoff_stress_structure_INH_2nd_term_LinALL = mu * (J_LinU * F + J * F_LinU
              - J_F_Inverse_T_LinU);
          ...
          piola_kirchhoff_stress_structure_STVK_LinALL = lambda * (F_LinU * tr_E * I + F
              * tr_E_LinU * I) + 2 * mu * (F_LinU * E + F * E_LinU);
          ...
          cauchy_stress_fluid_2nd_term_LinALL = phi_j_grads_v * F_Inverse + transpose(F_Inverse)
              * transpose(phi_j_grads_v) + v_grad * F_Inverse_LinU + transpose(F_Inverse_LinU)
              * transpose(v_grad);
          ...
          piola_kirchhoff_stress_fluid_2nd_term_LinALL = density_fluid * viscosity
              * (cauchy_stress_fluid_2nd_term_LinALL * J * F_Inverse_T + (v_grad * F_Inverse
                  + F_Inverse_T * transpose(v_grad)) * J_F_Inverse_T_LinU);
          ...
          incompressiblity_fluid_LinAll = phi_j_grads_v[0][0] + phi_j_grads_v[1][1]
              + (phi_j_grads_u[1][1] * _ugrads[q_point][0][0] - phi_j_grads_u[0][1]
                  * _ugrads[q_point][1][0] - phi_j_grads_u[1][0] * _ugrads[q_point][0][1]
          ...
          convection_fluid_LinAll = density_fluid * (J_LinU * v_grad * F_Inverse * v + J * v_grad
              * F_Inverse_LinU * v + J * (phi_j_grads_v * F_Inverse * v + v_grad * F_Inverse
              * phi_j_v));
          ...
          for (unsigned int i = 0; i < n_dofs_per_cell; i++)
          {
            ...
            const Tensor<1, 2> phi_i_grads_p = state_fe_values[pressure].gradient(i, q_point); // only for STVK
            ...
            if (material_id == 0)
            {
              local_entry_matrix(i, j) += (convection_fluid_LinAll * phi_i_v
                  + scalar_product(piola_kirchhoff_stress_1st_term_LinALL, phi_i_grads_v)
                  + scalar_product(piola_kirchhoff_stress_fluid_2nd_term_LinALL, phi_i_grads_v)
                  + incompressiblity_fluid_LinAll * phi_i_p + alpha_u * cell_diameter
                  * cell_diameter * scalar_product(phi_j_grads_u, phi_i_grads_u))
                  * state_fe_values.JxW(q_point);
            ...
            else if (material_id == 1)
            {
              ...
              local_entry_matrix(i, j)
                  += (scalar_product(piola_kirchhoff_stress_structure_STVK_LinALL, phi_i_grads_v)
                      + alpha_p * cell_diameter * cell_diameter * phi_j_grads_p * phi_i_grads_p // harmonic pressure
                      //+ phi_j_p * phi_i_p            // pressure zero
                          ...
                          double scale, double /*scale_ico*/)
    {
      ...
      if (color == 1)
      {
        ...
        for (unsigned int q_point = 0; q_point < n_q_points; q_point++)
        {
          ...
          const Tensor<2, 2> do_nothing_ALE = density_fluid * viscosity * transpose(invert(F))
              * transpose(v_grad);

          const Tensor<1, 2> neumann_value = (J * do_nothing_ALE * transpose(invert(F))
      ...
      if (color == 1)
      {
        ...
        for (unsigned int q_point = 0; q_point < n_q_points; q_point++)
        {
          ...
          for (unsigned int i = 0; i < n_dofs_per_cell; i++)
          {
            ...
            for (unsigned int j = 0; j < n_dofs_per_cell; j++)
            {
              const Tensor<2, 2> phi_j_grads_v = state_fe_face_values[velocities].gradient(j,
                                                                                           q_point);
              ...
              double J_LinU;
              J_LinU = (phi_j_grads_u[0][0] * (1 + _uboundarygrads[q_point][4][1]) + (1
                  + _uboundarygrads[q_point][3][0]) * phi_j_grads_u[1][1] - phi_j_grads_u[0][1]
                  * _uboundarygrads[q_point][4][0] - _uboundarygrads[q_point][3][1]
                  * phi_j_grads_u[1][0]);
              ...
              do_nothing_LinAll = density_fluid * viscosity * (J_F_Inverse_T_LinU
                  * transpose(v_grad) * transpose(F_Inverse) + J * transpose(F_Inverse)
                  * transpose(phi_j_grads_v) * transpose(F_Inverse) + J * transpose(F_Inverse)
                  * transpose(v_grad) * transpose(F_Inverse_LinU));
  ...
  private:
   vector<Vector<double> > _uvalues;
    ...
    // material variables
    double density_fluid, viscosity, alpha_u, alpha_p, mu, poisson_ratio_nu, lambda;
