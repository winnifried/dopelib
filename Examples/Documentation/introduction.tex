...
Copyright (C) 2012--2018 by the DOpElib authors\\[2mm]
%
...
\begin{itemize}
  \item Seshadri Basava (Gradient robust methods for vector valued problems)
  ...
  \item Christine Herter (Eigenvalue optimization)
  ...
  \item Katrin Mang (Rothe method for non-stationary problems,
    phase-field damage examples)
  \item Nicolai Simon (Example on coefficient control for VI)
  \item Mirjam Walloth (Error estimation in non-stationary problems)
  \end{itemize}
...
\section{Overview}
The main library can be roughly split into three parts:
\begin{verbatim}
DOpEsrc, Examples, ThirdPartyLibs
\end{verbatim}
Here, \texttt{ThirdPartyLibs} was already previously discussed.
The \texttt{Examples} will be discussed in the chapters \ref{chap:howtoex}
to \ref{sec_terminal_output}. In this chapter, we concentrate on 
\texttt{DOpEsrc}. Therein, the library implementation is divided into the following 
subfolders:
\begin{verbatim}
container       reducedproblems    wrapper
network         source             templates
basic           include            opt_algorithms  
interfaces      problemdata        tsschemes
\end{verbatim}
