...
template<template<int, int> class DH,typename VECTOR,int dopedim, int dealdim>
	...
	for (unsigned int q_point=0;q_point<n_q_points;q_point++)
	  { 
	      Tensor<2,2> pI;
	       ...
	       cauchy_stress_fluid = 1.0 * J * (-pI + _density_fluid * _viscosity *
					  (grad_v * F_Inverse + F_Inverse_T * transpose(grad_v))) * F_Inverse_T;

	      
	       Tensor<1,2> stress_normal;
	       stress_normal = cauchy_stress_fluid * state_fe_face_values.normal_vector(q_point);
	 ...
	 for (unsigned int q_point=0;q_point<n_q_points;q_point++)
	   {
	     functional_value_J += _mu_regularization * 0.5 * 
	       ((_qvalues(0) - _control_constant) * (_qvalues(0) - _control_constant))
	       * state_fe_face_values.JxW(q_point);
	   }
  ...
  void BoundaryValue_U(const FaceDataContainer<DH, VECTOR, dealdim>& fdc,
	...
	for (unsigned int q_point=0;q_point<n_q_points;q_point++)
	  { 
		...
		{	
	     ...
	     const Tensor<2,dealdim> pI = ALE_Transformations::get_pI<dealdim> (q_point, _ufacevalues);
	     const Tensor<2,dealdim> grad_v = ALE_Transformations::get_grad_v<dealdim> (q_point, _ufacegrads);	
	      
	      const Tensor<2,dealdim> grad_v_T = ALE_Transformations::get_grad_v_T<dealdim> (grad_v);		
	      const Tensor<2,dealdim> F = ALE_Transformations::get_F<dealdim> (q_point, _ufacegrads);
		...
		const double J_LinU =  ALE_Transformations
		  ::get_J_LinU<dealdim> (q_point, _ufacegrads, phi_grads_u[j]);
		...
		const Tensor<2,dealdim> F_Inverse_LinU = ALE_Transformations
		  ::get_F_Inverse_LinU (phi_grads_u[j],J, J_LinU,q_point,_ufacegrads);
		 ...
		 Tensor<1,2> neumann_value = 
		   (stress_fluid_ALE_1st_term_LinAll + stress_fluid_ALE_2nd_term_LinAll)
		   * state_fe_face_values.normal_vector(q_point);
			  
			  local_cell_vector(j) += scale* 		
			    neumann_value[0] * stress_normal[0] 
			    * state_fe_face_values.JxW(q_point);
  ...
  void BoundaryValue_Q(const FaceDataContainer<DH, VECTOR, dealdim>& fdc,
		       dealii::Vector<double> &local_cell_vector, double scale)
 {
   const auto & state_fe_face_values = fdc.GetFEFaceValuesState();
...
void BoundaryValue_QQ(const FaceDataContainer<DH, VECTOR, dealdim>& fdc,
		      dealii::Vector<double> &local_cell_vector, double scale)
 {
    const auto & state_fe_face_values = fdc.GetFEFaceValuesState();
   unsigned int n_dofs_per_cell = local_cell_vector.size();
...
void FaceValue_U(const FaceDataContainer<DH, VECTOR, dealdim>& fdc,
		       dealii::Vector<double> &local_cell_vector, double scale)
{
  const auto & state_fe_face_values = fdc.GetFEFaceValuesState();
  unsigned int n_dofs_per_cell = fdc.GetNDoFsPerCell();
...
void FaceValue_UU(const FaceDataContainer<DH, VECTOR, dealdim>& /*fdc*/,
		  dealii::Vector<double> &/*local_cell_vector*/, double /*scale*/)
{
...
void FaceValue_Q(const FaceDataContainer<DH, VECTOR, dealdim>&,
...
void FaceValue_QU(const FaceDataContainer<DH, VECTOR, dealdim>&,
		      dealii::Vector<double> &, double)
{

}

void FaceValue_UQ(const FaceDataContainer<DH, VECTOR, dealdim>&,
		      dealii::Vector<double> &, double)
{
...
void FaceValue_QQ(const FaceDataContainer<DH, VECTOR, dealdim>&,
		      dealii::Vector<double> &, double)
{
	      ...
	      dealii::Vector<double> &/*local_cell_vector*/, double /*scale*/)
{
  ...
  inline void GetFaceValues(const DOpEWrapper::FEFaceValues<dealdim>& fe_face_values,  
    ...
    inline void GetValues(const DOpEWrapper::FEValues<dealdim>& fe_values,  const map<string, const BlockVector<double>* >& domain_values,string name, vector<Vector<double> >& values)
    {
      typename map<string, const BlockVector<double>* >::const_iterator it = domain_values.find(name);
      if(it == domain_values.end())
	{
