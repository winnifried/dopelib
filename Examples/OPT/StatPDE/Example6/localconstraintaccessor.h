...
namespace DOpE
{
  ...
  public:
    ...
    inline void CopyLocalConstraintToTensor(const dealii::BlockVector<double>& constraint, Tensor<2,1>& tensor, unsigned int index) const
    {
      tensor[0][0] = constraint.block(0)(index);
    }
    inline void CopyTensorToLocalConstraint(const Tensor<2,1>& tensor, dealii::BlockVector<double>& constraint, unsigned int index) const
    {
      constraint.block(0)(index) = tensor[0][0];
    }

    inline void CopyLocalControlToVector(const dealii::BlockVector<double>& control, Vector<double>& vec, unsigned int index) const
    {
    ...
    inline void CopyLocalControlToTensor(const dealii::BlockVector<double>& control, Tensor<2,1>& tensor, unsigned int index) const
    {
      if(index >= control.block(0).size())
    ...
    inline void CopyTensorToLocalControl(const Tensor<2,1>& tensor, dealii::BlockVector<double>& control, unsigned int index) const
    {
      if(index >= control.block(0).size())
    ...
    inline void AddTensorToLocalControl(const Tensor<2,1>& tensor, dealii::BlockVector<double>& control, unsigned int index) const
    {
      if(index >= control.block(0).size())
	control.block(0)(index-control.block(0).size()) += tensor[0][0];
      else
    ...
    inline void AddVectorToLocalControl(const Vector<double>& vec, dealii::BlockVector<double>& control, unsigned int index) const
    {
      assert(vec.size() == 1);
      if(index < control.block(0).size())
      {
...
//    }
    ...
    inline unsigned int NLocalDirections() const
    {
      return 1;
    }

    inline void LocalControlToConstraintBlocks(const dealii::BlockVector<double>* q,std::vector<std::vector<unsigned int> >& constraint)
    {
    ...
    inline void GetLocalConstraintDerivative(Tensor<2,1>& derivative, const dealii::BlockVector<double>& constraints, unsigned int local_block, unsigned int local_index) const
    {
    ...
    inline void GetLocalConstraintDerivative(Tensor<2,1>& derivative, const dealii::BlockVector<double>& constraints, unsigned int local_block) const
    {
      if(local_block < constraints.block(0).size() /2)
      {
	// It is a lower bound
