      ...
      public:
        ...
        IsFeasible(const ConstraintVector<VECTOR>& g) const
        {
          for (unsigned int block = 0;
              block < g.GetSpacialVector("local").n_blocks(); block++)
          {
            for (unsigned int i = 0;
                i < g.GetSpacialVector("local").block(block).size(); i++)
            {
            ...
            ControlVector<VECTOR>& g, double lambda) const
        {
          ...
          for (unsigned int block = 0; block < g.GetSpacialVector().n_blocks();
              block++)
          {
            for (unsigned int i = 0;
                i < g.GetSpacialVector().block(block).size(); i++)
            {
              g.GetSpacialVector().block(block)(i) = (1. - lambda)
                  * g.GetSpacialVector().block(block)(i);
              g.GetSpacialVector().block(block)(i) += lambda
                  * g_hat.GetSpacialVector().block(block)(i);
            }
          }
        }
            ...
            const ConstraintVector<VECTOR>& g) const
        {
              ...
              block < g.GetSpacialVector("local").n_blocks(); block++)
          {
            ...
            {
              ret += fabs(
                  g.GetSpacialVector("local").block(block)(i)
                      * f.GetSpacialVector("local").block(block)(i));
            }
          }
          for (unsigned int i = 0; i < g.GetGlobalConstraints().size(); i++)
          {
            ret += fabs(
                g.GetGlobalConstraints()(i) * f.GetGlobalConstraints()(i));
          }
          return ret;
        }
