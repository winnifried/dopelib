          ...
          double /*scale_ico*/)
      {
        ...
        for (unsigned int q_point = 0; q_point < n_q_points; q_point++)
        {
          ...
          for (unsigned int i = 0; i < n_dofs_per_cell; i++)
          {
            ...
            local_cell_vector(i) += scale * pow(_qvalues[q_point](0), _p) * 0.25
          ...
          double /*scale_ico*/)
      {
        ...
        for (unsigned int q_point = 0; q_point < n_q_points; q_point++)
        {
          ...
          if (_p == 1.)
            ...
            for (unsigned int i = 0; i < n_dofs_per_cell; i++)
            {
              local_cell_vector(i) += scale * _p
                  * pow(_qvalues[q_point](0), _p - 1.)
                  * control_fe_values.shape_value(i, q_point) * 0.25
                  * scalar_product(u_grad + transpose(u_grad),
                      z_grad + transpose(z_grad))
                  * control_fe_values.JxW(q_point);
        ...
        for (unsigned int q_point = 0; q_point < n_q_points; q_point++)
        {
          for (unsigned int i = 0; i < n_dofs_per_cell; i++)
          {
            ...
            for (unsigned int j = 0; j < n_dofs_per_cell; j++)
            {
              ...
              local_entry_matrix(i, j) += pow(_qvalues[q_point](0), _p) * 0.25
