...
typedef OptProblem<FunctionalInterface<Multimesh_CellDataContainer,Multimesh_FaceDataContainer,DOFHANDLER, VECTOR, 2,2>,
...
int main(int argc, char **argv)
{
  ...
  //DOpEWrapper::FiniteElement<2>          control_fe(FE_DGP<2>(0),1);
  DOpEWrapper::FiniteElement<2>          control_fe(FE_Q<2>(1),1);
  DOpEWrapper::FiniteElement<2>          state_fe(FE_Q<2>(1),1);
...
//  std::vector<double> times(1,0.);
  ...
  MethodOfLines_MultiMesh_SpaceTimeHandler<FE, DOFHANDLER, BlockSparsityPattern,VECTOR, SparsityMaker<DOFHANDLER,2>, ConstraintsMaker<DOFHANDLER,2>, 2> DOFH(triangulation,control_fe, state_fe);
