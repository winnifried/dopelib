...
namespace DOpE
{
  ...
  public:
    ...
    InitialProblem<
    CrankNicolsonProblem<OPTPROBLEM, SPARSITYPATTERN, VECTOR, dealdim,
    ...
    /******************************************************/
    CrankNicolsonProblem<OPTPROBLEM, SPARSITYPATTERN, VECTOR, dealdim, FE,
    DH> &
    GetBaseProblem ()
    {
      return *this;
      ...
      if (this->GetPart () == "New")
        {
          ...
          this->GetProblem ().ElementEquation (edc, tmp, 0.5 * scale,
                                               scale);
          local_vector += tmp;
