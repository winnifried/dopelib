  ...
  public:
                     ...
                     double /*scale_ico*/)
    {
      if (this->GetPart () == "New")
        {
          damped_cn_theta =
            0.5 + this->GetProblem ().GetBaseProblem ().GetSpaceTimeHandler ()->GetStepSize ();
          ...
          this->GetProblem ().ElementEquation (dc, tmp,
                                               damped_cn_theta * scale, scale);
          local_vector += tmp;

          tmp = 0.0;
          this->GetProblem ().ElementTimeEquation (dc, tmp, scale);
          local_vector += tmp;

          this->GetProblem ().ElementTimeEquationExplicit (dc,
                                                           local_vector, scale);
        }
      ...
      if (this->GetPart () == "New")
        {
          ...
          this->GetProblem ().PointRhs (param_values, domain_values,
                                        rhs_vector, damped_cn_theta * scale);
        }
      else if (this->GetPart () == "Old")
        {
          damped_cn_theta =
            0.5 + this->GetProblem ().GetBaseProblem ().GetSpaceTimeHandler ()->GetStepSize ();
          this->GetProblem ().PointRhs (param_values, domain_values,
                                        rhs_vector, (1 - damped_cn_theta) * scale);
        }
      else
        {
          abort ();
