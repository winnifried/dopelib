     ...
     /******************************************************/
          ...
          if (_part == "New")
            {
              ...
              _OP.CellEquation(dc, tmp, 0.5 * scale, scale);
          ...
          else if (_part == "Old")
            {
              ...
              // Multiplication by 1/2 due to CN discretization
              _OP.CellEquation(dc, tmp, 0.5 * scale, 0.);
              local_cell_vector += tmp;
      ...
      template<typename DATACONTAINER>
            ...
            dealii::Vector<double> &local_cell_vector, double scale = 1.)
        {
          if (_part == "New")
            {
              _OP.CellRhs(dc, local_cell_vector, 0.5 * scale);
            }
          else if (_part == "Old")
            {
              _OP.CellRhs(dc, local_cell_vector, 0.5 * scale);
      ...
      template<typename DATACONTAINER>
            ...
            dealii::FullMatrix<double> &local_entry_matrix)
        {
          ...
          _OP.CellMatrix(dc, local_entry_matrix, 0.5, 1.);
          ...
          if (_part == "New")
            {
              ...
              _OP.FaceEquation(fdc, local_cell_vector, 0.5 * scale);
            }
          else if (_part == "Old")
            {
              _OP.FaceEquation(fdc, local_cell_vector, 0.5 * scale);
      ...
      template<typename FACEDATACONTAINER>
            ...
            dealii::Vector<double> &local_cell_vector, double scale = 1.)
        {
          if (_part == "New")
            {
              // Hier nicht mit _OP.GetBaseProblem().GetSpaceTimeHandler()->GetStepSize() multiplizieren, da local_cell_matrix schon skaliert ist
              _OP.BoundaryEquation(fdc, local_cell_vector, 0.5 * scale);
            }
          else if (_part == "Old")
            {
              _OP.BoundaryEquation(fdc, local_cell_vector, 0.5 * scale);
