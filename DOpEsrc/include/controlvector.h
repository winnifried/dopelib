...
//    void SetTime(double t, const TimeIterator& interval) const;
  ...
  void
  ControlVector<VECTOR>::ReSizeSpace (const unsigned int time_point)
  {
    if (GetBehavior () == DOpEtypes::VectorStorageType::fullmem)
        ...
        if (accessor_ >= 0)
            ...
            if (control_[accessor_]->size () != ndofs)
              {
                ...
                GetSpaceTimeHandler ()->ReinitVector (*control_[accessor_],
                                                      DOpEtypes::VectorType::control);
                if (existed)
                  GetSpaceTimeHandler ()->SpatialMeshTransferControl (
                    local_control_, *(control_[accessor_]));
              }
          }
        else
          {
            ...
            if (local_control_.size () != ndofs)
              GetSpaceTimeHandler ()->ReinitVector (local_control_,
                                                    DOpEtypes::VectorType::control);
          }
      }
    else
      {
        throw DOpEException (
          ...
          "ControlVector<VECTOR>::ReSizeSpace");
      }
  }
                ...
                if (control_[accessor_]->n_blocks () != nblocks)
                ...
                else
                  {
                    for (unsigned int i = 0; i < nblocks; i++)
                      if (control_[accessor_]->block (i).size () != dofs_per_block[i])
                        reinit = true;
                  }
              }
