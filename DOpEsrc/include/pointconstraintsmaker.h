...
namespace DOpE
{
  ...
  class PointConstraints : public UserDefinedDoFConstraints<DH, dopedim,
    dealdim>
  {
  public:
    PointConstraints (const std::vector<dealii::Point<dealdim> > &c_points,
                      const std::vector<std::vector<bool> > &c_comps)
      : UserDefinedDoFConstraints<DH, dopedim, dealdim> (),
        c_points_ (c_points),
        c_comps_ (c_comps)
    {
      if (c_points_.size () != c_comps_.size ())
        throw DOpEException ("Number of Entries not matching!",
  ...
  void
  PointConstraints<DH, dopedim, dealdim>::MakeStateDoFConstraints (const DOpEWrapper::DoFHandler<
      dealdim,
      DH> &dof_handler,
      dealii::ConstraintMatrix &constraint_matrix) const
  {
    std::vector<dealii::Point<dealdim> > support_points (
      dof_handler.n_dofs ());
    STHInternals::MapDoFsToSupportPoints (this->GetMapping (), dof_handler,
                                          support_points);
    for (unsigned int i = 0; i < c_points_.size (); i++)
      {
        ...
        for (unsigned int p = 0; p < support_points.size (); p++)
          {
            if (c_points_[i].distance (support_points[p]) < sqrt (
                  support_points[p].square () + c_points_[i].square ())
                * std::numeric_limits<
                double>::epsilon ())
              {
                found = true;
        ...
        if (!found)
          {
            throw DOpEException ("Points not found!",
