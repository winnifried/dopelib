...
namespace DOpE
{
  ...
  public:
    ...
    Multimesh_FaceDataContainer (const Quadrature<dim - 1>& quad,
                                 UpdateFlags update_flags,
                                 SpaceTimeHandler<FE, dealii::DoFHandler,
                                 SPARSITYPATTERN, VECTOR, dopedim,
                                 dealdim> &sth,
                            ...
                            update_flags)
    {
      ...
      control_prolongation_ = IdentityMatrix (
                                element_[this->GetControlIndex ()]->get_fe ().dofs_per_cell);
      state_prolongation_ = IdentityMatrix (
                              element_[this->GetStateIndex ()]->get_fe ().dofs_per_cell);
    }
     ...
     */
    inline void
    ReInit (unsigned int coarse_index,
  ...
  private:
    ...
    const typename std::vector<
    typename dealii::DoFHandler<dim>::cell_iterator> &element_;
    const typename std::vector<
    typename dealii::Triangulation<dim>::cell_iterator> &tria_element_;
  ...
  template <typename VECTOR, int dim>
  void
  Multimesh_FaceDataContainer<dealii::DoFHandler, VECTOR, dim>::ReInit (unsigned int coarse_index,
      unsigned int fine_index,
      const FullMatrix<
      double> &prolongation_matrix,
      unsigned int face_no)
  {
    face_ = face_no;
  ...
  template <typename VECTOR, int dim>
  unsigned int
  Multimesh_FaceDataContainer<dealii::DoFHandler, VECTOR, dim>::GetNbrMaterialId (unsigned int /*face*/) const
  {
    throw DOpEException (
      "This function has not been written since we do not know what the right neigbour is!",
  ...
  template <typename VECTOR, int dim>
  void
  Multimesh_FaceDataContainer<dealii::DoFHandler, VECTOR, dim>::GetParamValues (std::string name,
      Vector<
      double> &value) const
  {
  ...
  template <typename VECTOR, int dim>
  void
  Multimesh_FaceDataContainer<dealii::DoFHandler, VECTOR, dim>::GetFaceValuesState (std::string name,
      std::vector<
      double> &values) const
  {
    this->GetValues (element_[this->GetStateIndex ()], state_prolongation_,
                     this->GetFEFaceValuesState (), name, values);
  }
  /*********************************************/
  template <typename VECTOR, int dim>
  void
  ...
  template <typename VECTOR, int dim>
  void
  Multimesh_FaceDataContainer<dealii::DoFHandler, VECTOR, dim>::GetFaceValuesControl (std::string name,
      std::vector<
      double> &values) const
  {
    this->GetValues (element_[this->GetControlIndex ()],
                     control_prolongation_, this->GetFEFaceValuesControl (), name, values);
  }
  ...
  template <int targetdim>
  void
  Multimesh_FaceDataContainer<dealii::DoFHandler, VECTOR, dim>::GetFaceGradsState (std::string name,
      std::vector<
      Tensor<
      1,
      targetdim> > &values) const
  {
    this->GetGrads<targetdim> (element_[this->GetStateIndex ()],
                               state_prolongation_, this->GetFEFaceValuesState (), name, values);
  }
  ...
  template <int targetdim>
  void
  Multimesh_FaceDataContainer<dealii::DoFHandler, VECTOR, dim>::GetFaceGradsControl (std::string name,
      std::vector<
      Tensor<
      1,
      targetdim> > &values) const
  {
    this->GetGrads<targetdim> (element_[this->GetControlIndex ()],
                               control_prolongation_, this->GetFEFaceValuesControl (), name,
                               values);
  }
  /***********************************************************************/
  ...
  template <typename VECTOR, int dim>
  void
  Multimesh_FaceDataContainer<dealii::DoFHandler, VECTOR, dim>::GetValues (typename dealii::DoFHandler<
      dim>::cell_iterator element,
      const FullMatrix<
      double> &prolongation,
      const DOpEWrapper::FEFaceValues<
      dim> &fe_values,
      std::string name,
      std::vector<
      double> &values) const
  {
    ...
    for (unsigned int shape_func = 0; shape_func < dofs_per_element;
         ++shape_func)
      {
        ...
        const double *shape_value_ptr = &(fe_values.shape_value (shape_func,
                                                                 0));
        ...
        if (element->get_fe ().is_primitive (shape_func))
          {
            const unsigned int comp =
              element->get_fe ().system_to_component_index (shape_func).first;
            for (unsigned int point = 0; point < n_quadrature_points; ++point)
              values[point] (comp) += value
          ...
          for (unsigned int c = 0; c < n_components; ++c)
            {
              ...
              for (unsigned int point = 0; point < n_quadrature_points;
                   ++point)
                values[point] (c) += value
                                     * fe_values.shape_value_component (shape_func, point, c);
            }
      }
  }
  ...
  template <int targetdim>
  void
  Multimesh_FaceDataContainer<dealii::DoFHandler, VECTOR, dim>::GetGrads (typename dealii::DoFHandler<
      dim>::cell_iterator element,
      const FullMatrix<
      double> &prolongation,
      const DOpEWrapper::FEFaceValues<
          ...
          for (unsigned int c = 0; c < n_components; ++c)
            {
              ...
              for (unsigned int point = 0; point < n_quadrature_points;
                   ++point)
                values[point][c] += value
                                    * fe_values.shape_grad_component (shape_func, point, c);
            }
      }
  }
