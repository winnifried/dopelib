...
namespace DOpE
{
  ...
  class HigherOrderDWRContainerControl : public DWRDataContainer<STH, IDC,
    EDC, FDC, VECTOR>
  {
  public:
    /**
     ...
     */
    HigherOrderDWRContainerControl (STH &higher_order_sth,
                                    ...
                                    DOpEtypes::ResidualEvaluation res_eval =
                                      DOpEtypes::strong_residual)
      : DWRDataContainer<STH, IDC, EDC, FDC, VECTOR> (ee_terms),
        sth_higher_order_ (higher_order_sth),
        idc_higher_order_ (higher_order_idc),
        res_eval_ (res_eval),
        PI_h_u_ (NULL),
        PI_h_z_ (NULL),
        PI_h_q_ (NULL)
...
//          return DOpEtypes::strong_residual;
  ...
  template <class STH, class IDC, class EDC, class FDC, typename VECTOR>
  void
  HigherOrderDWRContainerControl<STH, IDC, EDC, FDC, VECTOR>::ReInit (unsigned int n_elements)
  {
    DWRDataContainer<STH, IDC, EDC, FDC, VECTOR>::ReInit (n_elements);

    GetHigherOrderSTH ().ReInit (control_n_blocks_, *control_block_component_,
                                 DirichletDescriptor (*control_dirichlet_colors_,
                                                      *control_dirichlet_comps_), state_n_blocks_,
                                 *state_block_component_,
                                 DirichletDescriptor (*state_dirichlet_colors_,
                                                      *state_dirichlet_comps_));
    if (this->GetEETerms () == DOpEtypes::primal_only || this->GetEETerms ()
        == DOpEtypes::mixed
        || this->GetEETerms () == DOpEtypes::mixed_control)
      {
