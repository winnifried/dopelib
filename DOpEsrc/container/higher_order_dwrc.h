...
namespace DOpE
{
  ...
  public:
     ...
     */
    HigherOrderDWRContainer (STH &higher_order_sth,
...
//          return DOpEtypes::strong_residual;
  ...
  template <class STH, class IDC, class EDC, class FDC, typename VECTOR>
  void
  HigherOrderDWRContainer<STH, IDC, EDC, FDC, VECTOR>::ReInit (unsigned int n_elements)
  {
    DWRDataContainer<STH, IDC, EDC, FDC, VECTOR>::ReInit (n_elements);

    GetHigherOrderSTH ().ReInit (state_n_blocks_, *state_block_component_,
