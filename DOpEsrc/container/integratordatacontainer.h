...
namespace DOpE
{
   ...
   */
  template <template <int, int> class DH, typename QUADRATURE,
            typename FACEQUADRATURE, typename VECTOR, int dim>
  ...
  public:
    ...
    void
    InitializeFDC (const FACEQUADRATURE &fquad,
    ...
    {
      delete fdc_;
      fdc_ = new FaceDataContainer<DH, VECTOR, dim> (fquad, update_flags,
                                                     sth, element, param_values, domain_values, need_interfaces);
    }
    ...
    template <typename STH>
    void
    InitializeFDC (UpdateFlags update_flags,
                   STH &sth,
                   const std::vector<
                   typename DOpEWrapper::DoFHandler<dim, DH>::active_cell_iterator>& element,
                   const std::map<std::string, const Vector<double>*> &param_values,
                   ...
                   bool need_interfaces = false)
    {
      InitializeFDC (GetFaceQuad (), update_flags, sth, element,
                     param_values, domain_values, need_interfaces);
    }
    ...
    template <typename STH>
    void
    InitializeEDC (const QUADRATURE &quad,
                   UpdateFlags update_flags,
                   STH &sth,
                   const std::vector<
                   typename DOpEWrapper::DoFHandler<dim, DH>::active_cell_iterator>& element,
    ...
    template <typename STH>
    void
    InitializeEDC (UpdateFlags update_flags,
                   STH &sth,
                   const std::vector<
                   typename DOpEWrapper::DoFHandler<dim, DH>::active_cell_iterator>& element,
                   const std::map<std::string, const Vector<double>*> &param_values,
    ...
    template <typename STH>
    void
    InitializeMMFDC (UpdateFlags update_flags,
                     STH &sth,
                     ...
                     bool need_interfaces = false)
    {
      ...
      mm_fdc_ = new Multimesh_FaceDataContainer<DH, VECTOR, dim> (
        GetFaceQuad (), update_flags, sth, element, tria_element,
        param_values, domain_values, need_interfaces);
    }
    ...
    template <typename STH>
    void
    InitializeMMEDC (UpdateFlags update_flags,
                     STH &sth,
                     const typename std::vector<
                     typename DH<dim, dim>::cell_iterator> &element,
                     const typename std::vector<
                     ...
                     const std::map<std::string, const VECTOR *> &domain_values)
    {
      ...
      mm_edc_ = new Multimesh_ElementDataContainer<DH, VECTOR, dim> (
        GetQuad (), update_flags, sth, element, tria_element,
