...
namespace DOpE
{
  ...
  class ReducedNewtonAlgorithmWithInverse : public ReducedNewtonAlgorithm<
    PROBLEM, VECTOR>
  {
  public:
    /**
  ...
  template <typename PROBLEM, typename VECTOR>
  void
  ReducedNewtonAlgorithmWithInverse<PROBLEM, VECTOR>::declare_params (ParameterReader &param_reader)
  {
  ...
  template <typename PROBLEM, typename VECTOR>
  ReducedNewtonAlgorithmWithInverse<PROBLEM, VECTOR>::ReducedNewtonAlgorithmWithInverse (PROBLEM *OP,
      ...
      VECTOR> *Output,
      int base_priority)
    : ReducedNewtonAlgorithm<PROBLEM, VECTOR> (OP, S, param_reader, Except,
  ...
  template <typename PROBLEM, typename VECTOR>
  int
  ReducedNewtonAlgorithmWithInverse<PROBLEM, VECTOR>::Solve (ControlVector<
                                                             VECTOR> &q,
                                                             double global_tol)
  {
  ...
  template <typename PROBLEM, typename VECTOR>
  int
  ReducedNewtonAlgorithmWithInverse<PROBLEM, VECTOR>::SolveReducedLinearSystem (const ControlVector<
      VECTOR> &q,
      const ControlVector<
      VECTOR> &gradient,
      const ControlVector<
  ...
  {
    int iter = 0;
    this->GetReducedProblem ()->ComputeReducedHessianInverseVector (q,
        gradient, dq);
//  {
//    //Check...
