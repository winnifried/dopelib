...
namespace DOpE
{
  ...
  protected:
     ...
     */
    virtual int
    ReducedNewtonLineSearch (const ControlVector<VECTOR> &dq,
                             const ControlVector<VECTOR> &gradient,
                             double &cost,
  ...
  private:
    ...
    double nonlinear_tol_, nonlinear_global_tol_, linear_tol_,
           linear_global_tol_, lineasearch_rho_, linesearch_c_;
  ...
  template <typename PROBLEM, typename VECTOR>
  void
  ReducedNewtonAlgorithm<PROBLEM, VECTOR>::declare_params (ParameterReader &param_reader)
  {
    ...
    param_reader.declare_entry ("linesearch_rho", "0.9",
                                Patterns::Double (0));
    param_reader.declare_entry ("linesearch_c", "0.1", Patterns::Double (0));

    param_reader.declare_entry ("compute_functionals_in_every_step", "false",
                                Patterns::Bool ());
                                         ...
                                         Output, base_priority)
  {
    ...
    compute_functionals_in_every_step_ = param_reader.get_bool (
  ...
  template <typename PROBLEM, typename VECTOR>
  double
  ReducedNewtonAlgorithm<PROBLEM, VECTOR>::NewtonResidual (const ControlVector<
                                                           VECTOR> &q)
  {
    //Solve j'(q) = 0
  ...
  template <typename PROBLEM, typename VECTOR>
  int
  ReducedNewtonAlgorithm<PROBLEM, VECTOR>::Solve (ControlVector<VECTOR> &q,
                                                  double global_tol)
  {
    ...
    while (((res >= global_tol * global_tol)
            && (res >= nonlinear_tol_ * nonlinear_tol_ * firstres))
           || iter < miniter)
      {
        iter++;
        this->GetOutputHandler ()->SetIterationNumber (iter,
        ...
        out << "\t Newton step: "
            ...
            << "\t LinearIters [" << liniter << "]\t LineSearch {" << lineiter
            << "} ";
        this->GetOutputHandler ()->Write (out, 3 + this->GetBasePriority ());
      }
  ...
  template <typename PROBLEM, typename VECTOR>
  int
  ReducedNewtonAlgorithm<PROBLEM, VECTOR>::SolveReducedLinearSystem (const ControlVector<
      VECTOR> &q,
      const ControlVector<
      VECTOR> &gradient,
      const ControlVector<
      ...
      VECTOR> &dq)
  {
    ...
    while (res >= std::min (0.25, sqrt (firstres)) * firstres
           && res >= linear_global_tol_ * linear_global_tol_)
      {
        ...
        if (res < 0. && (res > -1. * linear_global_tol_ * linear_global_tol_
                         || res > -1. * std::min (0.25, sqrt (firstres)) * firstres))
          {
            ...
            res = fabs (res);
            out << "\t There seem to be cancellation errors accumulating in the Residual,"
                << std::endl;
  ...
  template <typename PROBLEM, typename VECTOR>
  int
  ReducedNewtonAlgorithm<PROBLEM, VECTOR>::ReducedNewtonLineSearch (const ControlVector<
      VECTOR> &dq,
      const ControlVector<
      VECTOR> &gradient,
      double &cost,
      ...
      VECTOR> &q)
  {
    ...
    try
      {
        costnew = this->GetReducedProblem ()->ComputeReducedCostFunctional (
                    q);
      }
    catch (DOpEException &e)
      {
//    this->GetExceptionHandler()->HandleException(e);
        ...
        this->GetOutputHandler ()->Write ("Computing Cost Failed",
                                          4 + this->GetBasePriority ());
      }
    ...
    if (reduction > 0)
      {
        this->GetOutputHandler ()->WriteError (
    ...
    if (line_maxiter_ > 0)
      {
        ...
        if (std::isinf (costnew) || std::isnan (costnew)
            || (costnew >= cost + c * alpha * reduction) || force_linesearch)
          {
            this->GetOutputHandler ()->Write ("\t linesearch ",
                                              4 + this->GetBasePriority ());
            while (std::isinf (costnew) || std::isnan (costnew)
                   || (costnew >= cost + c * alpha * reduction)
                   || force_linesearch)
              {
                iter++;
                ...
                try
                  {
                    costnew =
                      this->GetReducedProblem ()->ComputeReducedCostFunctional (
                        q);
                  }
                catch (DOpEException &e)
                  {
