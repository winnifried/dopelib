...
#ifndef EV__ALGORITHM_H_
#define EV__ALGORITHM_H_
...
{
  /**
   * @class Algorithm for Eigenvalueproblem
   *
  ...
  class EVAlgorithm : public ReducedAlgorithm<PROBLEM, VECTOR>
  {
  public:
    /**
  ...
  template <typename PROBLEM, typename VECTOR>
  void EVAlgorithm<PROBLEM, VECTOR>::declare_params(ParameterReader &param_reader)
  {
    param_reader.SetSubsection("evalgorithm parameters");
  ...
  template <typename PROBLEM, typename VECTOR>
  EVAlgorithm<PROBLEM, VECTOR>::EVAlgorithm(PROBLEM *OP,
      ReducedProblemInterface<PROBLEM, VECTOR> *S,
      ParameterReader &param_reader,
  ...
  template <typename PROBLEM, typename VECTOR>
  EVAlgorithm<PROBLEM, VECTOR>::~EVAlgorithm()
  {
  ...
  template <typename PROBLEM, typename VECTOR>
  double EVAlgorithm<PROBLEM, VECTOR>::NewtonResidual(const ControlVector<VECTOR> &q)
  {
    //Solve j'(q) = 0
  ...
  template <typename PROBLEM, typename VECTOR>
  int EVAlgorithm<PROBLEM, VECTOR>::Solve(ControlVector<VECTOR> &q,double /*global_tol*/)
  {
    ...
    out << "**************************************************\n";
    out << "*        Starting Algorithm for Eigenvalue Optimization --NAME--   *\n";
    ...
    try
      {
    	std::cout << "in AlgorithmEVP: ComputeReducedCostfunctional " << std::endl;
        cost = this->GetReducedProblem()->ComputeReducedCostFunctional(q);

    	std::cout << "Ende AlgorithmEVP: ComputeReducedCostfunctional " << std::endl;
...
//      }
    ...
    try
      {
    	std::cout << "in AlgorithmEVP: ComputeReducedGradient " << std::endl;
       this->GetReducedProblem()->ComputeReducedGradient(q,gradient,gradient_transposed);
       std::cout << "Ende AlgorithmEVP: ComputeReducedGradient" << std::endl;
      }
    catch (DOpEException &e)
      {
        this->GetExceptionHandler()->HandleCriticalException(e,"algorithmForEigenvalueproblem::Solve");
      }
...
//    return iter;
  ...
  template <typename PROBLEM, typename VECTOR>
  int EVAlgorithm<PROBLEM, VECTOR>::SolveReducedLinearSystem(const ControlVector<VECTOR> &q,
      const ControlVector<VECTOR> &gradient,
      const ControlVector<VECTOR> &gradient_transposed,
      ControlVector<VECTOR> &dq)
  {
  ...
  template <typename PROBLEM, typename VECTOR>
  int EVAlgorithm<PROBLEM, VECTOR>::ReducedNewtonLineSearch(const ControlVector<VECTOR> &dq,
      const ControlVector<VECTOR>  &gradient,
      double &cost,
      ControlVector<VECTOR> &q)
  {
