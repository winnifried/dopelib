...
namespace DOpE
{
  ...
  class ReducedTrustregion_NewtonAlgorithm : public ReducedAlgorithm<PROBLEM,
    VECTOR>
  {
  public:
    /**
     ...
     */
    double
    NewtonResidual (const ControlVector<VECTOR> &q);
  protected:
    /**
     ...
     */
    virtual double
    Residual (const ControlVector<VECTOR> &gradient,
              const ControlVector<VECTOR> &gradient_transposed)
    {
  ...
  private:
    ...
    double nonlinear_tol_, nonlinear_global_tol_, tr_delta_max_,
           tr_delta_null_, tr_delta_eta_;
  ...
  template <typename PROBLEM, typename VECTOR>
  void
  ReducedTrustregion_NewtonAlgorithm<PROBLEM, VECTOR>::declare_params (ParameterReader &param_reader)
  {
    param_reader.SetSubsection (
      "reducedtrustregionnewtonalgorithm parameters");
    param_reader.declare_entry ("nonlinear_maxiter", "10",
                                Patterns::Integer (0));
    param_reader.declare_entry ("nonlinear_tol", "1.e-7",
                                Patterns::Double (0));
    param_reader.declare_entry ("nonlinear_global_tol", "1.e-11",
                                Patterns::Double (0));
    param_reader.declare_entry ("tr_delta_max", "1.", Patterns::Double (0));
    param_reader.declare_entry ("tr_delta_null", "0.25",
                                Patterns::Double (0));
    param_reader.declare_entry ("tr_delta_eta", "0.01",
                                Patterns::Double (0, 0.25));

    param_reader.declare_entry ("linear_maxiter", "40",
                                Patterns::Integer (0));
    param_reader.declare_entry ("linear_tol", "1.e-10", Patterns::Double (0));
    param_reader.declare_entry ("linear_global_tol", "1.e-12",
                                Patterns::Double (0));

    param_reader.declare_entry ("tr_method", "dogleg",
                                Patterns::Selection ("dogleg|exact|steinhaug"));
  ...
  template <typename PROBLEM, typename VECTOR>
  ReducedTrustregion_NewtonAlgorithm<PROBLEM, VECTOR>::ReducedTrustregion_NewtonAlgorithm (PROBLEM *OP,
  ...
  template <typename PROBLEM, typename VECTOR>
  double
  ReducedTrustregion_NewtonAlgorithm<PROBLEM, VECTOR>::NewtonResidual (const ControlVector<
      VECTOR> &q)
  {
    //Solve j'(q) = 0
    ...
    try
      {
        this->GetReducedProblem ()->ComputeReducedGradient (q, gradient,
                                                            gradient_transposed);
      }
    catch (DOpEException &e)
      {
  ...
  template <typename PROBLEM, typename VECTOR>
  int
  ReducedTrustregion_NewtonAlgorithm<PROBLEM, VECTOR>::Solve (ControlVector<
                                                              VECTOR> &q,
                                                              double global_tol)
  {
    ...
    q.ReInit ();
    //Solve j'(q) = 0
    ControlVector<VECTOR> p_u (q), p_b (q), gradient (q),
                  gradient_transposed (q), hessian (q), hessian_transposed (q), dq (q);
    ...
    out << "**************************************************";
    this->GetOutputHandler ()->Write (out, 1 + this->GetBasePriority (), 1,
                                      1);

    this->GetOutputHandler ()->SetIterationNumber (iter,
...
//}
    ...
    this->GetOutputHandler ()->Write (gradient, "NewtonResidual" + postindex_,
                                      "control");
    out << "\t Newton step: " << iter << "\t Residual (abs.): " << sqrt (res)
        << "\n";
    out << "\t Newton step: " << iter << "\t Residual (rel.): "
        << std::scientific << sqrt (res) / sqrt (res) << "\n";
    ...
    while (iter == 0
           || iter == 1
           || ((res >= global_tol * global_tol) && (res
                                                    >= nonlinear_tol_ * nonlinear_tol_ * firstres)))
      {
        this->GetOutputHandler ()->SetIterationNumber (iter,
                                                       "OptNewton" + postindex_);
        tr_model = 0.;
        if (iter > nonlinear_maxiter_)
          {
            ...
            out << "*             after " << std::setw (6) << iter
                << "  Iterations           *\n";
            out.precision (4);
            out << "*             with Residual " << std::scientific
                << std::setw (11) << sqrt (res) << "          *\n";
            out << "*             with Cost " << std::scientific
                << std::setw (11) << cost << "          *\n";
            out << "*             n_good: " << n_good << " n_bad: " << n_bad
                << "          *\n";
            out.precision (10);
            out << "**************************************************";
            this->GetOutputHandler ()->Write (out,
                                              1 + this->GetBasePriority (), 1, 1);
            throw DOpEIterationException ("Iteration count exceeded bounds!",
        ...
        double expand = 2.;
        bool good = ComputeTRModelMinimizer (q, gradient, gradient_transposed,
                                             hessian, hessian_transposed, p_u, p_b, dq, tr_delta, cost,
                                             tr_model, expand, liniter);
        ...
        if (tr_model != 0.)
          {
            ...
            if (std::max (fabs (last_cost - cost), fabs (tr_model)) < loc_delta)
              ...
              tr_rho = (last_cost - cost - loc_delta)
                       / (0. - tr_model - loc_delta);
          }
        else
          tr_rho = 0.;
        ...
        out << "TR-Newton Predicted Reduction: " << -tr_model
            << " Actual Reduction: " << last_cost - cost << " rho: " << tr_rho
            << " where TR-Minimizer is " << good << " with lenght: " << norm;
        ...
        try
          {
            cost = this->GetReducedProblem ()->ComputeReducedCostFunctional (
                     q);
          }
        catch (DOpEException &e)
          {
        ...
        out << "\t Residual:  (rel.):"
            << this->GetOutputHandler ()->ZeroTolerance (
              sqrt (res) / sqrt (firstres), 1.0)
            << "\t LinearIters [" << liniter << "]";
        this->GetOutputHandler ()->Write (out, 3 + this->GetBasePriority ());
        ...
        this->GetOutputHandler ()->Write (q, "Control" + postindex_,
                                          "control");
        this->GetOutputHandler ()->Write (gradient,
                                          "NewtonResidual" + postindex_, "control");
      }
    ...
    out << "*             after " << std::setw (6) << iter
        << "  Iterations           *\n";
    out.precision (4);
    out << "*             with rel. Residual "
        << std::scientific
        << std::setw (11)
        << this->GetOutputHandler ()->ZeroTolerance (sqrt (res) / sqrt (firstres),
                                                     1.0)
        << "          *\n";
    out << "*             with Cost " << std::scientific << std::setw (11)
        << cost << "          *\n";
    out << "*             n_good: " << n_good << " n_bad: " << n_bad
        << "          *\n";
    out.precision (10);
  ...
  template <typename PROBLEM, typename VECTOR>
  bool
  ReducedTrustregion_NewtonAlgorithm<PROBLEM, VECTOR>::ComputeTRModelMinimizer (const ControlVector<
      VECTOR> &q,
      const ControlVector<
      VECTOR> &gradient,
      const ControlVector<
      VECTOR> &gradient_transposed,
      ControlVector<
      VECTOR> &hessian,
      ControlVector<
      VECTOR> &hessian_transposed,
      ControlVector<
      VECTOR> &p_u,
      ControlVector<
      VECTOR> &p_b,
    ...
    if ("dogleg" == tr_method_)
      {
        ...
        try
          {
            liniter = SolveReducedLinearSystem (q, gradient,
                                                gradient_transposed, p_b);
          }
        ...
        try
          {
            this->GetReducedProblem ()->ComputeReducedHessianVector (q,
                                                                     gradient, hessian, hessian_transposed);
          }
        catch (DOpEException &e)
          {
            this->GetExceptionHandler ()->HandleCriticalException (e);
        ...
        {
          double scale = (gradient * gradient_transposed)
                         / (gradient * hessian_transposed);
...
//      assert(n_u < n_b);
      ...
      {
        throw DOpEException ("Method not yet implemented: " + tr_method_,
      ...
      {
        throw DOpEException ("Unknown Method: " + tr_method_,
                             "ReducedTrustregion_NewtonAlgorithm::ComputeTRModelMinimizer");
      }
  ...
  template <typename PROBLEM, typename VECTOR>
  int
  ReducedTrustregion_NewtonAlgorithm<PROBLEM, VECTOR>::SolveReducedLinearSystem (const ControlVector<
      VECTOR> &q,
      const ControlVector<
      VECTOR> &gradient,
      const ControlVector<
      ...
      VECTOR> &dq)
  {
    std::stringstream out;
    dq = 0.;
    ControlVector<VECTOR> r (q), r_transposed (q), d (q), Hd (q),
                  Hd_transposed (q);
    ...
    while (res >= std::min (0.25, sqrt (firstres)) * firstres
           && res >= linear_global_tol_ * linear_global_tol_)
      {
        iter++;
        this->GetOutputHandler ()->SetIterationNumber (iter,
                                                       "OptNewtonCg" + postindex_);
        ...
        try
          {
            this->GetReducedProblem ()->ComputeReducedHessianVector (q, d, Hd,
                                                                     Hd_transposed);
          }
        catch (DOpEException &e)
          {
        ...
        if (cgalpha < 0)
          {
            ...
            throw DOpENegativeCurvatureException (
