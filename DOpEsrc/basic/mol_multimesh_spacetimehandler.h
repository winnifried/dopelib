  ...
  template<typename FE, typename DOFHANDLER, typename SPARSITYPATTERN,
      ...
      MethodOfLines_MultiMesh_SpaceTimeHandler(
          ...
          const ActiveFEIndexSetterInterface<dim, dim>& index_setter =
            ...
            SpaceTimeHandler<FE, DOFHANDLER, SPARSITYPATTERN, VECTOR, dim,
                dim> ( index_setter), _state_triangulation(triangulation),
            _control_dof_handler(_control_triangulation),
            _state_dof_handler(_state_triangulation), _control_fe(&control_fe),
	...
	_control_triangulation.copy_triangulation(_state_triangulation);
      ...
      MethodOfLines_MultiMesh_SpaceTimeHandler(
          ...
          const ActiveFEIndexSetterInterface<dim, dim>& index_setter =
              ActiveFEIndexSetterInterface<dim, dim> ()) :
            SpaceTimeHandler<FE, DOFHANDLER, SPARSITYPATTERN, VECTOR, dim,
                dim> (times, index_setter), _state_triangulation(triangulation),
            _control_dof_handler(_control_triangulation),
            _state_dof_handler(_state_triangulation), _control_fe(&control_fe),
            _state_fe(&state_fe), _constraints(), _control_mesh_transfer(NULL),
            _sparse_mkr_dynamic(true), _constr_mkr_dynamic(true)
	...
	_control_triangulation.copy_triangulation(_state_triangulation);
          ...
          const std::vector<unsigned int>& state_block_component)
      {
        ...
        VECTOR, dim, dim>::SetActiveFEIndicesControl(_control_dof_handler);
        ...
        if(dim==dim)//FIXME ?? Isnt this always true?
          ...
          {
            throw DOpEException("Not implemented for dim != dim","MethodOfLines_MultiMesh_SpaceTimeHandler::ReInit");
          }
	...
	//return _constraints.local_n_dofs();
      ...
      const std::vector<Point<dim> >&
        GetMapDoFToSupportPoints()
        {
          ...
          DOpE::STHInternals::MapDoFsToSupportPoints(GetStateDoFHandler(), _support_points);
	...
	RefineControlSpace(ref_type,indicators,topfraction,bottomfraction);
       ...
       */
      void
      RefineStateSpace(std::string ref_type, const Vector<float>* indicators = NULL,
          double topfraction = 0.1, double bottomfraction = 0.0)
      {
        ...
        if ("global" == ref_type)
        ...
        else if ("fixednumber" == ref_type)
          {
            ...
            GridRefinement::refine_and_coarsen_fixed_number(_state_triangulation,
                *indicators, topfraction, bottomfraction);
          }
        else if ("fixedfraction" == ref_type)
          {
            assert(indicators != NULL);
            GridRefinement::refine_and_coarsen_fixed_fraction(_state_triangulation,
                *indicators, topfraction, bottomfraction);
          }
       ...
       */
      void
      RefineControlSpace(std::string ref_type, const Vector<float>* indicators = NULL,
          double topfraction = 0.1, double bottomfraction = 0.0)
      {
        assert(bottomfraction == 0.0);
        ...
        if ("global" == ref_type)
          ...
          {
            assert(indicators != NULL);
            GridRefinement::refine_and_coarsen_fixed_number(_control_triangulation,
                *indicators, topfraction, bottomfraction);
          }
        else if ("fixedfraction" == ref_type)
          {
            assert(indicators != NULL);
            GridRefinement::refine_and_coarsen_fixed_fraction(_control_triangulation,
                *indicators, topfraction, bottomfraction);
          }
        else if ("optimized" == ref_type)
          {
  ...
  template<>
    ...
    DOpE::MethodOfLines_MultiMesh_SpaceTimeHandler<
        ...
        ConstraintsMaker<dealii::DoFHandler<deal_II_dimension>,
            deal_II_dimension> , dope_dimension>::ComputeControlSparsityPattern(
