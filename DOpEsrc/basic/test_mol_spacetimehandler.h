...
#ifndef TEST_MOL_SPACE_TIME_HANDLER_H_
#define TEST_MOL_SPACE_TIME_HANDLER_H_
...
//          std::cout << GetFESystem("state").base_element(0).is_primitive()<< std::endl;

//          std::cout << GetFESystem("state").get_name() << std::endl;
//          std::cout << GetFESystem("state").get_sub_fe(comp_mask).get_name()<< std::endl;
//          std::cout << GetFESystem("state").get_sub_fe(2,1).get_name()<< std::endl;

          //getSubFe funktioniert:	(0,1)--> FE_Q<2>(1)
          //						(1,2)--> FE_Nedelec<2>(0)

          //getsubfe funktioniert nicht (0,0)(0,2)(1,0)(1,1)(1,2)(2,1)

          //TODO iteration über Comp_mask, Abfrage nach Element-Typ
          //Spezifische comp_mask anlegen jeweils für Curl und für den Rest
...
//          			Functions::ZeroFunction<dim>(dim + 1), constraints, psi_mask);
