  ...
  template<typename FE, typename DOFHANDLER, typename SPARSITYPATTERN,
    ...
    public:
      MethodOfLines_StateSpaceTimeHandler(
          ...
          const ActiveFEIndexSetterInterface<dealdim>& index_setter =
            ...
            StateSpaceTimeHandler<FE, DOFHANDLER, SPARSITYPATTERN, VECTOR, dealdim> ( index_setter),
      ...
      MethodOfLines_StateSpaceTimeHandler(
          ...
          const ActiveFEIndexSetterInterface<dealdim>& index_setter =
              ActiveFEIndexSetterInterface<dealdim> ()) :
            StateSpaceTimeHandler<FE, DOFHANDLER, SPARSITYPATTERN, VECTOR, dealdim> (times, index_setter),
        _sparse_mkr_dynamic(true), _constr_mkr_dynamic(true),
        _triangulation(triangulation),
        _state_dof_handler(_triangulation),
        _state_fe(&state_fe)
	...
	ReInit(unsigned int state_n_blocks,
	       const std::vector<unsigned int>& state_block_component)
      {
        ...
        StateSpaceTimeHandler<FE, DOFHANDLER, SPARSITYPATTERN, VECTOR, dealdim>::SetActiveFEIndicesState(_state_dof_handler);
	...
	GetStateDoFsPerBlock(int /*time_point*/ = -1) const
      ...
      GetMapDoFToSupportPoints()
      {
        ...
        DOpE::STHInternals::MapDoFsToSupportPoints<std::vector<Point<dealdim> >, dealdim >(GetStateDoFHandler(), _support_points);
