...
public:
   ...
   */
  Constraints(const std::vector<std::vector<unsigned int> >& local_control_constraints,
              unsigned int global_constraints)
  {
    _local_control_constraints.resize(local_control_constraints.size());
    _local_control_constraints_per_block.resize(local_control_constraints.size());
  ...
  Constraints(const Constraints& c)
  {
    ...
    _local_control_constraints_per_block.resize(c._local_control_constraints_per_block.size());
  ...
  {
    if(_local_control_constraints_per_block.size()==control_dofs_per_block.size())
    {
      _n_dofs = 0;
      for(unsigned int i = 0;  i < _local_control_constraints.size(); i++)
      {
	assert(control_dofs_per_block[i]%_local_control_constraints[i][0]==0);
	_local_control_constraints_per_block[i] =  control_dofs_per_block[i]/_local_control_constraints[i][0]*_local_control_constraints[i][1];
...
//  /**
//   * Returns the total number of global in time and space constraints.
//   */
//  unsigned int global_n_dofs() const
//  {
//    return _global_constraints;
//  }
//  /**
//   * Returns the total number of local in time and space constraints.
//   */
//  unsigned int local_n_dofs() const
//  {
//    assert(_n_dofs >= _local_global_constraints);
//    return _n_dofs-_local_global_constraints;
//  }
  /**
