...
namespace DOpE
{
  ...
  private:
    ...
    void
    InterpolateBoundaryValues (const DOpEWrapper::DoFHandler<dim, DH> *dof_handler,
    ...
    inline void
    ComputeNonlinearResidual_Recursive (PROBLEM &pde,
                                        VECTOR &residual,
                                        typename std::vector<
                                        typename DH<dim, dim>::cell_iterator> &element_iter,
                                        typename std::vector<
                                        typename dealii::Triangulation<
                                        dim>::cell_iterator> &tria_element_iter,
                                        const FullMatrix<SCALAR> &prolong_matrix,
                                        unsigned int coarse_index,
                                        unsigned int fine_index,
                                        Multimesh_ElementDataContainer<DH,
                                        VECTOR, dim> &edc,
                                        Multimesh_FaceDataContainer<DH,
                                        VECTOR, dim> &fdc);
    ...
    template <typename PROBLEM, template <int, int> class DH>
    inline void
    ComputeNonlinearRhs_Recursive (PROBLEM &pde,
                                   VECTOR &residual,
                                   typename std::vector<
                                   typename DH<dim, dim>::cell_iterator> &element_iter,
                                   typename std::vector<
                                   ...
                                   Multimesh_ElementDataContainer<DH,
                                   VECTOR, dim> &edc,
                                   Multimesh_FaceDataContainer<DH, VECTOR,
                                   dim> &fdc);
    ...
    inline void
    ComputeMatrix_Recursive (PROBLEM &pde,
                             MATRIX &matrix,
                             typename std::vector<
                             typename DH<dim, dim>::cell_iterator> &element_iter,
                             typename std::vector<
                             ...
                             unsigned int coarse_index,
                             unsigned int fine_index,
                             Multimesh_ElementDataContainer<DH, VECTOR,
                             dim> &edc,
    ...
    template <typename PROBLEM, template <int, int> class DH>
    inline SCALAR
    ComputeDomainScalar_Recursive (PROBLEM &pde,
                                   typename std::vector<
                                   typename DH<dim, dim>::cell_iterator> &element_iter,
                                   typename std::vector<
                                   typename dealii::Triangulation<dim>::cell_iterator> &tria_element_iter,
    ...
    template <typename PROBLEM, template <int, int> class DH>
    inline SCALAR
    ComputeBoundaryScalar_Recursive (PROBLEM &pde,
                                     typename std::vector<
                                     typename DH<dim, dim>::cell_iterator> &element_iter,
                                     typename std::vector<
                                     typename dealii::Triangulation<
  ...
  template <typename INTEGRATORDATACONT, typename VECTOR, typename SCALAR,
            int dim>
  IntegratorMultiMesh<INTEGRATORDATACONT, VECTOR, SCALAR, dim>::IntegratorMultiMesh (INTEGRATORDATACONT &idc)
    : idc_ (idc)
  {
  }
  ...
  template <typename PROBLEM>
  void
  IntegratorMultiMesh<INTEGRATORDATACONT, VECTOR, SCALAR, dim>::ComputeNonlinearResidual (PROBLEM &pde,
      VECTOR &residual)
  {
    residual = 0.;
    ...
    const auto tria_element_list = GridTools::get_finest_common_cells (
                                     dof_handler[0]->GetDEALDoFHandler ().get_triangulation (),
    ...
    const auto element_list = GridTools::get_finest_common_cells (
                                dof_handler[0]->GetDEALDoFHandler (),
    ...
    idc_.InitializeMMEDC (pde.GetUpdateFlags (),
                          *(pde.GetBaseProblem ().GetSpaceTimeHandler ()), element,
                          tria_element, this->GetParamData (), this->GetDomainData ());
    ...
    for (; element_iter != element_list.end (); element_iter++)
      {
        ...
        if (element[0]->has_children ())
          {
            prolong_matrix = IdentityMatrix (
                               element[1]->get_fe ().dofs_per_cell);
            coarse_index = 1;
            fine_index = 0;
          }
        else
          {
            if (element[1]->has_children ())
              {
                prolong_matrix = IdentityMatrix (
                                   element[0]->get_fe ().dofs_per_cell);
                coarse_index = 0;
                fine_index = 1;
              }
            else
        ...
        ComputeNonlinearResidual_Recursive (pde, residual, element,
                                            tria_element, prolong_matrix, coarse_index, fine_index, edc,
                                            fdc);
        tria_element_iter++;
      }
  ...
  template <typename PROBLEM>
  void
  IntegratorMultiMesh<INTEGRATORDATACONT, VECTOR, SCALAR, dim>::ComputeNonlinearLhs (PROBLEM &pde,
      VECTOR &residual)
  {
    {
      throw DOpEException ("This function needs to be implemented!",
  ...
  template <typename PROBLEM>
  void
  IntegratorMultiMesh<INTEGRATORDATACONT, VECTOR, SCALAR, dim>::ComputeNonlinearRhs (PROBLEM &pde,
      VECTOR &residual)
  {
    residual = 0.;
    ...
    for (; element_iter != element_list.end (); element_iter++)
      {
        ...
        ComputeNonlinearRhs_Recursive (pde, residual, element, tria_element,
                                       prolong_matrix, coarse_index, fine_index, edc, fdc);
        tria_element_iter++;
      }
    //Check if some preset righthandside exists.
    AddPresetRightHandSide (1., residual);
  ...
  template <typename PROBLEM, typename MATRIX>
  void
  IntegratorMultiMesh<INTEGRATORDATACONT, VECTOR, SCALAR, dim>::ComputeMatrix (PROBLEM &pde,
      MATRIX &matrix)
  {
    matrix = 0.;
    ...
    for (; element_iter != element_list.end (); element_iter++)
      {
        ...
        ComputeMatrix_Recursive (pde, matrix, element, tria_element,
                                 prolong_matrix, coarse_index, fine_index, edc, fdc);
        tria_element_iter++;
      }
  }
  ...
  template <typename PROBLEM>
  SCALAR
  IntegratorMultiMesh<INTEGRATORDATACONT, VECTOR, SCALAR, dim>::ComputeDomainScalar (PROBLEM &pde)
  {
    SCALAR ret = 0.;
    ...
    for (; element_iter != element_list.end (); element_iter++)
      {
        ...
        ret += ComputeDomainScalar_Recursive (pde, element, tria_element,
                                              prolong_matrix, coarse_index, fine_index, edc);
        tria_element_iter++;
      }
    return ret;
  }
  ...
  template <typename PROBLEM>
  SCALAR
  IntegratorMultiMesh<INTEGRATORDATACONT, VECTOR, SCALAR, dim>::ComputePointScalar (PROBLEM &pde)
  {
  ...
  template <typename PROBLEM>
  SCALAR
  IntegratorMultiMesh<INTEGRATORDATACONT, VECTOR, SCALAR, dim>::ComputeBoundaryScalar (PROBLEM &pde)
  {
    SCALAR ret = 0.;
    // Begin integration
    const auto &dof_handler =
    ...
    for (; element_iter != element_list.end (); element_iter++)
      {
        ...
        ret += ComputeBoundaryScalar_Recursive (pde, element, tria_element,
                                                prolong_matrix, coarse_index, fine_index, fdc);
        tria_element_iter++;
      }
  ...
  template <typename PROBLEM>
  SCALAR
  IntegratorMultiMesh<INTEGRATORDATACONT, VECTOR, SCALAR, dim>::ComputeFaceScalar (PROBLEM & /*pde*/
                                                                                  )
  {
    throw DOpEException ("This function needs to be implemented!",
...
//          }
  ...
  template <typename PROBLEM>
  SCALAR
  IntegratorMultiMesh<INTEGRATORDATACONT, VECTOR, SCALAR, dim>::ComputeAlgebraicScalar (PROBLEM &pde)
  {
  ...
  template <typename PROBLEM>
  void
  IntegratorMultiMesh<INTEGRATORDATACONT, VECTOR, SCALAR, dim>::ComputeNonlinearAlgebraicResidual (PROBLEM &pde,
      VECTOR &residual)
  {
    residual = 0.;
  ...
  template <typename PROBLEM>
  void
  IntegratorMultiMesh<INTEGRATORDATACONT, VECTOR, SCALAR, dim>::ComputeLocalControlConstraints (PROBLEM &pde,
      VECTOR &constraints)
  {
    constraints = 0.;
  ...
  template <typename PROBLEM>
  void
  IntegratorMultiMesh<INTEGRATORDATACONT, VECTOR, SCALAR, dim>::ApplyInitialBoundaryValues (PROBLEM &pde,
      VECTOR &u)
  {
  ...
  template <typename INTEGRATORDATACONT, typename VECTOR, typename SCALAR,
            int dim>
  void
  IntegratorMultiMesh<INTEGRATORDATACONT, VECTOR, SCALAR, dim>::AddDomainData (std::string name,
      const VECTOR *new_data)
  {
  ...
  template <typename INTEGRATORDATACONT, typename VECTOR, typename SCALAR,
            int dim>
  void
  IntegratorMultiMesh<INTEGRATORDATACONT, VECTOR, SCALAR, dim>::DeleteDomainData (std::string name)
  {
  ...
  template <typename INTEGRATORDATACONT, typename VECTOR, typename SCALAR,
            int dim>
  void
  IntegratorMultiMesh<INTEGRATORDATACONT, VECTOR, SCALAR, dim>::AddParamData (std::string name,
      const dealii::Vector<
      SCALAR> *new_data)
  {
  ...
  template <typename INTEGRATORDATACONT, typename VECTOR, typename SCALAR,
            int dim>
  void
  IntegratorMultiMesh<INTEGRATORDATACONT, VECTOR, SCALAR, dim>::DeleteParamData (std::string name)
  {
  ...
  template <template <int, int> class DH>
  void
  IntegratorMultiMesh<INTEGRATORDATACONT, VECTOR, SCALAR, dim>::InterpolateBoundaryValues (const DOpEWrapper::DoFHandler<
      dim,
      DH> *dof_handler,
      const unsigned int color,
      ...
      bool> &comp_mask) const
  {
    dealii::VectorTools::interpolate_boundary_values (
      dof_handler->GetDEALDoFHandler (), color, function, boundary_values,
      comp_mask);
  }
  ...
  template <typename PROBLEM, template <int, int> class DH>
  void
  IntegratorMultiMesh<INTEGRATORDATACONT, VECTOR, SCALAR, dim>::ComputeNonlinearResidual_Recursive (PROBLEM &pde,
      VECTOR &residual,
      typename std::vector<
      typename DH<
      dim,
      dim>::cell_iterator> &element,
      typename std::vector<
      typename dealii::Triangulation<
      dim>::cell_iterator> &tria_element,
      const FullMatrix<
      SCALAR> &prolong_matrix,
      unsigned int coarse_index,
      unsigned int fine_index,
    ...
    if (!element[0]->has_children () && !element[1]->has_children ())
      {
        ...
        bool need_boundary_integrals =
          (boundary_equation_colors.size () > 0);
        bool need_interfaces = pde.HasInterfaces ();
        ...
        if (need_boundary_integrals && element[b_index]->at_boundary ())
          {
            for (unsigned int face = 0;
                 face < dealii::GeometryInfo<dim>::faces_per_cell; ++face)
              {
                ...
                if (element[b_index]->face (face)->at_boundary () && (find (
                                                                        boundary_equation_colors.begin (),
                                                                        boundary_equation_colors.end (),
                                                                        element[b_index]->face (
                                                                          face)->boundary_id ())
                                                                      != boundary_equation_colors.end ()))
#else
...
#endif
                  {
                    fdc.ReInit (coarse_index, fine_index, prolong_matrix,
                                face);
    ...
    else
      {
        ...
        unsigned int local_n_dofs =
          element[coarse_index]->get_fe ().dofs_per_cell;

        typename DH<dim, dim>::cell_iterator dofh_fine = element[fine_index];
        typename dealii::Triangulation<dim>::cell_iterator tria_fine =
          tria_element[fine_index];

        for (unsigned int child = 0;
             child < GeometryInfo<dim>::max_children_per_cell; ++child)
          {
            FullMatrix<SCALAR> new_matrix (local_n_dofs);
            element[coarse_index]->get_fe ().get_prolongation_matrix (child).mmult (
              new_matrix, prolong_matrix);
            element[fine_index] = dofh_fine->child (child);
            tria_element[fine_index] = tria_fine->child (child);

            ComputeNonlinearResidual_Recursive (pde, residual, element,
                                                tria_element, new_matrix, coarse_index, fine_index, edc,
                                                fdc);
          }
      }
  }
  ...
  template <typename PROBLEM, template <int, int> class DH>
  void
  IntegratorMultiMesh<INTEGRATORDATACONT, VECTOR, SCALAR, dim>::ComputeNonlinearRhs_Recursive (PROBLEM &pde,
      VECTOR &residual,
      typename std::vector<
      typename DH<
      dim,
    ...
    else
      {
        ...
        for (unsigned int child = 0;
             child < GeometryInfo<dim>::max_children_per_cell; ++child)
          {
            ...
            ComputeNonlinearRhs_Recursive (pde, residual, element,
                                           tria_element, new_matrix, coarse_index, fine_index, edc,
                                           fdc);
          }
      }
  ...
  void
  IntegratorMultiMesh<INTEGRATORDATACONT, VECTOR, SCALAR, dim>::ComputeMatrix_Recursive (PROBLEM &pde,
      MATRIX &matrix,
      typename std::vector<
      typename DH<
      dim,
    ...
    else
      {
        ...
        for (unsigned int child = 0;
             child < GeometryInfo<dim>::max_children_per_cell; ++child)
          {
            ...
            ComputeMatrix_Recursive (pde, matrix, element, tria_element,
                                     new_matrix, coarse_index, fine_index, edc, fdc);
          }
      }
  }
  ...
  template <typename PROBLEM, template <int, int> class DH>
  SCALAR
  IntegratorMultiMesh<INTEGRATORDATACONT, VECTOR, SCALAR, dim>::ComputeDomainScalar_Recursive (PROBLEM &pde,
      typename std::vector<
      typename DH<
      dim,
      dim>::cell_iterator> &element,
    ...
    else
      {
        ...
        for (unsigned int child = 0;
             child < GeometryInfo<dim>::max_children_per_cell; ++child)
          {
            ...
            ret += ComputeDomainScalar_Recursive (pde, element,
                                                  tria_element, new_matrix, coarse_index, fine_index, edc);
          }
        return ret;
      }
  ...
  template <typename PROBLEM, template <int, int> class DH>
  SCALAR
  IntegratorMultiMesh<INTEGRATORDATACONT, VECTOR, SCALAR, dim>::ComputeBoundaryScalar_Recursive (PROBLEM &pde,
      typename std::vector<
      typename DH<
      dim,
      dim>::cell_iterator> &element,
...
#if DEAL_II_VERSION_GTE(8,3,0)
            if (element[b_index]->face (face)->at_boundary () && (find (
                                                                    boundary_functional_colors.begin (),
    ...
    else
      {
        ...
        for (unsigned int child = 0;
             child < GeometryInfo<dim>::max_children_per_cell; ++child)
          {
            ...
            ret += ComputeBoundaryScalar_Recursive (pde, element,
                                                    tria_element, new_matrix, coarse_index, fine_index, fdc);
          }
        return ret;
      }
  }
