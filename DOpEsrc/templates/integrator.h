...
namespace DOpE
{
  ...
  public:
    ...
    ComputeRefinementIndicators (PROBLEM &pde,
                                 DWRDataContainer<STH, INTEGRATORDATACONT,
                                 EDC, FDC, VECTOR> &dwrc);
    /**
     * This function is used to calculate indicators based
...
//          }
  ...
  template <typename INTEGRATORDATACONT, typename VECTOR, typename SCALAR,
            int dim>
  Integrator<INTEGRATORDATACONT, VECTOR, SCALAR, dim>::Integrator (INTEGRATORDATACONT &idc)
    : idc1_ (idc),
      idc2_ (idc)
  {
  }
  ...
  template <typename INTEGRATORDATACONT, typename VECTOR, typename SCALAR,
            int dim>
  Integrator<INTEGRATORDATACONT, VECTOR, SCALAR, dim>::Integrator (INTEGRATORDATACONT &idc1,
      INTEGRATORDATACONT &idc2)
    : idc1_ (idc1),
      idc2_ (idc2)
  {
  ...
  template <typename PROBLEM>
  void
  Integrator<INTEGRATORDATACONT, VECTOR, SCALAR, dim>::ComputeNonlinearResidual (PROBLEM &pde,
      VECTOR &residual)
  {
    residual = 0.;
    ...
    if (need_point_rhs)
      {
        VECTOR point_rhs;
        pde.PointRhs (this->GetParamData (), this->GetDomainData (),
                      point_rhs, -1.);
        residual += point_rhs;
      }
  ...
  template <typename PROBLEM>
  void
  Integrator<INTEGRATORDATACONT, VECTOR, SCALAR, dim>::ComputeNonlinearLhs (PROBLEM &pde,
      VECTOR &residual)
  {
    residual = 0.;
    // Begin integration
  ...
  template <typename PROBLEM>
  void
  Integrator<INTEGRATORDATACONT, VECTOR, SCALAR, dim>::ComputeNonlinearRhs (PROBLEM &pde,
      VECTOR &residual)
  {
    residual = 0.;
...
//        }
  ...
  template <typename PROBLEM, typename MATRIX>
  void
  Integrator<INTEGRATORDATACONT, VECTOR, SCALAR, dim>::ComputeMatrix (PROBLEM &pde,
      MATRIX &matrix)
  {
    matrix = 0.;
  ...
  template <typename PROBLEM>
  SCALAR
  Integrator<INTEGRATORDATACONT, VECTOR, SCALAR, dim>::ComputeDomainScalar (PROBLEM &pde)
  {
    {
      SCALAR ret = 0.;
      ...
      return dealii::Utilities::MPI::sum (ret,
                                          pde.GetBaseProblem ().GetSpaceTimeHandler ()->GetMPIComm ());
    }
  }
  /*******************************************************************************************/
  ...
  template <typename PROBLEM>
  SCALAR
  Integrator<INTEGRATORDATACONT, VECTOR, SCALAR, dim>::ComputePointScalar (PROBLEM &pde)
  {
  ...
  template <typename PROBLEM>
  SCALAR
  Integrator<INTEGRATORDATACONT, VECTOR, SCALAR, dim>::ComputeBoundaryScalar (PROBLEM &pde)
  {
    {
      SCALAR ret = 0.;
      // Begin integration
      ...
      GetIntegratorDataContainerFunc ().InitializeFDC (
        ...
        *(pde.GetBaseProblem ().GetSpaceTimeHandler ()), element,
        this->GetParamData (), this->GetDomainData ());
      auto &fdc =
        GetIntegratorDataContainerFunc ().GetFaceDataContainer ();

      std::vector<unsigned int> boundary_functional_colors =
        pde.GetBoundaryFunctionalColors ();
      bool need_boundary_integrals = (boundary_functional_colors.size ()
                                      > 0);
      if (!need_boundary_integrals)
        {
  ...
  template <typename PROBLEM>
  SCALAR
  Integrator<INTEGRATORDATACONT, VECTOR, SCALAR, dim>::ComputeFaceScalar (PROBLEM &pde)
  {
    SCALAR ret = 0.;
    // Begin integration
    const auto &dof_handler =
    ...
    if (!need_faces)
      {
        throw DOpEException ("No faces required!",
  ...
  template <typename PROBLEM>
  SCALAR
  Integrator<INTEGRATORDATACONT, VECTOR, SCALAR, dim>::ComputeAlgebraicScalar (PROBLEM &pde)
  {
  ...
  template <typename PROBLEM>
  void
  Integrator<INTEGRATORDATACONT, VECTOR, SCALAR, dim>::ComputeNonlinearAlgebraicResidual (PROBLEM &pde,
      VECTOR &residual)
  {
    residual = 0.;
  ...
  template <typename PROBLEM>
  void
  Integrator<INTEGRATORDATACONT, VECTOR, SCALAR, dim>::ComputeLocalControlConstraints (PROBLEM &pde,
      VECTOR &constraints)
  {
    constraints = 0.;
  ...
  template <typename PROBLEM>
  void
  Integrator<INTEGRATORDATACONT, VECTOR, SCALAR, dim>::ApplyTransposedInitialBoundaryValues (PROBLEM & /*pde*/,
      VECTOR &/*u*/)
  {
  ...
  template <typename PROBLEM>
  void
  Integrator<INTEGRATORDATACONT, VECTOR, SCALAR, dim>::ApplyInitialBoundaryValues (PROBLEM &pde,
      VECTOR &u)
  {
  ...
  template <typename INTEGRATORDATACONT, typename VECTOR, typename SCALAR,
            int dim>
  void
  Integrator<INTEGRATORDATACONT, VECTOR, SCALAR, dim>::AddDomainData (std::string name,
      const VECTOR *new_data)
  {
  ...
  template <typename INTEGRATORDATACONT, typename VECTOR, typename SCALAR,
            int dim>
  void
  Integrator<INTEGRATORDATACONT, VECTOR, SCALAR, dim>::DeleteDomainData (std::string name)
  {
  ...
  template <typename INTEGRATORDATACONT, typename VECTOR, typename SCALAR,
            int dim>
  void
  Integrator<INTEGRATORDATACONT, VECTOR, SCALAR, dim>::AddParamData (std::string name,
      const dealii::Vector<
      SCALAR> *new_data)
  {
  ...
  template <typename INTEGRATORDATACONT, typename VECTOR, typename SCALAR,
            int dim>
  void
  Integrator<INTEGRATORDATACONT, VECTOR, SCALAR, dim>::DeleteParamData (std::string name)
  {
  ...
  template <typename PROBLEM, class STH, class EDC, class FDC>
  void
  Integrator<INTEGRATORDATACONT, VECTOR, SCALAR, dim>::ComputeRefinementIndicators (PROBLEM &pde,
      DWRDataContainer<
      STH,
      INTEGRATORDATACONT,
      EDC,
...
#else
            face_integrals[element_it->face (face_no)->vertex_index ()] =
              face_init;
#endif
          }
      }
        ...
        if (element[0]->is_locally_owned ())
          {
            ...
            for (unsigned int l = 0; l < n_error_comps; l++)
              {
                dwrc.GetErrorIndicators (l) (element_index) =
                  element_sum[l];
              }
                ...
                if (face_it->at_boundary ())
                  {
                    ...
                    pde.BoundaryErrorContribution (fdc, dwrc, element_sum,
                                                   1.);
...
#else
                    Assert(
                      face_integrals.find (element[0]->face(face)->vertex_index()) != face_integrals.end(),
                      ExcInternalError());
                    Assert(
                      face_integrals[element[0]->face(face)->vertex_index()] == face_init,
                      ExcInternalError());
                    ...
                    if (element[0]->neighbor (face)->has_children ())
                      {
                        ...
                        for (unsigned int subface_no = 0;
                             ...
                             ++subface_no)
                          {
                            ...
                            pde.FaceErrorContribution (fdc, dwrc,
                                                       element_sum, 1.);
                            for (unsigned int l = 0; l < n_error_comps; l++)
                              {
                                sum[l] += element_sum[l];
...
#else
                            face_integrals[element[0]->neighbor_child_on_subface (
                                             face, subface_no)->face (
                                             element[0]->neighbor_of_neighbor (face))->vertex_index ()] =
                                               element_sum;
#endif
                            element_sum.clear ();
                            element_sum.resize (n_error_comps, 0);
...
//        bool need_boundary_integrals = (boundary_equation_colors.size() > 0);
  ...
  template <template <int, int> class DH>
  void
  Integrator<INTEGRATORDATACONT, VECTOR, SCALAR, dim>::InterpolateBoundaryValues (const DOpEWrapper::Mapping<
      dim,
      DH> &mapping,
