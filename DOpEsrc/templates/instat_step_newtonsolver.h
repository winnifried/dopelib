...
namespace DOpE
{
  ...
  public:
     ...
     */
    InstatStepNewtonSolver (INTEGRATOR &integrator,
  ...
  template <typename INTEGRATOR, typename LINEARSOLVER, typename VECTOR>
  void
  InstatStepNewtonSolver<INTEGRATOR, LINEARSOLVER, VECTOR>::declare_params (ParameterReader &param_reader)
  {
  ...
  template <typename INTEGRATOR, typename LINEARSOLVER, typename VECTOR>
  InstatStepNewtonSolver<INTEGRATOR, LINEARSOLVER, VECTOR>::InstatStepNewtonSolver (INTEGRATOR &integrator,
      ParameterReader &param_reader)
  ...
  template <typename INTEGRATOR, typename LINEARSOLVER, typename VECTOR>
  InstatStepNewtonSolver<INTEGRATOR, LINEARSOLVER, VECTOR>::~InstatStepNewtonSolver ()
  {
  }
  ...
  template <typename PROBLEM>
  void
  InstatStepNewtonSolver<INTEGRATOR, LINEARSOLVER, VECTOR>::ReInit (PROBLEM &pde)
  {
  ...
  template <typename PROBLEM>
  void
  InstatStepNewtonSolver<INTEGRATOR, LINEARSOLVER, VECTOR>::NonlinearLastTimeEvals (PROBLEM &pde,
      const VECTOR &last_time_solution,
      VECTOR &residual)
  {
    VECTOR tmp_residual;
  ...
  template <typename PROBLEM>
  bool
  InstatStepNewtonSolver<INTEGRATOR, LINEARSOLVER, VECTOR>::NonlinearSolve_Initial (PROBLEM &pde,
      VECTOR &solution,
      ...
      std::string algo_level)
  {
    ...
    out << algo_level << "Newton step: " << 0 << "\t Residual (rel.):   "
        << std::scientific
        << pde.GetOutputHandler ()->ZeroTolerance (firstres / firstres, 1.0);
  ...
  template <typename PROBLEM>
  bool
  InstatStepNewtonSolver<INTEGRATOR, LINEARSOLVER, VECTOR>::NonlinearSolve (PROBLEM &pde,
      const VECTOR &last_time_solution,
      VECTOR &solution,
      bool apply_boundary_values,
      bool force_matrix_build,
  ...
  template <typename INTEGRATOR, typename LINEARSOLVER, typename VECTOR>
  INTEGRATOR &
  InstatStepNewtonSolver<INTEGRATOR, LINEARSOLVER, VECTOR>::GetIntegrator ()
  {
    return integrator_;
  }
