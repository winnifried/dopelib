...
namespace DOpE
{
  ...
  public:
     ...
     */
    FractionalStepThetaStepNewtonSolver (INTEGRATOR &integrator,
    ...
    template <typename PROBLEM>
    bool
    NonlinearSolve_Initial (PROBLEM &pde,
                            VECTOR &solution,
                            bool apply_boundary_values = true,
                            bool force_matrix_build = false,
                            int priority = 5,
    ...
    template <typename PROBLEM>
    void
    NonlinearLastTimeEvals (PROBLEM &pde,
                            const VECTOR &last_time_solution,
                            VECTOR &residual);
  ...
  template <typename INTEGRATOR, typename LINEARSOLVER, typename VECTOR>
  void
  FractionalStepThetaStepNewtonSolver<INTEGRATOR, LINEARSOLVER, VECTOR>::declare_params (ParameterReader &param_reader)
  {
    param_reader.SetSubsection ("newtonsolver parameters");
    param_reader.declare_entry ("nonlinear_global_tol", "1.e-12",
                                Patterns::Double (0), "global tolerance for the newton iteration");
    param_reader.declare_entry ("nonlinear_tol", "1.e-10",
                                Patterns::Double (0), "relative tolerance for the newton iteration");
    param_reader.declare_entry ("nonlinear_maxiter", "10",
                                Patterns::Integer (0), "maximal number of newton iterations");
    param_reader.declare_entry ("nonlinear_rho", "0.1", Patterns::Double (0),
  ...
  template <typename INTEGRATOR, typename LINEARSOLVER, typename VECTOR>
  FractionalStepThetaStepNewtonSolver<INTEGRATOR, LINEARSOLVER, VECTOR>::FractionalStepThetaStepNewtonSolver (INTEGRATOR &integrator,
      ParameterReader &param_reader)
    : LINEARSOLVER (param_reader),
  ...
  template <typename INTEGRATOR, typename LINEARSOLVER, typename VECTOR>
  FractionalStepThetaStepNewtonSolver<INTEGRATOR, LINEARSOLVER, VECTOR>::~FractionalStepThetaStepNewtonSolver ()
  {
  }
  ...
  template <typename PROBLEM>
  void
  FractionalStepThetaStepNewtonSolver<INTEGRATOR, LINEARSOLVER, VECTOR>::ReInit (PROBLEM &pde)
  {
  ...
  template <typename PROBLEM>
  void
  FractionalStepThetaStepNewtonSolver<INTEGRATOR, LINEARSOLVER, VECTOR>::NonlinearLastTimeEvals (PROBLEM &pde,
      const VECTOR &last_time_solution,
      VECTOR &residual)
  {
    VECTOR tmp_residual;
    ...
    residual = 0.;
    GetIntegrator ().AddDomainData ("last_newton_solution",
                                    &last_time_solution);
    GetIntegrator ().AddDomainData ("last_time_solution",
                                    &last_time_solution);
  ...
  template <typename PROBLEM>
  bool
  FractionalStepThetaStepNewtonSolver<INTEGRATOR, LINEARSOLVER, VECTOR>::NonlinearSolve_Initial (PROBLEM &pde,
      VECTOR &solution,
      ...
      std::string algo_level)
  {
    ...
    pde.GetOutputHandler ()->Write (residual, "Residual" + pde.GetType (),
                                    pde.GetDoFType ());
    ...
    while (res > nonlinear_global_tol_ && res > firstres * nonlinear_tol_)
      {
        ...
        //Linesearch
        {
          ...
          pde.GetOutputHandler ()->Write (residual,
                                          "Residual" + pde.GetType (), pde.GetDoFType ());
          pde.GetOutputHandler ()->Write (du, "Update" + pde.GetType (),
                                          pde.GetDoFType ());
  ...
  template <typename PROBLEM>
  bool
  FractionalStepThetaStepNewtonSolver<INTEGRATOR, LINEARSOLVER, VECTOR>::NonlinearSolve (PROBLEM &pde,
      const VECTOR &last_time_solution,
      VECTOR &solution,
      bool apply_boundary_values,
      bool force_matrix_build,
  ...
  template <typename INTEGRATOR, typename LINEARSOLVER, typename VECTOR>
  INTEGRATOR &
  FractionalStepThetaStepNewtonSolver<INTEGRATOR, LINEARSOLVER, VECTOR>::GetIntegrator ()
  {
    return integrator_;
  }
