...
namespace DOpE
{
  namespace Networks
  {
    namespace fdcinternal
    {
      ...
      {
      public:
        Network_FaceDataContainerInternal (unsigned int pipe_,
                                           unsigned int n_pipes,
                                           unsigned int n_comp,
                                           const std::map<std::string,
                                           const dealii::Vector<double>*> &param_values,
                                           const std::map<std::string,
                                           const dealii::BlockVector<
        ...
        GetNbrFaceGradsControl (std::string name,
                                std::vector<
                                std::vector<
                                dealii::Tensor<1, targetdim> > > &values) const;
      ...
      /**********************************************************************/
      template <int dim>
      Network_FaceDataContainerInternal<dim>::Network_FaceDataContainerInternal (unsigned int pipe,
          unsigned int n_pipes,
          unsigned int n_comp,
          const std::map<
          std::string,
        ...
        : param_values_ (param_values),
          ...
          need_neighbour_ (need_neighbour)
      {
        pipe_ = pipe;
        n_pipes_ = n_pipes;
        n_comp_ = n_comp;
      ...
      template <int dim>
      void
      Network_FaceDataContainerInternal<dim>::GetFaceValuesState (std::string name,
                                                                  std::vector<
                                                                  double> &values) const
      {
      ...
      template <int dim>
      void
      Network_FaceDataContainerInternal<dim>::GetFaceValuesControl (std::string name,
          std::vector<
          double> &values) const
      {
      ...
      template <int targetdim>
      void
      Network_FaceDataContainerInternal<dim>::GetFaceGradsState (std::string name,
                                                                 std::vector<
                                                                 dealii::Tensor<
                                                                 1,
                                                                 targetdim> > &values) const
      {
        this->GetGrads<targetdim> (this->GetFEFaceValuesState (), name,
                                   values);
      }
      ...
      template <int targetdim>
      void
      Network_FaceDataContainerInternal<dim>::GetFaceGradsControl (std::string name,
          std::vector<
          dealii::Tensor<
          1,
          targetdim> > &values) const
      ...
      template <int dim>
      void
      Network_FaceDataContainerInternal<dim>::GetNbrFaceValuesState (std::string name,
          std::vector<
          double> &values) const
      {
      ...
      template <int dim>
      void
      Network_FaceDataContainerInternal<dim>::GetNbrFaceValuesControl (std::string name,
          std::vector<
          double> &values) const
      {
      ...
      template <int targetdim>
      void
      Network_FaceDataContainerInternal<dim>::GetNbrFaceGradsState (std::string name,
          std::vector<
          Tensor<
          1,
          targetdim> > &values) const
      ...
      template <int targetdim>
      void
      Network_FaceDataContainerInternal<dim>::GetNbrFaceGradsControl (std::string name,
          std::vector<
          Tensor<
          1,
          targetdim> > &values) const
      ...
      template <int dim>
      void
      Network_FaceDataContainerInternal<dim>::GetValues (const dealii::FEFaceValuesBase<
                                                         dim> &fe_values,
                                                         std::string name,
                                                         std::vector<double> &values) const
      {
      ...
      template <int targetdim>
      void
      Network_FaceDataContainerInternal<dim>::GetGrads (const dealii::FEFaceValuesBase<
                                                        dim> &fe_values,
                                                        std::string name,
                                                        std::vector<
                                                        dealii::Tensor<
                                                        1, targetdim> > &values) const
      {
        typename std::map<std::string, const dealii::BlockVector<double> *>::const_iterator it =
          this->GetDomainValues ().find (name);
        if (it == this->GetDomainValues ().end ())
        ...
        fe_values.get_function_gradients (it->second->block (pipe_),
                                          values);
      }
