  ...
  namespace Networks
  {
    ...
    public:
      ...
      Network_FaceDataContainer (unsigned int pipe,
                                 ...
                                 bool need_neighbour)
        : fdcinternal::Network_FaceDataContainerInternal<dim> (pipe,
                                                               n_pipes, n_comp, param_values, domain_values,
                                                               need_neighbour),
        element_ (element),
                            ...
                            (sth.GetFESystem ("control")), quad, update_flags)
      {
        ...
        this->PrivateConstructor (quad, update_flags, sth,
                                  need_neighbour);
      }
      /**
      ...
      Network_FaceDataContainer (unsigned int pipe,
                                 ...
                                 dealii::DoFHandler, SPARSITYPATTERN,
        ...
        if (need_neighbour) //so we need FEFAcevalues etc. for the neighbour too.
          {
            nbr_state_fe_values_ = new DOpEWrapper::FEFaceValues<dim> (
              (sth.GetFESystem ("state")), quad, update_flags);
            state_fe_subface_values_ = new DOpEWrapper::FESubfaceValues<
            dim> ((sth.GetFESystem ("state")), quad, update_flags);
          }
        else
          {
...
#pragma GCC diagnostic pop
    ...
    template <typename VECTOR, int dim>
    void
    Network_FaceDataContainer<dealii::DoFHandler, VECTOR, dim>::ReInit (unsigned int face_no)
    {
      ...
      if (this->GetControlIndex () < element_.size ())
        {
          control_fe_subface_values_->reinit (
            element_[this->GetControlIndex ()], face_no,
            this->GetSubFace ());
    ...
    template <typename VECTOR, int dim>
    unsigned int
    Network_FaceDataContainer<dealii::DoFHandler, VECTOR, dim>::GetNbrMaterialId (unsigned int face) const
    {
...
//      return element_[0]->face(this->GetFace())->diameter();
    ...
    template <typename VECTOR, int dim>
    void
    Network_FaceDataContainer<dealii::DoFHandler, VECTOR, dim>::GetFluxValues (std::string name,
        std::vector<
        double> &values) const
    {
      ...
      assert(
        it->second->block (this->GetNPipes ()).size () == 2
        * this->GetNPipes ());
      if (this->GetBoundaryIndicator () == 0)
        {
          //left boundary
          values[0] = it->second->block (this->GetNPipes ())[this->GetPipe ()
                                                             * this->GetNComp ()];
        }
      else
        {
          //right boundary
          values[0] =
            it->second->block (this->GetNPipes ())[this->GetNPipes ()
                                                   * this->GetNComp ()
                                                   + this->GetPipe () * this->GetNComp ()];
        }
    }
      ...
      if (this->GetBoundaryIndicator () == 0)
        {
          ...
          for (unsigned int c = 0; c < this->GetNComp (); c++)
            {
              values[0][c] =
                it->second->block (this->GetNPipes ())[this->GetPipe ()
                                                       * this->GetNComp ()
                                                       + c];
            }
        }
