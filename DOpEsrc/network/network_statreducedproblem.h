...
namespace DOpE
{
  namespace Networks
  {
    ...
    class Network_StatReducedProblem : public StatReducedProblem<
      CONTROLNONLINEARSOLVER, NONLINEARSOLVER, CONTROLINTEGRATOR,
      INTEGRATOR, PROBLEM, dealii::BlockVector<double>, dopedim, dealdim>
    {
    public:
      /**
      ...
      Network_StatReducedProblem (PROBLEM *OP,
       ...
       */
      void
      WriteToFile (const dealii::BlockVector<double> &v,
                   std::string name,
                   std::string outfile,
                   std::string dof_type,
                   std::string filetype);
       ...
       */
      void
      WriteToFile (const ControlVector<dealii::BlockVector<double>> &v,
                   std::string name,
                   std::string dof_type);
    ...
    private:
      friend class SolutionExtractor<
        Network_StatReducedProblem<CONTROLNONLINEARSOLVER,
        NONLINEARSOLVER, CONTROLINTEGRATOR, INTEGRATOR, PROBLEM,
        dopedim, dealdim>, dealii::BlockVector<double>> ;

      typedef StatReducedProblem<CONTROLNONLINEARSOLVER, NONLINEARSOLVER,
              CONTROLINTEGRATOR, INTEGRATOR, PROBLEM,
              dealii::BlockVector<double>, dopedim, dealdim> BASE_;
    ...
    Network_StatReducedProblem<CONTROLNONLINEARSOLVER, NONLINEARSOLVER,
                               CONTROLINTEGRATOR, INTEGRATOR, PROBLEM, dopedim, dealdim>::declare_params (ParameterReader &param_reader)
    {
    ...
    template <typename INTEGRATORDATACONT>
    Network_StatReducedProblem<CONTROLNONLINEARSOLVER, NONLINEARSOLVER,
                               CONTROLINTEGRATOR, INTEGRATOR, PROBLEM, dopedim, dealdim>::Network_StatReducedProblem (PROBLEM *OP,
                                   DOpEtypes::VectorStorageType state_behavior,
                                   ParameterReader &param_reader,
                                   INTEGRATORDATACONT &idc,
                                   int base_priority)
    ...
    Network_StatReducedProblem<CONTROLNONLINEARSOLVER, NONLINEARSOLVER,
                               CONTROLINTEGRATOR, INTEGRATOR, PROBLEM, dopedim, dealdim>::Network_StatReducedProblem (PROBLEM *OP,
                                   ...
                                   STATEINTEGRATORDATACONT &s_idc,
                                   int base_priority)
                                 : BASE_ (OP, state_behavior, param_reader, c_idc, s_idc,
                                          base_priority)
    {
    }
    ...
    void
    Network_StatReducedProblem<CONTROLNONLINEARSOLVER, NONLINEARSOLVER,
                               CONTROLINTEGRATOR, INTEGRATOR, PROBLEM, dopedim, dealdim>::WriteToFile (const BlockVector<
                                   double> &v,
                                   std::string name,
                                   std::string outfile,
                                   std::string dof_type,
                                   std::string filetype)
    {
      if (dof_type == "state")
        {
          STH_* sth =
            dynamic_cast<STH_ *> (this->GetProblem ()->GetSpaceTimeHandler ());
          if (sth == NULL)
            {
              throw DOpEException (
                "Using Networks::Network_StatReducedProblem with wrong SpaceTimeHandler",
                "Networks::Network_StatReducedProblem::WriteToFile");
            }
          ...
          for (unsigned int p = 0; p < n_pipes; p++)
            {
              ...
              tmp2 << "_Pipe_" << std::setfill ('0') << std::setw (5) << p
                   << "_";
          ...
          out << "# For Residuals the first " << n_pipes
              << " lines are the residual between the outflow of the "
              << n_pipes << " pipes compared to the outflow flux."
              << std::endl;
          out << "#    the remaining " << n_pipes
              << " lines are the residuals of the node coupling conditions"
              << std::endl;
          out << "# For all other vectors the first " << n_pipes
              << " lines are the fluxes at the 'left' of the pipes"
              << std::endl;
          out << "#    the remaining " << n_pipes
              << " lines are the fluxes at the 'right'." << std::endl;
    ...
    void
    Network_StatReducedProblem<CONTROLNONLINEARSOLVER, NONLINEARSOLVER,
                               CONTROLINTEGRATOR, INTEGRATOR, PROBLEM, dopedim, dealdim>::WriteToFile (const ControlVector<
                                   dealii::BlockVector<
                                   double>> &v,
                                   std::string name,
                                   std::string dof_type)
