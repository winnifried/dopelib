...
namespace DOpE
{
  namespace Networks
  {
    ...
    class MethodOfLines_Network_SpaceTimeHandler : public SpaceTimeHandler<FE,
      DH, dealii::BlockSparsityPattern, VECTOR, dopedim, dealdim>
    {
    public:
      /**
       ...
       */
      MethodOfLines_Network_SpaceTimeHandler (std::vector<
                                              dealii::Triangulation<
                                              dealdim> *> &triangulation,
        ...
        : SpaceTimeHandler<FE, DH, dealii::BlockSparsityPattern, VECTOR,
          dopedim, dealdim> (type),
          ...
          network_ (network)
      {
        if (triangulation.size () != GetNPipes ())
          {
            throw DOpEException (
              "Mismatch between given triangulations and number of pipes.",
              "MethodOfLines_Network_SpaceTimeHandler::MethodOfLines_Network_SpaceTimeHandler");
          }

        if (dealdim != 1)
          {
            throw DOpEException ("Not implemented for dealdim != 1",
                                 "MethodOfLines_Network_SpaceTimeHandler::MethodOfLines_Network_SpaceTimeHandler");
          }
        if (dopedim != 0)
          {
            throw DOpEException ("Not implemented for dopedim != 0",
                                 "MethodOfLines_Network_SpaceTimeHandler::MethodOfLines_Network_SpaceTimeHandler");
          }
        sth_s_.resize (GetNPipes (), NULL);
        for (unsigned int i = 0; i < sth_s_.size (); i++)
          {
            sth_s_[i] = new MethodOfLines_StateSpaceTimeHandler<FE, DH,
            dealii::SparsityPattern, dealii::Vector<double>, dealdim> (
              ...
              const DirichletDescriptor &DD_state)
      {
        ...
        {
          for (unsigned int i = 0; i < control_dofs_per_block_.size ();
               i++)
            {
              control_dofs_per_block_[i] = 0;
            }
          for (unsigned int i = 0; i < control_block_component.size ();
               i++)
            {
        ...
        for (unsigned int i = 0; i < sth_s_.size (); i++)
          {
            sth_s_[i]->ReInit (state_n_blocks, state_block_component,
                               DD_state);
          }
      ...
      GetStateDoFHandler () const
      {
        ...
        abort ();
        //No pipe selected
        throw DOpEException ("No pipe selected",
                             "MethodOfLines_Network_SpaceTimeHandler::GetStateDoFHandler");
      }
      /**
       * Implementation of virtual function in SpaceTimeHandler
       */
      ...
      GetMapping () const
      {
        ...
        abort ();
        throw DOpEException ("No pipe selected",
                             "MethodOfLines_Network_SpaceTimeHandler::GetMapping");
      }
      ...
      GetStateDoFConstraints () const
      {
        ...
        abort ();
        throw DOpEException ("No pipe selected",
                             "MethodOfLines_Network_SpaceTimeHandler::GetStateDoFConstraints");
...
//      return state_dof_constraints_;
      ...
      GetMapDoFToSupportPoints ()
      {
        ...
        abort ();
        throw DOpEException ("No pipe selected",
                             "MethodOfLines_Network_SpaceTimeHandler::GetMapDoFToSupportPoints");
      }
      ...
      ComputeStateSparsityPattern (dealii::BlockSparsityPattern &sparsity) const
      {
                ...
                if (j == i)
                ...
                else
                  {
                    ...
                    sparsity.block (i, j).reinit (
                      sth_s_[i]->GetStateNDoFs (),
                      sth_s_[j]->GetStateNDoFs (), 0);
                  }
              }
          }
        ...
        for (unsigned int i = 0; i < n_pipes; i++)
          {
            //Anzahl der Zeilen
            std::vector<unsigned int> row_lenght (
              sth_s_[i]->GetStateNDoFs (), 0);
            //Find-coupling lines
            ...
            //Reinit-size
            sparsity.block (i, n_pipes).reinit (sth_s_[i]->GetStateNDoFs (),
                                                2 * n_pipes * n_comp, row_lenght);
                ...
                for (unsigned int c = 0; c < n_comp; c++)
                  {
                    ...
                    //Left Values
                    sparsity.block (i, n_pipes).add (left_vals[i][n],
                                                     i * n_comp + c);
                    //Right Values
                    sparsity.block (i, n_pipes).add (right_vals[i][n],
                                                     n_pipes * n_comp + i * n_comp + c);
                    //For symmetry
                    sparsity.block (i, n_pipes).add (right_vals[i][n],
                                                     i * n_comp + c);
                  }
        ...
        for (unsigned int i = 0; i < n_pipes; i++)
          {
            ...
            //Reinit-size
            sparsity.block (n_pipes, i).reinit (2 * n_pipes * n_comp,
                                                sth_s_[i]->GetStateNDoFs (), row_lenght);
            //Add couplings (Left-Pipes are stored in [0,n_pipes*n_comp) and right in [n_pipes*n_comp, 2*n_pipes*n_comp])
            //For the i-th pipe the offset for the first component-flux is i*n_comp
            for (unsigned int n = 0; n < left_vals[i].size (); n++)
              {
                for (unsigned int c = 0; c < n_comp; c++)
                  {
                    //Left Values
                    sparsity.block (n_pipes, i).add (i * n_comp + n,
                                                     left_vals[i][c]);
                    //Right Values
                    sparsity.block (n_pipes, i).add (i * n_comp + n,
                                                     right_vals[i][c]);
                    //Symmetrie with final collumn
                    sparsity.block (n_pipes, i).add (
                      n_pipes * n_comp + i * n_comp + n,
                      right_vals[i][c]);
                  }
              }
        ...
        GetNetwork ().GetFluxSparsityPattern (
          sparsity.block (n_pipes, n_pipes));
                                ...
                                VECTOR &new_values) const
      {
        ...
        for (unsigned int i = 0; i < sth_s_.size (); i++)
          {
            sth_s_[i]->SpatialMeshTransferState (old_values.block (i),
                                                 new_values.block (i));
          }
        if (new_values.block (sth_s_.size ()).size () == old_values.block (
              sth_s_.size ()).size ())
          {
            new_values.block (sth_s_.size ()) = old_values.block (
                                                  sth_s_.size ());
          }
        else
          {
      ...
      MethodOfLines_StateSpaceTimeHandler<FE, DH, dealii::SparsityPattern,
                                          dealii::Vector<double>, dealdim> *
                                          GetPipeSTH ()
      {
        if (selected_pipe_ < sth_s_.size ())
        ...
        //No pipe selected
        throw DOpEException ("No pipe selected",
                             "MethodOfLines_Network_SpaceTimeHandler::GetPipeSTH");
      }

      const std::vector<std::vector<unsigned int> > &
      GetPipeToLeftDoF () const
      {
        return pipe_to_leftdofs_;
      }
      const std::vector<std::vector<unsigned int> > &
      GetPipeToRightDoF () const
      {
    ...
    private:
        ...
        for (unsigned int i = 0; i < sth_s_.size (); i++)
          {
            ...
            assert(
              sth_s_[i]->GetStateDoFsPerBlock ()[0] == sth_s_[i]->GetStateNDoFs ());
            state_dofs_per_block_[i] = sth_s_[i]->GetStateNDoFs ();
            sum += sth_s_[i]->GetStateNDoFs ();
          }
        state_dofs_per_block_[sth_s_.size ()] = 2
                                                * GetNPipes () * GetFESystem ("state").n_components ();
      ...
      InitializePipeBoundaryMapping ()
      {
        ...
        pipe_to_leftdofs_.resize (GetNPipes (),
                                  std::vector<unsigned int> (
                                    GetFESystem ("state").n_components (), 0));
        pipe_to_rightdofs_.resize (GetNPipes (),
                                   std::vector<unsigned int> (
                                     GetFESystem ("state").n_components (), 0));
        ...
        left.insert (0);
        right.insert (1);
        dealii::ComponentMask comp_mask (
          GetFESystem ("state").n_components (), true);
        for (unsigned int i = 0; i < GetNPipes (); i++)
          {
            std::vector<bool> selected_left;
            DoFTools::extract_dofs_with_support_on_boundary (
              sth_s_[i]->GetStateDoFHandler ().GetDEALDoFHandler (),
      ...
      std::vector<
      MethodOfLines_StateSpaceTimeHandler<FE, DH,
                                          dealii::SparsityPattern, dealii::Vector<double>, dealdim> *> sth_s_;
    ...
    template <template <int, int> class FE, template <int, int> class DH,
              typename VECTOR, int dopedim, int dealdim>
    void
    MethodOfLines_Network_SpaceTimeHandler<FE, DH, VECTOR, dopedim, dealdim>::ComputeControlSparsityPattern (dealii::BlockSparsityPattern &/*sparsity*/) const
    {
      abort ();
    }
    ...
    MethodOfLines_Network_SpaceTimeHandler<dealii::FESystem,
                                           dealii::DoFHandler, dealii::BlockVector<double>, dope_dimension,
                                           deal_II_dimension>::ComputeControlSparsityPattern (dealii::BlockSparsityPattern &/*sparsity*/) const
    {
      abort ();
    }
