...
namespace DOpE
{
  ...
  /*******************************************************/
  template <typename VECTOR>
  DOpEOutputHandler<VECTOR>::DOpEOutputHandler (ReducedProblemInterface_Base<
                                                VECTOR> *SI,
                                                ParameterReader &param_reader)
  {
    ...
    if (!SI)
      {
        std::cerr
            << "Attention: DOpEOutputHandler is configured without a ReducedProblem, hence no vectors can be written!"
            << std::endl;
        Solver_ = NULL;
      }
    else
    ...
    functional_number_precision_ = param_reader.get_integer (
                                     "functional_number_precision");
  ...
  template <typename VECTOR>
  void
  DOpEOutputHandler<VECTOR>::ReInit ()
  {
  ...
  template <typename VECTOR>
  void
  DOpEOutputHandler<VECTOR>::SetIterationNumber (unsigned int iteration,
                                                 std::string type)
  {
    if (AllowIteration (type))
      {
        std::map<std::string, unsigned int>::const_iterator pos =
          iteration_type_pos_.find (type);
        if (pos == iteration_type_pos_.end ())
          {
            log_ << "LOG: Allowing IterationCounter `" << type
                 << "' for filenames!" << std::endl;
  ...
  template <typename VECTOR>
  void
  DOpEOutputHandler<VECTOR>::ParseString (const std::string tmp,
  ...
  template <typename VECTOR>
  bool
  DOpEOutputHandler<VECTOR>::AllowWrite (std::string name)
  {
    if (disallow_all_)
      {
        if (debug_)
          {
            log_ << "DEBUG: Deny write of `" << name
                 << "'! Since all output is supressed!" << std::endl;
          }
        return false;
      }
        ...
        if (name.find (never_write_list[i]) != std::string::npos)
          {
            if (debug_)
              {
                log_ << "DEBUG: Deny write of `" << name
                     << "'! It containes the substring "
                     << never_write_list[i] << std::endl;
              }
            return false;
          }
      }
  ...
  template <typename VECTOR>
  bool
  DOpEOutputHandler<VECTOR>::AllowIteration (std::string name)
  {
        ...
        if (name.find (ignore_iterations[i]) != std::string::npos)
          {
            if (debug_)
              {
                log_ << "DEBUG: Deny Iteration counter `" << name
                     << "'! It containes the substring "
                     << ignore_iterations[i] << std::endl;
              }
            return false;
          }
      }
  ...
  DOpEOutputHandler<VECTOR>::ReorderAndInsert (std::string type)
  {
  ...
  template <typename VECTOR>
  void
  DOpEOutputHandler<VECTOR>::WriteError (std::string msg)
  {
    if (disallow_all_)
      {
        if (debug_)
          {
            log_
                << "DEBUG: Output of Error was suppresed since all output is supressed!"
                << std::endl;
          }
      }
    else
  ...
  template <typename VECTOR>
  void
  DOpEOutputHandler<VECTOR>::WriteAux (std::string msg,
                                       std::string file,
                                       bool append)
  {
  ...
  template <typename VECTOR>
  void
  DOpEOutputHandler<VECTOR>::Write (std::string msg,
                                    int priority,
        ...
        if (priority < printlevel_ || printlevel_ < 0)
          {
            if (debug_)
              {
                log_ << "DEBUG: Write with priority " << priority
                     << " allowed." << std::endl;
              }
          ...
          {
            log_ << "DEBUG: Write because priority " << priority
                 << " is too small for printing at level " << printlevel_
                 << std::endl;
            {
              log_ << "D\t" << msg << std::endl;
            }
  ...
  template <typename VECTOR>
  void
  DOpEOutputHandler<VECTOR>::Write (std::stringstream &msg,
                                    int priority,
                                    unsigned int pre_newlines,
                                    unsigned int post_newlines)
  {
  ...
  template <typename VECTOR>
  void
  DOpEOutputHandler<VECTOR>::Write (const VECTOR &q,
                                    std::string name,
                                    std::string dof_type)
  {
    if (AllowWrite (name))
  ...
  template <typename VECTOR>
  void
  DOpEOutputHandler<VECTOR>::Write (const ControlVector<VECTOR> &q,
                                    std::string name,
                                    std::string dof_type)
  {
    if (AllowWrite (name))
  ...
  template <typename VECTOR>
  void
  DOpEOutputHandler<VECTOR>::Write (const std::vector<double> &q,
                                    std::string name,
                                    std::string dof_type)
  {
    if (AllowWrite (name))
  ...
  template <typename VECTOR>
  std::string
  DOpEOutputHandler<VECTOR>::ZeroTolerance (double value,
                                            double reference_value)
  {
    ...
    else if (std::abs (value)
             <= std::abs (reference_value) * user_eps_machine_)
      ret << "< " << std::setfill (' ') << std::setw (number_precision_ + 6)
          << std::scientific << user_eps_machine_;
    else
      ret << " " << std::setfill (' ') << std::setw (number_precision_ + 7)
  ...
  template <typename VECTOR>
  void
  DOpEOutputHandler<VECTOR>::InitNewtonOut (std::stringstream &msg)
  {
  ...
  template <typename VECTOR>
  void
  DOpEOutputHandler<VECTOR>::InitOut (std::stringstream &msg)
  {
  ...
  template <typename VECTOR>
  std::string
  DOpEOutputHandler<VECTOR>::GetPostIndex ()
  {
    std::stringstream ret;
    for (unsigned int n = 0; n < iteration_number_.size (); n++)
      {
        ret << "." << std::setfill ('0') << std::setw (5)
            << iteration_number_[n];
      }
    return ret.str ();
  }
  ...
  template <typename VECTOR>
  void
  DOpEOutputHandler<VECTOR>::StartSaveCTypeOutputToLog ()
  {
    if (log_.good ())
      {
        log_.close ();
  ...
  template <typename VECTOR>
  void
  DOpEOutputHandler<VECTOR>::StopSaveCTypeOutputToLog ()
  {
    fflush (stdout);
  ...
  template <typename VECTOR>
  std::string
  DOpEOutputHandler<VECTOR>::GetResultsDir () const
  {
  ...
  template <typename VECTOR>
  void
  DOpEOutputHandler<VECTOR>::PrintCopyrightNotice ()
  {
    std::stringstream out;

    out << "DOpElib Copyright (C) 2012 - " << DOpE::VERSION::year
        << " DOpElib authors" << std::endl;
    out << "This program comes with ABSOLUTELY NO WARRANTY." << std::endl;
    out << "For License details read LICENSE.TXT distributed with this software!"
        << std::endl;
    out << std::endl;
    out << "This is DOpElib Version: " << DOpE::VERSION::major << "."
        << DOpE::VERSION::minor;
    out << "." << DOpE::VERSION::fix << " " << DOpE::VERSION::postfix
        << std::endl;
    out << "\tStatus as of: " << std::setfill ('0') << std::setw (2)
        << DOpE::VERSION::day;
