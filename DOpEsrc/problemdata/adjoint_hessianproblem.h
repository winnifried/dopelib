...
namespace DOpE
{
  ...
  {
  public:
    Adjoint_HessianProblem (OPTPROBLEM &OP,
                            PDE &pde)
      : pde_ (pde),
        opt_problem_ (OP)
    {
                     ...
                     double scale)
    {
      if (opt_problem_.GetFunctional ()->NeedTime ())
        {
          if (opt_problem_.GetFunctional ()->GetType ().find ("timelocal") != std::string::npos)
            {
              if (opt_problem_.GetFunctional ()->GetType ().find (
                    "domain")
                  != std::string::npos)
                {
                  opt_problem_.GetFunctional ()->ElementValue_UU (edc,
                                                                  local_vector, scale);
                  opt_problem_.GetFunctional ()->ElementValue_QU (edc,
                                                                  local_vector, scale);
                }
            }
        }
     ...
     */
    void
    Init_PointRhs (const std::map<std::string, const dealii::Vector<double>*> &param_values,
              ...
              if (opt_problem_.GetFunctional ()->GetType ().find ("point") != std::string::npos)
                {
                  opt_problem_.GetFunctional ()->PointValue_UU (
                    param_values, domain_values, rhs_vector, scale);
                }
            }
        }
    }
...
//
    ...
    template <typename FDC>
    inline void
    BoundaryEquation_BV (const FDC & /*fdc*/,
                         dealii::Vector<double> &/*local_vector*/,
                         double /*scale*/,
                         double /*scale_ico*/)
    {
    ...
    template <typename FDC>
    inline void
    BoundaryMatrix_BV (const FDC & /*fdc*/,
  ...
  template <typename EDC>
  void
  Adjoint_HessianProblem<OPTPROBLEM, PDE, DD, SPARSITYPATTERN, VECTOR, dim>::ElementEquation (const EDC &edc,
      dealii::Vector<
      double> &local_vector,
      double scale,
      double scale_ico)
  {
    pde_.ElementEquation_UTT (edc, local_vector, scale * interval_length_,
  ...
  template <typename EDC>
  void
  Adjoint_HessianProblem<OPTPROBLEM, PDE, DD, SPARSITYPATTERN, VECTOR, dim>::ElementTimeEquation (const EDC &edc,
      dealii::Vector<
      double> &local_vector,
      double scale)
  {
  ...
  template <typename EDC>
  void
  Adjoint_HessianProblem<OPTPROBLEM, PDE, DD, SPARSITYPATTERN, VECTOR, dim>::ElementTimeEquationExplicit (const EDC &edc,
      dealii::Vector<
      double> &local_vector,
      double scale)
  {
    pde_.ElementTimeEquationExplicit_UTT (edc, local_vector, scale);
  }
  ...
  template <typename FDC>
  void
  Adjoint_HessianProblem<OPTPROBLEM, PDE, DD, SPARSITYPATTERN, VECTOR, dim>::FaceEquation (const FDC &fdc,
      dealii::Vector<
      double> &local_vector,
      double scale,
      double scale_ico)
  {
    pde_.FaceEquation_UTT (fdc, local_vector, scale * interval_length_,
                           scale_ico * interval_length_);
  }
  ...
  template <typename FDC>
  void
  Adjoint_HessianProblem<OPTPROBLEM, PDE, DD, SPARSITYPATTERN, VECTOR, dim>::InterfaceEquation (const FDC &fdc,
      dealii::Vector<
      double> &local_vector,
      double scale,
      double scale_ico)
  {
    pde_.InterfaceEquation_UTT (fdc, local_vector, scale * interval_length_,
                                scale_ico * interval_length_);
  }
  /******************************************************/
  ...
  template <typename FDC>
  void
  Adjoint_HessianProblem<OPTPROBLEM, PDE, DD, SPARSITYPATTERN, VECTOR, dim>::BoundaryEquation (const FDC &fdc,
      dealii::Vector<
      double> &local_vector,
      double scale,
      double scale_ico)
  {
    pde_.BoundaryEquation_UTT (fdc, local_vector, scale * interval_length_,
                               scale_ico * interval_length_);
  }
  ...
  template <typename EDC>
  void
  Adjoint_HessianProblem<OPTPROBLEM, PDE, DD, SPARSITYPATTERN, VECTOR, dim>::ElementRhs (const EDC &edc,
      dealii::Vector<
      double> &local_vector,
      double scale)
  {
        ...
        if (opt_problem_.GetFunctional ()->GetType ().find ("domain") != std::string::npos)
          {
            ...
            if (opt_problem_.GetFunctional ()->GetType ().find (
                  "timedistributed")
                != std::string::npos
                && opt_problem_.GetFunctional ()->GetType ().find (
                  "timelocal")
                != std::string::npos)
              {
                throw DOpEException (
                  "Conflicting functional types: " + opt_problem_.GetFunctional ()->GetType (),
    ...
    //Equation Values
    scale *= -1;
    pde_.ElementEquation_UU (edc, local_vector, scale * interval_length_,
                             scale * interval_length_);
    pde_.ElementEquation_QU (edc, local_vector, scale * interval_length_,
                             scale * interval_length_);
    //TODO: make some example where this realy matters to check if this is right
    pde_.ElementTimeEquationExplicit_UU (edc, local_vector, scale);
  ...
  template <typename OPTPROBLEM, typename PDE, typename DD,
            typename SPARSITYPATTERN, typename VECTOR, int dim>
  void
  Adjoint_HessianProblem<OPTPROBLEM, PDE, DD, SPARSITYPATTERN, VECTOR, dim>::PointRhs (const std::map<
      std::string,
      const dealii::Vector<
      double>*> &param_values,
      const std::map<
            ...
            if (opt_problem_.GetFunctional ()->GetType ().find (
                ...
                != std::string::npos)
              {
                opt_problem_.GetFunctional ()->PointValue_UU (
                  ...
                  param_values, domain_values, rhs_vector,
                  scale * interval_length_);
  ...
  template <typename FDC>
  void
  Adjoint_HessianProblem<OPTPROBLEM, PDE, DD, SPARSITYPATTERN, VECTOR, dim>::FaceRhs (const FDC &fdc,
      dealii::Vector<
      double> &local_vector,
      double scale)
  {
        ...
        if (opt_problem_.GetFunctional ()->GetType ().find ("face") != std::string::npos)
          {
            if (opt_problem_.GetFunctional ()->GetType ().find (
                ...
                != std::string::npos)
              {
                opt_problem_.GetFunctional ()->FaceValue_QU (fdc,
                                                             local_vector, scale * interval_length_);
                opt_problem_.GetFunctional ()->FaceValue_UU (fdc,
                                                             local_vector, scale * interval_length_);
              }
            else
              {
    ...
    //PDE
    scale *= -1;
    pde_.FaceEquation_UU (fdc, local_vector, scale * interval_length_,
                          scale * interval_length_);
    pde_.FaceEquation_QU (fdc, local_vector, scale * interval_length_,
                          scale * interval_length_);
  ...
  template <typename FDC>
  void
  Adjoint_HessianProblem<OPTPROBLEM, PDE, DD, SPARSITYPATTERN, VECTOR, dim>::BoundaryRhs (const FDC &fdc,
      dealii::Vector<
      double> &local_vector,
      double scale)
  {
        ...
        if (opt_problem_.GetFunctional ()->GetType ().find ("boundary") != std::string::npos)
          {
            if (opt_problem_.GetFunctional ()->GetType ().find (
                ...
                != std::string::npos)
              {
                opt_problem_.GetFunctional ()->BoundaryValue_UU (fdc,
                                                                 local_vector, scale * interval_length_);
                opt_problem_.GetFunctional ()->BoundaryValue_QU (fdc,
                                                                 local_vector, scale * interval_length_);
              }
            else
              {
  ...
  template <typename EDC>
  void
  Adjoint_HessianProblem<OPTPROBLEM, PDE, DD, SPARSITYPATTERN, VECTOR, dim>::ElementMatrix (const EDC &edc,
      dealii::FullMatrix<
      double> &local_entry_matrix,
      double scale,
      double scale_ico)
  {
    pde_.ElementMatrix_T (edc, local_entry_matrix, scale * interval_length_,
                          scale_ico * interval_length_);
  }
  ...
  template <typename EDC>
  void
  Adjoint_HessianProblem<OPTPROBLEM, PDE, DD, SPARSITYPATTERN, VECTOR, dim>::ElementTimeMatrix (const EDC &edc,
      FullMatrix<
      double> &local_entry_matrix)
  {
  ...
  template <typename EDC>
  void
  Adjoint_HessianProblem<OPTPROBLEM, PDE, DD, SPARSITYPATTERN, VECTOR, dim>::ElementTimeMatrixExplicit (const EDC &edc,
      dealii::FullMatrix<
      double> &local_entry_matrix)
  {
  ...
  template <typename FDC>
  void
  Adjoint_HessianProblem<OPTPROBLEM, PDE, DD, SPARSITYPATTERN, VECTOR, dim>::FaceMatrix (const FDC &fdc,
      FullMatrix<
      double> &local_entry_matrix,
      double scale,
      double scale_ico)
  {
    pde_.FaceMatrix_T (fdc, local_entry_matrix, scale * interval_length_,
                       scale_ico * interval_length_);
  }
  ...
  template <typename FDC>
  void
  Adjoint_HessianProblem<OPTPROBLEM, PDE, DD, SPARSITYPATTERN, VECTOR, dim>::InterfaceMatrix (const FDC &fdc,
      FullMatrix<
      double> &local_entry_matrix,
      double scale,
      double scale_ico)
  {
    pde_.InterfaceMatrix_T (fdc, local_entry_matrix,
                            scale * interval_length_, scale_ico * interval_length_);
  }
  ...
  template <typename FDC>
  void
  Adjoint_HessianProblem<OPTPROBLEM, PDE, DD, SPARSITYPATTERN, VECTOR, dim>::BoundaryMatrix (const FDC &fdc,
      FullMatrix<
      double> &local_matrix,
      double scale,
      double scale_ico)
  {
    pde_.BoundaryMatrix_T (fdc, local_matrix, scale * interval_length_,
                           scale_ico * interval_length_);
  }
  ...
  template <typename OPTPROBLEM, typename PDE, typename DD,
            typename SPARSITYPATTERN, typename VECTOR, int dim>
  void
  Adjoint_HessianProblem<OPTPROBLEM, PDE, DD, SPARSITYPATTERN, VECTOR, dim>::SetTime (double time,
      unsigned int time_dof_number,
      const TimeIterator &interval,
      bool initial)
  {
  ...
  template <typename OPTPROBLEM, typename PDE, typename DD,
            typename SPARSITYPATTERN, typename VECTOR, int dim>
  void
  Adjoint_HessianProblem<OPTPROBLEM, PDE, DD, SPARSITYPATTERN, VECTOR, dim>::ComputeSparsityPattern (SPARSITYPATTERN &sparsity) const
  {
    opt_problem_.GetSpaceTimeHandler ()->ComputeStateSparsityPattern (
      sparsity);
  }
...
//
  ...
  {
    return pde_.HasFaces () || opt_problem_.GetFunctional ()->HasFaces ();
  }
  ...
  Adjoint_HessianProblem<OPTPROBLEM, PDE, DD, SPARSITYPATTERN, VECTOR, dim>::GetDirichletCompMask (unsigned int color) const
  {
    ...
    if (comp == dirichlet_colors_.size ())
      {
        ...
        throw DOpEException (s.str (),
                             "Adjoint_HessianProblem::GetDirichletCompMask");
      }
  ...
  template <typename OPTPROBLEM, typename PDE, typename DD,
            typename SPARSITYPATTERN, typename VECTOR, int dim>
  const Function<dim> &
  Adjoint_HessianProblem<OPTPROBLEM, PDE, DD, SPARSITYPATTERN, VECTOR, dim>::GetDirichletValues (unsigned int color,
      const std::map<
      std::string,
      const dealii::Vector<
      double>*> &/*param_values*/,
      const std::map<
      std::string,
      const VECTOR *> &/*domain_values*/) const
  {
  ...
  template <typename ELEMENTITERATOR>
  bool
  Adjoint_HessianProblem<OPTPROBLEM, PDE, DD, SPARSITYPATTERN, VECTOR, dim>::AtInterface (ELEMENTITERATOR &element,
      unsigned int face) const
  {
