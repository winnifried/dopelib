...
namespace DOpE
{
  ...
  {
  public:
    PDE_Adjoint_For_EEProblem (OPTPROBLEM &OP,
                               PDE &pde)
      : pde_ (pde),
        opt_problem_ (OP)
    {
                          ...
                          double /*scale_ico*/)
    {
      throw DOpEException (
        ...
        "PDE_Adjoint_For_EEProblem::Init_ElementEquation");
    }
                     ...
                     double /*scale*/)
    {
      throw DOpEException (
        "This method is not implemented; the problem is tested for stationary problems, only!",
        "PDE_Adjoint_For_EEProblem::Init_ElementRhs");
    }
                   ...
                   double /*scale=1.*/)
    {
      throw DOpEException (
        "This method is not implemented; the problem is tested for stationary problems, only!",
        "PDE_Adjoint_For_EEProblem::Init_PointRhs");
                        ...
                        double /*scale_ico*/)
    {
      throw DOpEException (
        "This method is not implemented; the problem is tested for stationary problems, only!",
        "PDE_Adjoint_For_EEProblem::Init_ElementMatrix");
    }
...
//
  ...
  template <typename OPTPROBLEM, typename PDE, typename DD,
            typename SPARSITYPATTERN, typename VECTOR, int dim>
  void
  PDE_Adjoint_For_EEProblem<OPTPROBLEM, PDE, DD, SPARSITYPATTERN, VECTOR, dim>::PointRhs (const std::map<
      std::string,
      const dealii::Vector<
      double>*> &param_values,
      const std::map<
  ...
  template <typename OPTPROBLEM, typename PDE, typename DD,
            typename SPARSITYPATTERN, typename VECTOR, int dim>
  void
  PDE_Adjoint_For_EEProblem<OPTPROBLEM, PDE, DD, SPARSITYPATTERN, VECTOR, dim>::SetTime (double time,
      unsigned int time_dof_number,
      const TimeIterator &interval,
      bool initial)
  {
  ...
  template <typename OPTPROBLEM, typename PDE, typename DD,
            typename SPARSITYPATTERN, typename VECTOR, int dim>
  void
  PDE_Adjoint_For_EEProblem<OPTPROBLEM, PDE, DD, SPARSITYPATTERN, VECTOR, dim>::ComputeSparsityPattern (SPARSITYPATTERN &sparsity) const
  {
...
//   }
  ...
  PDE_Adjoint_For_EEProblem<OPTPROBLEM, PDE, DD, SPARSITYPATTERN, VECTOR, dim>::GetDirichletCompMask (unsigned int color) const
  {
    ...
    if (comp == dirichlet_colors_.size ())
      {
        ...
        throw DOpEException (s.str (),
                             "PDE_Adjoint_For_EEProblem::GetDirichletCompMask");
      }
  ...
  template <typename OPTPROBLEM, typename PDE, typename DD,
            typename SPARSITYPATTERN, typename VECTOR, int dim>
  const Function<dim> &
  PDE_Adjoint_For_EEProblem<OPTPROBLEM, PDE, DD, SPARSITYPATTERN, VECTOR, dim>::GetDirichletValues (unsigned int color,
      const std::map<
      std::string,
      const dealii::Vector<
      double>*> &/*param_values*/,
