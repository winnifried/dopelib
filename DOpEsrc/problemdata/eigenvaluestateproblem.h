...
#ifndef EIGENVALUESTATE_PROBLEM_H_
#define EIGENVALUESTATE_PROBLEM_H_
...
namespace DOpE
{
  ...
  class EigenvalueStateProblem
  {
  public:
    EigenvalueStateProblem(OPTPROBLEM &OP, PDE &pde) :
    ...
    InitialNewtonProblem<OPTPROBLEM,EigenvalueStateProblem<OPTPROBLEM,PDE,DD,SPARSITYPATTERN,VECTOR,dim>, VECTOR, dim> &
      ...
      if (NeedInitialState())
        {
          if ( initial_newton_problem_ == NULL)
            {
              initial_newton_problem_ = new InitialNewtonProblem<OPTPROBLEM,EigenvalueStateProblem<OPTPROBLEM,PDE,DD,SPARSITYPATTERN,VECTOR,dim>, VECTOR, dim>
              (*this);
            }
        ...
        {
          throw DOpEException("GetNewtonInitialProblem has no meaning in case NeedInitialState() == false","EigenvalueProblem::GetNewtonInitialProblem");
        }
    }
    ...
    */
    ...
    void Init_ElementMassEquation(const EDC &edc,
                                  ...
                                  double scale_ico)
    {
      pde_.Init_ElementMassEquation(edc, local_vector, scale, scale_ico);
    }
    ...
    */
    ...
    template<typename EDC>
    void Init_ElementMassMatrix(const EDC &edc,
                                dealii::FullMatrix<double> &local_entry_matrix, double scale,
                                double scale_ico)
    {
      pde_.Init_ElementMassMatrix(edc, local_entry_matrix, scale, scale_ico);
    }
...
//              unsigned int n_levels) const;
  ...
  private:
    ...
    InitialNewtonProblem<OPTPROBLEM,EigenvalueStateProblem<OPTPROBLEM,PDE,DD,SPARSITYPATTERN,VECTOR,dim>, VECTOR, dim> *initial_newton_problem_;
  ...
  template<typename EDC>
  void
  EigenvalueStateProblem<OPTPROBLEM, PDE, DD, SPARSITYPATTERN, VECTOR,
  ...
  template<typename OPTPROBLEM, typename PDE, typename DD,
           typename SPARSITYPATTERN, typename VECTOR, int dim>
  void
  EigenvalueStateProblem<OPTPROBLEM, PDE, DD, SPARSITYPATTERN, VECTOR, dim>::PointRhs(
    const std::map<std::string, const dealii::Vector<double>*> &/*param_values*/,
    const std::map<std::string, const VECTOR *> &/*domain_values*/,
    VECTOR & /*rhs_vector*/, double /*scale*/)
  {
  ...
  EigenvalueStateProblem<OPTPROBLEM, PDE, DD, SPARSITYPATTERN, VECTOR,
                         dim>::ElementMassMatrix(const EDC &edc,
                                                 ...
                                                 double scale_ico)
  {
    pde_.ElementMassMatrix(edc, local_entry_matrix, scale*interval_length_, scale_ico*interval_length_);
  }
  ...
  EigenvalueStateProblem<OPTPROBLEM, PDE, DD, SPARSITYPATTERN, VECTOR,
                         dim>::ElementTimeMatrix(const EDC &edc,
                                                 ...
                                                 double scale_ico)
  {
    pde_.ElementTimeMatrix(edc, local_entry_matrix, scale*interval_length_, scale_ico*interval_length_);
  }
  ...
  template<typename OPTPROBLEM, typename PDE, typename DD,
           typename SPARSITYPATTERN, typename VECTOR, int dim>
  UpdateFlags
  EigenvalueStateProblem<OPTPROBLEM, PDE, DD, SPARSITYPATTERN, VECTOR, dim>::GetUpdateFlags() const
  {
    UpdateFlags r;
  ...
  template<typename OPTPROBLEM, typename PDE, typename DD,
           typename SPARSITYPATTERN, typename VECTOR, int dim>
  UpdateFlags
  EigenvalueStateProblem<OPTPROBLEM, PDE, DD, SPARSITYPATTERN, VECTOR, dim>::GetFaceUpdateFlags() const
  {
    UpdateFlags r;
  ...
  template<typename OPTPROBLEM, typename PDE, typename DD,
           typename SPARSITYPATTERN, typename VECTOR, int dim>
  void
  EigenvalueStateProblem<OPTPROBLEM, PDE, DD, SPARSITYPATTERN, VECTOR, dim>::SetTime(
    double time,
  ...
  template<typename OPTPROBLEM, typename PDE, typename DD,
           typename SPARSITYPATTERN, typename VECTOR, int dim>
  void
  EigenvalueStateProblem<OPTPROBLEM, PDE, DD, SPARSITYPATTERN, VECTOR, dim>::ComputeSparsityPattern(
...
//
  ...
  template<typename OPTPROBLEM, typename PDE, typename DD,
           typename SPARSITYPATTERN, typename VECTOR, int dim>
  bool
  EigenvalueStateProblem<OPTPROBLEM, PDE, DD, SPARSITYPATTERN, VECTOR, dim>::HasFaces() const
  {
  ...
  template<typename OPTPROBLEM, typename PDE, typename DD,
           typename SPARSITYPATTERN, typename VECTOR, int dim>
  bool
  EigenvalueStateProblem<OPTPROBLEM, PDE, DD, SPARSITYPATTERN, VECTOR, dim>::HasPoints() const
  {
  ...
  template<typename OPTPROBLEM, typename PDE, typename DD,
           typename SPARSITYPATTERN, typename VECTOR, int dim>
  bool
  EigenvalueStateProblem<OPTPROBLEM, PDE, DD, SPARSITYPATTERN, VECTOR, dim>::HasInterfaces() const
  {
  ...
  template<typename OPTPROBLEM, typename PDE, typename DD,
           typename SPARSITYPATTERN, typename VECTOR, int dim>
  bool
  EigenvalueStateProblem<OPTPROBLEM, PDE, DD, SPARSITYPATTERN, VECTOR, dim>::HasVertices() const
  {
  ...
  EigenvalueStateProblem<OPTPROBLEM, PDE, DD, SPARSITYPATTERN, VECTOR, dim>::GetDirichletColors() const
  {
  ...
  EigenvalueStateProblem<OPTPROBLEM, PDE, DD, SPARSITYPATTERN, VECTOR, dim>::GetDirichletCompMask(
  ...
  template<typename OPTPROBLEM, typename PDE, typename DD,
           typename SPARSITYPATTERN, typename VECTOR, int dim>
  const Function<dim> &
  EigenvalueStateProblem<OPTPROBLEM, PDE, DD, SPARSITYPATTERN, VECTOR, dim>::GetDirichletValues(
    unsigned int color,
    const std::map<std::string, const dealii::Vector<double>*> &param_values,
    const std::map<std::string, const VECTOR *> &domain_values) const
  {
  ...
  template<typename OPTPROBLEM, typename PDE, typename DD,
           typename SPARSITYPATTERN, typename VECTOR, int dim>
  const std::vector<unsigned int> &
  EigenvalueStateProblem<OPTPROBLEM, PDE, DD, SPARSITYPATTERN, VECTOR, dim>::GetBoundaryEquationColors() const
  {
  ...
  EigenvalueStateProblem<OPTPROBLEM, PDE, DD, SPARSITYPATTERN, VECTOR, dim>::GetDoFConstraints() const
  {
  ...
  EigenvalueProblem<OPTPROBLEM, PDE, DD, SPARSITYPATTERN, VECTOR, dim>::GetDoFConstraints() const
  {
    return opt_problem_.GetSpaceTimeHandler()->GetStateDoFConstraints();
  }
#endif
  ...
  template<typename OPTPROBLEM, typename PDE, typename DD,
           typename SPARSITYPATTERN, typename VECTOR, int dim>
  const dealii::AffineConstraints<double> &
  EigenvalueStateProblem<OPTPROBLEM, PDE, DD, SPARSITYPATTERN, VECTOR, dim>::GetHNConstraints() const
  {
  ...
  template<typename OPTPROBLEM, typename PDE, typename DD,
           typename SPARSITYPATTERN, typename VECTOR, int dim>
  const dealii::ConstraintMatrix &
  EigenvalueProblem<OPTPROBLEM, PDE, DD, SPARSITYPATTERN, VECTOR, dim>::GetHNConstraints() const
  {
    return opt_problem_.GetSpaceTimeHandler()->GetStateHNConstraints();
  }
#endif
  ...
  EigenvalueStateProblem<OPTPROBLEM, PDE, DD, SPARSITYPATTERN, VECTOR, dim>::GetInitialValues() const
  {
  ...
  template<typename OPTPROBLEM, typename PDE, typename DD,
           typename SPARSITYPATTERN, typename VECTOR, int dim>
  template<typename ELEMENTITERATOR>
  bool EigenvalueStateProblem<OPTPROBLEM, PDE, DD, SPARSITYPATTERN, VECTOR, dim>
