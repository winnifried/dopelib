          ...
          if (opt_problem_.GetFunctional ()->GetType ().find ("timelocal") != std::string::npos)
            {
              if (opt_problem_.GetFunctional ()->GetType ().find (
                  ...
                  != std::string::npos)
                {
                  opt_problem_.GetFunctional ()->ElementValue_U (edc,
                                                                 local_vector, scale);
                }
            }
        }
          ...
          if (opt_problem_.GetFunctional ()->GetType ().find ("timelocal") != std::string::npos)
            {
              if (opt_problem_.GetFunctional ()->GetType ().find ("point") != std::string::npos)
                {
                  opt_problem_.GetFunctional ()->PointValue_U (param_values,
...
//              unsigned int n_levels) const;
  ...
  template <typename EDC>
  void
  AdjointProblem<OPTPROBLEM, PDE, DD, SPARSITYPATTERN, VECTOR, dim>::ElementEquation (const EDC &edc,
      dealii::Vector<
      double> &local_vector,
      double scale,
      double scale_ico)
  {
    pde_.ElementEquation_U (edc, local_vector, scale * interval_length_,
  ...
  template <typename EDC>
  void
  AdjointProblem<OPTPROBLEM, PDE, DD, SPARSITYPATTERN, VECTOR, dim>::ElementTimeEquation (const EDC &edc,
      dealii::Vector<
      double> &local_vector,
      double scale)
  {
  ...
  template <typename EDC>
  void
  AdjointProblem<OPTPROBLEM, PDE, DD, SPARSITYPATTERN, VECTOR, dim>::ElementTimeEquationExplicit (const EDC &edc,
      dealii::Vector<
      double> &local_vector,
      double scale)
  {
    pde_.ElementTimeEquationExplicit_U (edc, local_vector, scale);
  }
  ...
  template <typename FDC>
  void
  AdjointProblem<OPTPROBLEM, PDE, DD, SPARSITYPATTERN, VECTOR, dim>::FaceEquation (const FDC &fdc,
      dealii::Vector<
      double> &local_vector,
      double scale,
      double scale_ico)
  {
    pde_.FaceEquation_U (fdc, local_vector, scale * interval_length_,
                         scale_ico * interval_length_);
  }
  ...
  template <typename FDC>
  void
  AdjointProblem<OPTPROBLEM, PDE, DD, SPARSITYPATTERN, VECTOR, dim>::InterfaceEquation (const FDC &fdc,
      dealii::Vector<
      double> &local_vector,
      double scale,
      double scale_ico)
  {
    pde_.InterfaceEquation_U (fdc, local_vector, scale * interval_length_,
                              scale_ico * interval_length_);
  }
  /******************************************************/
  ...
  template <typename FDC>
  void
  AdjointProblem<OPTPROBLEM, PDE, DD, SPARSITYPATTERN, VECTOR, dim>::BoundaryEquation (const FDC &fdc,
      dealii::Vector<
      double> &local_vector,
      double scale,
      double scale_ico)
  {
    pde_.BoundaryEquation_U (fdc, local_vector, scale * interval_length_,
                             scale_ico * interval_length_);
  }
  ...
  template <typename EDC>
  void
  AdjointProblem<OPTPROBLEM, PDE, DD, SPARSITYPATTERN, VECTOR, dim>::ElementRhs (const EDC &edc,
      dealii::Vector<
      double> &local_vector,
      double scale)
  {
  ...
  template <typename OPTPROBLEM, typename PDE, typename DD,
            typename SPARSITYPATTERN, typename VECTOR, int dim>
  void
  AdjointProblem<OPTPROBLEM, PDE, DD, SPARSITYPATTERN, VECTOR, dim>::PointRhs (const std::map<
      std::string,
      const dealii::Vector<
      double>*> &param_values,
      const std::map<
  ...
  template <typename FDC>
  void
  AdjointProblem<OPTPROBLEM, PDE, DD, SPARSITYPATTERN, VECTOR, dim>::FaceRhs (const FDC &fdc,
      dealii::Vector<
      double> &local_vector,
      double scale)
  {
        ...
        if (opt_problem_.GetFunctional ()->GetType ().find ("face") != std::string::npos)
          {
            if (opt_problem_.GetFunctional ()->GetType ().find (
                ...
                != std::string::npos)
              {
                opt_problem_.GetFunctional ()->FaceValue_U (fdc,
                                                            local_vector, scale * interval_length_);
              }
            else
              {
  ...
  template <typename FDC>
  void
  AdjointProblem<OPTPROBLEM, PDE, DD, SPARSITYPATTERN, VECTOR, dim>::BoundaryRhs (const FDC &fdc,
      dealii::Vector<
      double> &local_vector,
      double scale)
  {
        ...
        if (opt_problem_.GetFunctional ()->GetType ().find ("boundary") != std::string::npos)
          {
            if (opt_problem_.GetFunctional ()->GetType ().find (
                ...
                != std::string::npos)
              {
                opt_problem_.GetFunctional ()->BoundaryValue_U (fdc,
                                                                local_vector, scale * interval_length_);
              }
            else
              {
  ...
  template <typename EDC>
  void
  AdjointProblem<OPTPROBLEM, PDE, DD, SPARSITYPATTERN, VECTOR, dim>::ElementMatrix (const EDC &edc,
      dealii::FullMatrix<
  ...
  template <typename EDC>
  void
  AdjointProblem<OPTPROBLEM, PDE, DD, SPARSITYPATTERN, VECTOR, dim>::ElementTimeMatrix (const EDC &edc,
      FullMatrix<
      double> &local_entry_matrix)
  {
  ...
  template <typename EDC>
  void
  AdjointProblem<OPTPROBLEM, PDE, DD, SPARSITYPATTERN, VECTOR, dim>::ElementTimeMatrixExplicit (const EDC &edc,
      dealii::FullMatrix<
      double> &local_entry_matrix)
  {
  ...
  template <typename FDC>
  void
  AdjointProblem<OPTPROBLEM, PDE, DD, SPARSITYPATTERN, VECTOR, dim>::FaceMatrix (const FDC &fdc,
      FullMatrix<
      double> &local_entry_matrix,
      double scale,
      double scale_ico)
  ...
  template <typename FDC>
  void
  AdjointProblem<OPTPROBLEM, PDE, DD, SPARSITYPATTERN, VECTOR, dim>::InterfaceMatrix (const FDC &fdc,
      FullMatrix<
      double> &local_entry_matrix,
      double scale,
      double scale_ico)
  ...
  template <typename FDC>
  void
  AdjointProblem<OPTPROBLEM, PDE, DD, SPARSITYPATTERN, VECTOR, dim>::BoundaryMatrix (const FDC &fdc,
      FullMatrix<
      double> &local_matrix,
      double scale,
      double scale_ico)
  ...
  template <typename OPTPROBLEM, typename PDE, typename DD,
            typename SPARSITYPATTERN, typename VECTOR, int dim>
  void
  AdjointProblem<OPTPROBLEM, PDE, DD, SPARSITYPATTERN, VECTOR, dim>::SetTime (double time,
      unsigned int time_dof_number,
      const TimeIterator &interval,
      bool initial)
  {
  ...
  template <typename OPTPROBLEM, typename PDE, typename DD,
            typename SPARSITYPATTERN, typename VECTOR, int dim>
  void
  AdjointProblem<OPTPROBLEM, PDE, DD, SPARSITYPATTERN, VECTOR, dim>::ComputeSparsityPattern (SPARSITYPATTERN &sparsity) const
  {
...
//    }
  ...
  AdjointProblem<OPTPROBLEM, PDE, DD, SPARSITYPATTERN, VECTOR, dim>::GetDirichletCompMask (unsigned int color) const
  {
    ...
    if (comp == dirichlet_colors_.size ())
      {
        ...
        throw DOpEException (s.str (),
                             "AdjointProblem::GetDirichletCompMask");
      }
  ...
  template <typename OPTPROBLEM, typename PDE, typename DD,
            typename SPARSITYPATTERN, typename VECTOR, int dim>
  const Function<dim> &
  AdjointProblem<OPTPROBLEM, PDE, DD, SPARSITYPATTERN, VECTOR, dim>::GetDirichletValues (unsigned int color,
      const std::map<
      std::string,
      const dealii::Vector<
      double>*> &/*param_values*/,
  ...
  template <typename ELEMENTITERATOR>
  bool
  AdjointProblem<OPTPROBLEM, PDE, DD, SPARSITYPATTERN, VECTOR, dim>::AtInterface (ELEMENTITERATOR &element,
      unsigned int face) const
  {
