...
namespace DOpE
{
  ...
  {
  public:
    StateProblem (OPTPROBLEM &OP,
                  PDE &pde)
      : pde_ (pde),
        opt_problem_ (OP),
        initial_newton_problem_ (NULL)
    {
    ...
    InitialNewtonProblem<OPTPROBLEM,
                         StateProblem<OPTPROBLEM, PDE, DD, SPARSITYPATTERN, VECTOR, dim>,
                         VECTOR, dim> &
      ...
      if (NeedInitialState ())
        {
          if (initial_newton_problem_ == NULL)
            {
              initial_newton_problem_ = new InitialNewtonProblem<OPTPROBLEM,
        ...
        {
          throw DOpEException (
            "GetNewtonInitialProblem has no meaning in case NeedInitialState() == false",
            "StateProblem::GetNewtonInitialProblem");
        }
    }
    ...
    Init_ElementRhs (const EDC &edc,
                     ...
                     double scale)
    {
      pde_.Init_ElementRhs (&GetInitialValues (), edc, local_vector,
                            scale);
    }
    ...
    inline void
    Init_OutflowValues (const FDC &fdc,
                        std::vector<bool> &present_in_outflow,
                        dealii::Vector<double> &local_vector,
                        double scale,
                        double scale_ico)
    {
      pde_.Init_OutflowValues (fdc, present_in_outflow, local_vector,
                               scale, scale_ico);
    }
    template <typename FDC>
    inline void
    Init_OutflowMatrix (const FDC &fdc,
                        ...
                        double scale_ico)
    {
      pde_.Init_OutflowMatrix (fdc, present_in_outflow, local_matrix,
                               scale, scale_ico);
    }
    inline void
...
//              unsigned int n_levels) const;
    ...
    BoundaryMatrix_BV (const FDC &fdc,
                       ...
                       double scale_ico)
    {
      pde_.BoundaryMatrix_BV (fdc, present_in_outflow, local_entry_matrix,
                              scale, scale_ico);
    }
    ...
    OutflowValues (const FDC &fdc,
                   ...
                   double scale_ico)
    {
      pde_.OutflowValues (fdc, present_in_outflow, local_vector, scale,
                          scale_ico);
    }
    ...
    OutflowMatrix (const FDC &fdc,
                   ...
                   double scale_ico)
    {
      pde_.OutflowMatrix (fdc, present_in_outflow, local_entry_matrix,
                          scale, scale_ico);
    }
    inline void
  ...
  private:
    ...
    InitialNewtonProblem<OPTPROBLEM,
                         StateProblem<OPTPROBLEM, PDE, DD, SPARSITYPATTERN, VECTOR, dim>,
                         VECTOR, dim> *initial_newton_problem_;

    std::vector<unsigned int> dirichlet_colors_;
    std::vector<std::vector<bool> > dirichlet_comps_;
    std::vector<PrimalDirichletData<DD, VECTOR, dim>*> primal_dirichlet_values_;
  ...
  template <typename EDC>
  void
  StateProblem<OPTPROBLEM, PDE, DD, SPARSITYPATTERN, VECTOR, dim>::ElementEquation (const EDC &edc,
      dealii::Vector<
      double> &local_vector,
      double scale,
      double scale_ico)
  {
    pde_.ElementEquation (edc, local_vector, scale * interval_length_,
  ...
  template <typename EDC>
  void
  StateProblem<OPTPROBLEM, PDE, DD, SPARSITYPATTERN, VECTOR, dim>::ElementTimeEquation (const EDC &edc,
      dealii::Vector<
      double> &local_vector,
      double scale)
  {
  ...
  template <typename EDC>
  void
  StateProblem<OPTPROBLEM, PDE, DD, SPARSITYPATTERN, VECTOR, dim>::ElementTimeEquationExplicit (const EDC &edc,
      dealii::Vector<
      double> &local_vector,
      double scale)
  {
  ...
  template <typename FDC>
  void
  StateProblem<OPTPROBLEM, PDE, DD, SPARSITYPATTERN, VECTOR, dim>::FaceEquation (const FDC &fdc,
      dealii::Vector<
      double> &local_vector,
      double scale,
      double scale_ico)
  {
    pde_.FaceEquation (fdc, local_vector, scale * interval_length_,
                       scale_ico * interval_length_);
  }
  ...
  template <typename FDC>
  void
  StateProblem<OPTPROBLEM, PDE, DD, SPARSITYPATTERN, VECTOR, dim>::InterfaceEquation (const FDC &fdc,
      dealii::Vector<
      double> &local_vector,
      double scale,
      double scale_ico)
  {
    pde_.InterfaceEquation (fdc, local_vector, scale * interval_length_,
                            scale_ico * interval_length_);
  }
  /******************************************************/
  ...
  template <typename FDC>
  void
  StateProblem<OPTPROBLEM, PDE, DD, SPARSITYPATTERN, VECTOR, dim>::BoundaryEquation (const FDC &fdc,
      dealii::Vector<
      double> &local_vector,
      double scale,
      double scale_ico)
  {
    pde_.BoundaryEquation (fdc, local_vector, scale * interval_length_,
                           scale_ico * interval_length_);
  }
  ...
  template <typename EDC>
  void
  StateProblem<OPTPROBLEM, PDE, DD, SPARSITYPATTERN, VECTOR, dim>::ElementRhs (const EDC &edc,
      dealii::Vector<
      double> &local_vector,
      double scale)
  {
  ...
  template <typename OPTPROBLEM, typename PDE, typename DD,
            typename SPARSITYPATTERN, typename VECTOR, int dim>
  void
  StateProblem<OPTPROBLEM, PDE, DD, SPARSITYPATTERN, VECTOR, dim>::PointRhs (const std::map<
      std::string,
      const dealii::Vector<
      double>*> &/*param_values*/,
      const std::map<
  ...
  template <typename FDC>
  void
  StateProblem<OPTPROBLEM, PDE, DD, SPARSITYPATTERN, VECTOR, dim>::FaceRhs (const FDC &fdc,
      dealii::Vector<
      double> &local_vector,
      double scale)
  {
  ...
  template <typename FDC>
  void
  StateProblem<OPTPROBLEM, PDE, DD, SPARSITYPATTERN, VECTOR, dim>::BoundaryRhs (const FDC &fdc,
      dealii::Vector<
      double> &local_vector,
      double scale)
  {
    pde_.BoundaryRightHandSide (fdc, local_vector,
  ...
  template <typename EDC>
  void
  StateProblem<OPTPROBLEM, PDE, DD, SPARSITYPATTERN, VECTOR, dim>::ElementMatrix (const EDC &edc,
      dealii::FullMatrix<
      double> &local_entry_matrix,
      double scale,
      double scale_ico)
  {
    pde_.ElementMatrix (edc, local_entry_matrix, scale * interval_length_,
                        scale_ico * interval_length_);
  }
  ...
  template <typename EDC>
  void
  StateProblem<OPTPROBLEM, PDE, DD, SPARSITYPATTERN, VECTOR, dim>::ElementTimeMatrix (const EDC &edc,
      FullMatrix<
      double> &local_entry_matrix)
  {
  ...
  template <typename EDC>
  void
  StateProblem<OPTPROBLEM, PDE, DD, SPARSITYPATTERN, VECTOR, dim>::ElementTimeMatrixExplicit (const EDC &edc,
      dealii::FullMatrix<
      double> &local_entry_matrix)
  {
  ...
  template <typename FDC>
  void
  StateProblem<OPTPROBLEM, PDE, DD, SPARSITYPATTERN, VECTOR, dim>::FaceMatrix (const FDC &fdc,
      FullMatrix<
      double> &local_entry_matrix,
      double scale,
      double scale_ico)
  {
    pde_.FaceMatrix (fdc, local_entry_matrix, scale * interval_length_,
                     scale_ico * interval_length_);
  }
  ...
  template <typename FDC>
  void
  StateProblem<OPTPROBLEM, PDE, DD, SPARSITYPATTERN, VECTOR, dim>::InterfaceMatrix (const FDC &fdc,
      FullMatrix<
      double> &local_entry_matrix,
      double scale,
      double scale_ico)
  {
    pde_.InterfaceMatrix (fdc, local_entry_matrix, scale * interval_length_,
                          scale_ico * interval_length_);
  }
  ...
  template <typename FDC>
  void
  StateProblem<OPTPROBLEM, PDE, DD, SPARSITYPATTERN, VECTOR, dim>::BoundaryMatrix (const FDC &fdc,
      FullMatrix<
      double> &local_matrix,
      double scale,
      double scale_ico)
  {
    pde_.BoundaryMatrix (fdc, local_matrix, scale * interval_length_,
                         scale_ico * interval_length_);
  }
  ...
  template <typename OPTPROBLEM, typename PDE, typename DD,
            typename SPARSITYPATTERN, typename VECTOR, int dim>
  void
  StateProblem<OPTPROBLEM, PDE, DD, SPARSITYPATTERN, VECTOR, dim>::SetTime (double time,
      unsigned int time_dof_number,
      const TimeIterator &interval,
      bool initial)
  {
  ...
  template <typename OPTPROBLEM, typename PDE, typename DD,
            typename SPARSITYPATTERN, typename VECTOR, int dim>
  void
  StateProblem<OPTPROBLEM, PDE, DD, SPARSITYPATTERN, VECTOR, dim>::ComputeSparsityPattern (SPARSITYPATTERN &sparsity) const
  {
...
//
  ...
  StateProblem<OPTPROBLEM, PDE, DD, SPARSITYPATTERN, VECTOR, dim>::GetDirichletCompMask (unsigned int color) const
  {
  ...
  template <typename OPTPROBLEM, typename PDE, typename DD,
            typename SPARSITYPATTERN, typename VECTOR, int dim>
  const Function<dim> &
  StateProblem<OPTPROBLEM, PDE, DD, SPARSITYPATTERN, VECTOR, dim>::GetDirichletValues (unsigned int color,
      const std::map<
      std::string,
      const dealii::Vector<
      double>*> &param_values,
  ...
  template <typename ELEMENTITERATOR>
  bool
  StateProblem<OPTPROBLEM, PDE, DD, SPARSITYPATTERN, VECTOR, dim>::AtInterface (ELEMENTITERATOR &element,
      unsigned int face) const
  {
