...
namespace DOpE
{
  ...
  {
  public:
    OPT_Adjoint_For_EEProblem (OPTPROBLEM &OP,
                               PDE &pde)
      : pde_ (pde),
        opt_problem_ (OP)
    {
    ...
    {
      throw DOpEException (
        "This method is not implemented; the problem is tested for stationary problems, only!",
        "OPT_Adjoint_For_EEProblem::Init_ElementEquation");
    }
                     ...
                     double /*scale*/)
    {
      throw DOpEException (
        "This method is not implemented; the problem is tested for stationary problems, only!",
        "OPT_Adjoint_For_EEProblem::Init_ElementRhs");
    }
                   ...
                   double /*scale=1.*/)
    {
      throw DOpEException (
        "This method is not implemented; the problem is tested for stationary problems, only!",
        "OPT_Adjoint_For_EEProblem::Init_PointRhs");
                        ...
                        double /*scale_ico*/)
    {
      throw DOpEException (
        "This method is not implemented; the problem is tested for stationary problems, only!",
        "OPT_Adjoint_For_EEProblem::Init_ElementMatrix");
    }
...
//              unsigned int n_levels) const;
        ...
        if (opt_problem_.aux_functionals_[opt_problem_.functional_for_ee_num_]->GetType ().find (
            ...
            != std::string::npos)
          {
            if (opt_problem_.aux_functionals_[opt_problem_.functional_for_ee_num_]->GetType ().find (
                ...
                != std::string::npos)
              {
                opt_problem_.aux_functionals_[opt_problem_.functional_for_ee_num_]->ElementValue_U (
                  edc, local_vector, scale * interval_length_);
              }
            else
              {
                opt_problem_.aux_functionals_[opt_problem_.functional_for_ee_num_]->ElementValue_U (
                  edc, local_vector, scale);
              }
            if (opt_problem_.aux_functionals_[opt_problem_.functional_for_ee_num_]->GetType ().find (
                  "timedistributed")
                != std::string::npos
                && opt_problem_.aux_functionals_[opt_problem_.functional_for_ee_num_]->GetType ().find (
                  "timelocal")
                != std::string::npos)
              {
                throw DOpEException (
                  "Conflicting functional types: " + opt_problem_.aux_functionals_[opt_problem_.functional_for_ee_num_]->GetType (),
  ...
  template <typename OPTPROBLEM, typename PDE, typename DD,
            typename SPARSITYPATTERN, typename VECTOR, int dim>
  void
  OPT_Adjoint_For_EEProblem<OPTPROBLEM, PDE, DD, SPARSITYPATTERN, VECTOR, dim>::PointRhs (const std::map<
      std::string,
      const dealii::Vector<
      double>*> &param_values,
      const std::map<
        ...
        if (opt_problem_.aux_functionals_[opt_problem_.functional_for_ee_num_]->GetType ().find (
            ...
            != std::string::npos)
          {
            if (opt_problem_.aux_functionals_[opt_problem_.functional_for_ee_num_]->GetType ().find (
                ...
                != std::string::npos)
              {
                opt_problem_.aux_functionals_[opt_problem_.functional_for_ee_num_]->FaceValue_U (
                  fdc, local_vector, scale * interval_length_);
              }
            else
              {
                opt_problem_.aux_functionals_[opt_problem_.functional_for_ee_num_]->FaceValue_U (
                  fdc, local_vector, scale);
              }
            if (opt_problem_.aux_functionals_[opt_problem_.functional_for_ee_num_]->GetType ().find (
                  "timedistributed")
                != std::string::npos
        ...
        if (opt_problem_.aux_functionals_[opt_problem_.functional_for_ee_num_]->GetType ().find (
            ...
            != std::string::npos)
          {
            if (opt_problem_.aux_functionals_[opt_problem_.functional_for_ee_num_]->GetType ().find (
                ...
                != std::string::npos)
              {
                opt_problem_.aux_functionals_[opt_problem_.functional_for_ee_num_]->BoundaryValue_U (
                  fdc, local_vector, scale * interval_length_);
              }
            else
              {
  ...
  template <typename OPTPROBLEM, typename PDE, typename DD,
            typename SPARSITYPATTERN, typename VECTOR, int dim>
  void
  OPT_Adjoint_For_EEProblem<OPTPROBLEM, PDE, DD, SPARSITYPATTERN, VECTOR, dim>::SetTime (double time,
      unsigned int time_dof_number,
      const TimeIterator &interval,
      bool initial)
  {
  ...
  template <typename OPTPROBLEM, typename PDE, typename DD,
            typename SPARSITYPATTERN, typename VECTOR, int dim>
  void
  OPT_Adjoint_For_EEProblem<OPTPROBLEM, PDE, DD, SPARSITYPATTERN, VECTOR, dim>::ComputeSparsityPattern (SPARSITYPATTERN &sparsity) const
  {
...
//
  ...
  OPT_Adjoint_For_EEProblem<OPTPROBLEM, PDE, DD, SPARSITYPATTERN, VECTOR, dim>::GetDirichletCompMask (unsigned int color) const
  {
    ...
    if (comp == dirichlet_colors_.size ())
      {
        ...
        throw DOpEException (s.str (),
                             "OPT_Adjoint_For_EEProblem::GetDirichletCompMask");
      }
  ...
  template <typename OPTPROBLEM, typename PDE, typename DD,
            typename SPARSITYPATTERN, typename VECTOR, int dim>
  const Function<dim> &
  OPT_Adjoint_For_EEProblem<OPTPROBLEM, PDE, DD, SPARSITYPATTERN, VECTOR, dim>::GetDirichletValues (unsigned int color,
      const std::map<
      std::string,
      const dealii::Vector<
      double>*> &/*param_values*/,
  ...
  bool
  OPT_Adjoint_For_EEProblem<OPTPROBLEM, PDE, DD, SPARSITYPATTERN, VECTOR,
                            dim>::AtInterface (ELEMENTITERATOR &element,
                                               unsigned int face) const
  {
