...
#ifndef EIGENVALUEDERIVATIVE_PROBLEM_H_
#define EIGENVALUEDERIVATIVE_PROBLEM_H_
...
namespace DOpE
{
  ...
  class EigenvalueDerivativeProblem
  {
  public:
    EigenvalueDerivativeProblem(OPTPROBLEM &OP, PDE &pde) :
    ...
    GetName() const
    {
      return "EigenvalueDerivativeProblem";
    }
    std::string
    GetType() const
    {
      return "eigenvaluederivative";
    }
...
//              unsigned int n_levels) const;
  ...
  template<typename EDC>
  void
  EigenvalueDerivativeProblem<OPTPROBLEM, PDE, DD, SPARSITYPATTERN, VECTOR,
                 dim>::ElementEquation(const EDC &edc,
                                       dealii::Vector<double> &local_vector, double scale,
                                       double scale_ico)
  {
    pde_.ElementEquation_Q(edc, local_vector, scale*interval_length_/*, scale_ico*interval_length_*/);
  ...
  /******************************************************/
    ...
    {
    	pde_.ElementMassEquation_Q(edc, local_vector, scale*interval_length_, scale_ico*interval_length_);
  ...
  EigenvalueDerivativeProblem<OPTPROBLEM, PDE, DD, SPARSITYPATTERN, VECTOR,
                 dim>::ElementTimeEquation(const EDC &edc,
                                           dealii::Vector<double> &local_vector, double scale)
  {
    pde_.ElementTimeEquation_Q(edc, local_vector, scale);
  }
  ...
  EigenvalueDerivativeProblem<OPTPROBLEM, PDE, DD, SPARSITYPATTERN, VECTOR,
                 dim>::ElementTimeEquationExplicit(const EDC &edc,
                                                   dealii::Vector<double> &local_vector, double scale)
  {
    pde_.ElementTimeEquationExplicit_Q(edc, local_vector,
                                       scale);
  }
  ...
  EigenvalueDerivativeProblem<OPTPROBLEM, PDE, DD, SPARSITYPATTERN, VECTOR,
                 dim>::FaceEquation(const FDC &fdc,
                                    dealii::Vector<double> &local_vector, double scale, double scale_ico)
  {
    pde_.FaceEquation_Q(fdc, local_vector, scale*interval_length_, scale_ico*interval_length_);
  }
...
//                                         dealii::Vector<double> &local_vector, double scale, double scale_ico)
//  {
//    pde_.InterfaceEquation_Q(fdc,  local_vector, scale*interval_length_, scale_ico*interval_length_);
//  }
  /******************************************************/
  ...
  EigenvalueDerivativeProblem<OPTPROBLEM, PDE, DD, SPARSITYPATTERN, VECTOR,
                 dim>::BoundaryEquation(const FDC &fdc,
                                        dealii::Vector<double> &local_vector, double scale, double scale_ico)
  {
    pde_.BoundaryEquation_Q(fdc, local_vector, scale*interval_length_, scale_ico*interval_length_);
  }
  ...
  template<typename OPTPROBLEM, typename PDE, typename DD,
           typename SPARSITYPATTERN, typename VECTOR, int dim>
  void
  EigenvalueDerivativeProblem<OPTPROBLEM, PDE, DD, SPARSITYPATTERN, VECTOR, dim>::PointRhs(
    const std::map<std::string, const dealii::Vector<double>*> &param_values,
    const std::map<std::string, const VECTOR *> &domain_values,
    VECTOR &rhs_vector, double scale)
  {
        ...
        if (opt_problem_.GetFunctional()->GetType().find("point") != std::string::npos)
          {
            ...
            if (opt_problem_.GetFunctional()->GetType().find("timedistributed") != std::string::npos && opt_problem_.GetFunctional()->GetType().find("timelocal") != std::string::npos)
              {
                throw DOpEException("Conflicting functional types: "+ opt_problem_.GetFunctional()->GetType(),
                                    "EigenvalueAdjointProblem::PointRhs");
              }
          }
      }
  }
        ...
        if (opt_problem_.GetFunctional()->GetType().find("face") != std::string::npos)
          {
            ...
            if (opt_problem_.GetFunctional()->GetType().find("timedistributed") != std::string::npos && opt_problem_.GetFunctional()->GetType().find("timelocal") != std::string::npos)
              {
                throw DOpEException("Conflicting functional types: "+ opt_problem_.GetFunctional()->GetType(),
                                    "EigenvalueAdjointProblem::FaceRhs");
              }
          }
      }
  }
        ...
        if (opt_problem_.GetFunctional()->GetType().find("boundary") != std::string::npos)
          {
            ...
            if (opt_problem_.GetFunctional()->GetType().find("timedistributed") != std::string::npos && opt_problem_.GetFunctional()->GetType().find("timelocal") != std::string::npos)
              {
                throw DOpEException("Conflicting functional types: "+ opt_problem_.GetFunctional()->GetType(),
                                    "EigenvalueAdjointProblem::BoundaryRhs");
              }
          }
      }
  }
  ...
  template<typename OPTPROBLEM, typename PDE, typename DD,
           typename SPARSITYPATTERN, typename VECTOR, int dim>
  std::string
  EigenvalueDerivativeProblem<OPTPROBLEM, PDE, DD, SPARSITYPATTERN, VECTOR, dim>::GetDoFType() const
  {
    return "state";
  }
  ...
  EigenvalueDerivativeProblem<OPTPROBLEM, PDE, DD, SPARSITYPATTERN, VECTOR, dim>::GetFESystem() const
  {
  ...
  EigenvalueDerivativeProblem<OPTPROBLEM, PDE, DD, SPARSITYPATTERN, VECTOR, dim>::GetFECollection() const
  {
  ...
  template<typename OPTPROBLEM, typename PDE, typename DD,
           typename SPARSITYPATTERN, typename VECTOR, int dim>
  UpdateFlags
  EigenvalueDerivativeProblem<OPTPROBLEM, PDE, DD, SPARSITYPATTERN, VECTOR, dim>::GetUpdateFlags() const
  {
    UpdateFlags r;
  ...
  template<typename OPTPROBLEM, typename PDE, typename DD,
           typename SPARSITYPATTERN, typename VECTOR, int dim>
  UpdateFlags
  EigenvalueDerivativeProblem<OPTPROBLEM, PDE, DD, SPARSITYPATTERN, VECTOR, dim>::GetFaceUpdateFlags() const
  {
    UpdateFlags r;
  ...
  template<typename OPTPROBLEM, typename PDE, typename DD,
           typename SPARSITYPATTERN, typename VECTOR, int dim>
  void
  EigenvalueDerivativeProblem<OPTPROBLEM, PDE, DD, SPARSITYPATTERN, VECTOR, dim>::SetTime(
    double time,
  ...
  template<typename OPTPROBLEM, typename PDE, typename DD,
           typename SPARSITYPATTERN, typename VECTOR, int dim>
  void
  EigenvalueDerivativeProblem<OPTPROBLEM, PDE, DD, SPARSITYPATTERN, VECTOR, dim>::ComputeSparsityPattern(
...
//    }
  ...
  template<typename OPTPROBLEM, typename PDE, typename DD,
           typename SPARSITYPATTERN, typename VECTOR, int dim>
  bool
  EigenvalueDerivativeProblem<OPTPROBLEM, PDE, DD, SPARSITYPATTERN, VECTOR, dim>::HasFaces() const
  {
  ...
  template<typename OPTPROBLEM, typename PDE, typename DD,
           typename SPARSITYPATTERN, typename VECTOR, int dim>
  bool
  EigenvalueDerivativeProblem<OPTPROBLEM, PDE, DD, SPARSITYPATTERN, VECTOR, dim>::HasPoints() const
  {
  ...
  template<typename OPTPROBLEM, typename PDE, typename DD,
           typename SPARSITYPATTERN, typename VECTOR, int dim>
  bool
  EigenvalueDerivativeProblem<OPTPROBLEM, PDE, DD, SPARSITYPATTERN, VECTOR, dim>::HasInterfaces() const
  {
  ...
  template<typename OPTPROBLEM, typename PDE, typename DD,
           typename SPARSITYPATTERN, typename VECTOR, int dim>
  bool
  EigenvalueDerivativeProblem<OPTPROBLEM, PDE, DD, SPARSITYPATTERN, VECTOR, dim>::HasVertices() const
  {
  ...
  EigenvalueDerivativeProblem<OPTPROBLEM, PDE, DD, SPARSITYPATTERN, VECTOR, dim>::GetDirichletColors() const
  {
  ...
  EigenvalueDerivativeProblem<OPTPROBLEM, PDE, DD, SPARSITYPATTERN, VECTOR, dim>::GetDirichletCompMask(
    ...
    if (comp == dirichlet_colors_.size())
      {
        ...
        throw DOpEException(s.str(), "EigenvalueAdjointProblem::GetDirichletCompMask");
      }
  ...
  template<typename OPTPROBLEM, typename PDE, typename DD,
           typename SPARSITYPATTERN, typename VECTOR, int dim>
  const Function<dim> &
  EigenvalueDerivativeProblem<OPTPROBLEM, PDE, DD, SPARSITYPATTERN, VECTOR, dim>::GetDirichletValues(
    unsigned int color,
    const std::map<std::string, const dealii::Vector<double>*> &/*param_values*/,
    const std::map<std::string, const VECTOR *> &/*domain_values*/) const
  {
  ...
  template<typename OPTPROBLEM, typename PDE, typename DD,
           typename SPARSITYPATTERN, typename VECTOR, int dim>
  const std::vector<unsigned int> &
  EigenvalueDerivativeProblem<OPTPROBLEM, PDE, DD, SPARSITYPATTERN, VECTOR, dim>::GetBoundaryEquationColors() const
  {
  ...
  EigenvalueDerivativeProblem<OPTPROBLEM, PDE, DD, SPARSITYPATTERN, VECTOR, dim>::GetDoFConstraints() const
  {
  ...
  template<typename OPTPROBLEM, typename PDE, typename DD,
           typename SPARSITYPATTERN, typename VECTOR, int dim>
  const dealii::AffineConstraints<double> &
  EigenvalueDerivativeProblem<OPTPROBLEM, PDE, DD, SPARSITYPATTERN, VECTOR, dim>::GetHNConstraints() const
  {
  ...
  EigenvalueDerivativeProblem<OPTPROBLEM, PDE, DD, SPARSITYPATTERN, VECTOR, dim>::GetInitialValues() const
  {
  ...
  template<typename OPTPROBLEM, typename PDE, typename DD,
           typename SPARSITYPATTERN, typename VECTOR, int dim>
  template<typename ELEMENTITERATOR>
  bool EigenvalueDerivativeProblem<OPTPROBLEM, PDE, DD, SPARSITYPATTERN, VECTOR, dim>
